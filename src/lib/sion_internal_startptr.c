...
#define DFUNCTION "_lsion_calculate_set_alignment"
sion_int64 _lsion_calculate_set_alignment( _sion_filedesc *sion_filedesc, int t) {
 ...
 */
#define DFUNCTION "_lsion_get_size_metadatablock1"
int _lsion_get_size_metadatablock1( _sion_filedesc *sion_filedesc )
{
  int rc = 0;
  int firstsize;
 ...
 */
#define DFUNCTION "_lsion_calculate_startpointers"
int _lsion_calculate_startpointers( _sion_filedesc *sion_filedesc )
{
  ...
  firstsize=_lsion_get_size_metadatablock1(sion_filedesc);
  ...
  for (i = 1; i < sion_filedesc->ntasks; i++) {
    ...
    lsize=_lsion_calculate_set_alignment(sion_filedesc,i-1);
  ...
  lsize=_lsion_calculate_set_alignment(sion_filedesc,sion_filedesc->ntasks - 1);
  sion_filedesc->globalskip += lsize;
 ...
 * @retval SION_SUCCESS if ok
 */
#define DFUNCTION "_lsion_calculate_num_collector"
int _lsion_calculate_num_collector( _sion_filedesc *sion_filedesc, _sion_collstat *collstat, int *num_collectors) {
  int        rc = SION_SUCCESS;
 ...
 * @retval SION_SUCCESS if ok
 */
#define DFUNCTION "_lsion_calculate_startpointers_collective"
int _lsion_calculate_startpointers_collective( _sion_filedesc *sion_filedesc )
{
  ...
  if (sion_filedesc->fileversion<=3) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,
			    "_lsion_calculate_startpointers_collective: files with old sionlib file format (<3) can not be read by collective calls, please use standard read calls, aborting ...\n"));
  }

  /* statistics */
  collstat=_lsion_create_and_init_collstat(sion_filedesc);

  /* calculate size of first block for meta-information */
  collstat->firstsize=firstsize=_lsion_get_size_metadatablock1(sion_filedesc);
  _lsion_calculate_num_collector(sion_filedesc, collstat, &num_collectors);
  ...
  if(sion_filedesc->colldebug>=1) {
    _lsion_update_collstat(collstat,sion_filedesc);
    _lsion_print_collstat(collstat, sion_filedesc);
  }
  
  _lsion_debugprint_collstat(collstat, sion_filedesc);
  
  _lsion_destroy_collstat(collstat);
...
int _lsion_calculate_startpointers_collective_msa(_sion_filedesc *fd)
{
  ...
  if (fd->fileversion<=3) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,
			    "_lsion_calculate_startpointers_collective_msa: files with old sionlib file format (<3) can not be read by collective calls, please use standard read calls, aborting ...\n"));
  }

  sion_int64 firstsize = _lsion_get_size_metadatablock1(fd);
 ...
 * @retval SION_SUCCESS if ok
 */
#define DFUNCTION "_lsion_calculate_startpointers_collective_merge"
int _lsion_calculate_startpointers_collective_merge( _sion_filedesc *sion_filedesc )
{
  ...
  for (i = 1; i < sion_filedesc->ntasks; i++) {
	...
	for(s=lastcoll+1;s<i;s++) {
	  /* leave chunksize of sender on aligned original size to
	     ...
	     flushes */
	  _lsion_calculate_set_alignment(sion_filedesc,s);
