...
#define DFUNCTION "_lsion_create_and_init_collstat"
_sion_collstat * _lsion_create_and_init_collstat( _sion_filedesc *sion_filedesc ) {
  _sion_collstat *collstat;
  int t;

  collstat = (_sion_collstat *) malloc(sizeof(_sion_collstat));
  if (collstat == NULL) {
    _lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"cannot allocate collstat structure of size %lu (sion_collstat), aborting ...\n",
...
#define DFUNCTION "_lsion_update_collstat"
int _lsion_update_collstat( _sion_collstat *collstat, _sion_filedesc *sion_filedesc ) {
  int rc = SION_SUCCESS;
  int t, s; 
...
#define DFUNCTION "_lsion_print_collstat"
int _lsion_print_collstat( _sion_collstat *collstat, _sion_filedesc *sion_filedesc ) {
  int rc = SION_SUCCESS;
  int t,s;
...
#define DFUNCTION "_lsion_debugprint_collstat"
int _lsion_debugprint_collstat( _sion_collstat *sion_collstat, _sion_filedesc *sion_filedesc ) {
  int rc = SION_SUCCESS;
  int t, s; 
...
int _lsion_destroy_collstat( _sion_collstat *sion_collstat ) {
  int rc = SION_SUCCESS;
