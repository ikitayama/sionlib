...
#define DFUNCTION "_lsion_keyvalue_keymngr_init"
_sion_keyvalue_keymngr* _lsion_keyvalue_keymngr_init(int size) {
  ...
  if (keymngr == NULL) {
    _lsion_errorprint(0,_SION_ERROR_RETURN,"cannot allocate internal keyvalue keymngr of size %lu , aborting ...\n", (unsigned long) sizeof(_sion_keyvalue_keymngr));
    return(NULL);
  }
  ...
  keymngr->size = size;
  keymngr->key_table     = _lsion_keyvalue_table_init(size);
  if (keymngr->key_table == NULL) {
    _lsion_errorprint(0,_SION_ERROR_RETURN,"cannot allocate internal keyvalue table of for %lu entries , aborting ...\n", (unsigned long) size);
    free(keymngr);
    return(NULL);
  }
...
#define DFUNCTION "_lsion_keyvalue_keymngr_destroy"
int _lsion_keyvalue_keymngr_destroy(_sion_keyvalue_keymngr** keymngr) {
   ...
   if((*keymngr)->key_table) {
     rc=_lsion_keyvalue_table_destroy(&((*keymngr)->key_table));
   }
   free((*keymngr));
   *keymngr = NULL;
...
#define DFUNCTION "_lsion_keyvalue_keymngr_add_block"
int _lsion_keyvalue_keymngr_add_block(_sion_keyvalue_keymngr* keymngr, sion_table_key_t key, size_t offset, size_t len) {
  size_t rc=SION_SUCCESS;
  ...
  p=_lsion_keyvalue_table_lookup(keymngr->key_table,key);
  if( p==NULL ) {
    /* new key */

    entry = (_sion_key_entry*) malloc(sizeof(_sion_key_entry));
    if (entry == NULL) {
      return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"cannot allocate internal keyvalue table entry, aborting ...\n"));
    }
    entry->key         = key;
    ...
    rc=_lsion_keyvalue_table_store(keymngr->key_table,key,p);
  ...
  if (new_block == NULL) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"cannot allocate internal keyvalue table block entry, aborting ...\n"));
  }
...
#define DFUNCTION "_lsion_keyvalue_keymngr_update_read_pos"
int _lsion_keyvalue_keymngr_update_read_pos(_sion_keyvalue_keymngr* keymngr, sion_table_key_t key, 
...
#define DFUNCTION "_lsion_keyvalue_keymngr_set_next_scan_pos"
int _lsion_keyvalue_keymngr_set_next_scan_pos(_sion_keyvalue_keymngr* keymngr, size_t pos) {
  size_t rc=SION_SUCCESS;
...
#define DFUNCTION "_lsion_keyvalue_keymngr_get_next_scan_pos"
int _lsion_keyvalue_keymngr_get_next_scan_pos(_sion_keyvalue_keymngr* keymngr, size_t *pos) {
  size_t rc=SION_SUCCESS;
  DPRINTFP((2, DFUNCTION, -1, "enter\n"));
...
#define DFUNCTION "_lsion_keyvalue_keymngr_set_scan_done"
int _lsion_keyvalue_keymngr_set_scan_done(_sion_keyvalue_keymngr* keymngr) {
  size_t rc=SION_SUCCESS;
...
#define DFUNCTION "_lsion_keyvalue_keymngr_is_scan_done"
int _lsion_keyvalue_keymngr_is_scan_done(_sion_keyvalue_keymngr* keymngr) {
...
#define DFUNCTION "_lsion_keyvalue_keymngr_lookup"
int _lsion_keyvalue_keymngr_lookup(_sion_keyvalue_keymngr* keymngr, sion_table_key_t key, size_t *current_pos, size_t *bytes_left) {
  size_t rc=SION_SUCCESS;
  _sion_key_entry *entry;
  void *p;
...
#define DFUNCTION "_lsion_keyvalue_keymngr_lookup_and_set_pos"
int _lsion_keyvalue_keymngr_lookup_and_set_pos(_sion_keyvalue_keymngr* keymngr, sion_table_key_t key, int blocknum, sion_int64 posinblock,
  ...
  *bytes_left  = 0;
  ...
  if(blocknum != SION_ABSOLUTE_POS) {
    ...
    if(block==NULL) {
      ...
      return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"internal error: somethink went wrong, seek, aborting ...\n"));
    }
...
#define DFUNCTION "_lsion_keyvalue_keymngr_iterator_reset"
  int _lsion_keyvalue_keymngr_iterator_reset(_sion_keyvalue_keymngr* keymngr) {
...
#define DFUNCTION "_lsion_keyvalue_keymngr_iterator_next"
  int _lsion_keyvalue_keymngr_iterator_next(_sion_keyvalue_keymngr* keymngr, sion_table_key_t *key, size_t *current_pos, size_t *offset, size_t *len) {
    int rc=SION_SUCCESS;
    int found = 0;
	...
	if(keymngr->iterator_last_block==NULL) {
	  return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"internal error: somethink went wrong, checking end of list , aborting ...\n"));
	}    
	found=1;
      }
    }
...
#define DFUNCTION "_lsion_keyvalue_keymngr_key_list_iterator_reset"
  int _lsion_keyvalue_keymngr_key_list_iterator_reset(_sion_keyvalue_keymngr* keymngr) {

    int rc=SION_SUCCESS;

    DPRINTFP((2, DFUNCTION, -1, "enter\n"));
  
    _lsion_keyvalue_table_iterator_reset(keymngr->key_table);
...
#define DFUNCTION "_lsion_keyvalue_keymngr_key_list_iterator_next"
  int _lsion_keyvalue_keymngr_key_list_iterator_next(_sion_keyvalue_keymngr* keymngr, sion_table_key_t *key) {
    int rc=0;
    void *data;

    DPRINTFP((2, DFUNCTION, -1, "enter\n"));

    rc=_lsion_keyvalue_table_iterator_next_in_store_order(keymngr->key_table, key, &data);
...
#define DFUNCTION "_lsion_keyvalue_keymngr_dup"
_sion_keyvalue_keymngr* _lsion_keyvalue_keymngr_dup(_sion_keyvalue_keymngr* keymngr_orig, int dup_mode, sion_table_key_t sel_key) {
    _sion_keyvalue_keymngr *keymngr=NULL;
    _sion_key_block_entry  *block; 
    _sion_key_entry *entry;
    void *p;
    ...
    keymngr->key_table = _lsion_keyvalue_table_init(keymngr_orig->size);
    if (keymngr->key_table == NULL) {
    ...
    if(dup_mode==SION_DESCSTATE_DUP_SEL_RANK_KEY) {
      ...
      p=_lsion_keyvalue_table_lookup(keymngr_orig->key_table,sel_key);
      if( p!=NULL ) {
	entry = (_sion_key_entry *) p;
	block=entry->blocklist_head;
	while(block!=NULL) {
	  ...
	  _lsion_keyvalue_keymngr_add_block(keymngr,block->entry->key,block->offset,block->len);
	  block=block->next;
	}
      }
...
#define DFUNCTION "_lsion_keyvalue_keymngr_key_get_stat"
int _lsion_keyvalue_keymngr_key_get_stat(_sion_keyvalue_keymngr* keymngr, sion_table_key_t key, sion_key_stat_t *keystat) {
  size_t rc=SION_SUCCESS;
  _sion_key_entry *entry;
  void *p;
...
#define DFUNCTION "_lsion_keyvalue_keymngr_key_get_sizeof"
int _lsion_keyvalue_keymngr_key_get_sizeof(_sion_keyvalue_keymngr* keymngr) {
  ...
  if(keymngr->key_table) {
    help_bytes=_lsion_keyvalue_table_get_size(keymngr->key_table);
    DPRINTFP((2, DFUNCTION, -1, " sizeof key_blocks=          %5d\n", help_bytes));
    bytes+=help_bytes;
