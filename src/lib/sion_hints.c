 ...
 */
int  _lsion_hints_check_env(_sion_filedesc *sion_filedesc) {
  const char *t;
  int         rc = SION_SUCCESS;

  t = _lsion_getenv("SION_HINTS");
  if(t) {
    if(atoi(t) == 1) {
  ...
  t = _lsion_getenv("SION_HINT_TYPE_GPFS");
  if(t) {
    if(atoi(t) == 1) {
      sion_filedesc->hinttype=SION_HINTS_TYPE_GPFS;
    }
  }
  t = _lsion_getenv("SION_HINT_TYPE_LINUX");
  if(t) {
    if(atoi(t) == 1) {
      sion_filedesc->hinttype=SION_HINTS_TYPE_LINUX;
    }
  ...
  DPRINTFP((2, "_lsion_hints_check_env", -1, "usehints=%d hinttype=%d\n", sion_filedesc->usehints, sion_filedesc->hinttype));
 ...
 */
int  _lsion_apply_hints(_sion_filedesc *sion_filedesc, int access_type) {
  int rc=SION_SUCCESS;

  DPRINTFP((2, "_lsion_apply_hints", -1, "start usehints=%d hinttype=%d access_type=%d\n",
  ...
  if(sion_filedesc->hinttype==SION_HINTS_TYPE_LINUX) {
    DPRINTFP((2, "_lsion_apply_hints", -1, "apply Linux hints\n"));
    int fd=_lsion_file_get_fd(sion_filedesc->fileptr);
    int iswrite=sion_filedesc->mode == SION_FILEMODE_WRITE;
    if(access_type==SION_HINTS_ACCESS_TYPE_METADATABLOCK1) {
      long long startpos = 0;
      long long length   = _lsion_get_size_metadatablock1( sion_filedesc );
     
      DPRINTFP((2, "_lsion_apply_hints", -1, "apply linux hints for metablock1 (%d,%lld,%lld)\n",fd, startpos, length));
      rc=_lsion_apply_hint_linux_access_range(fd, startpos, length, iswrite);
    }
  }
#endif
  ...
  if(sion_filedesc->hinttype==SION_HINTS_TYPE_GPFS) {
    ...
    if(access_type==SION_HINTS_ACCESS_TYPE_METADATABLOCK1) {
      long long startpos = 0;
      long long length   = _lsion_get_size_metadatablock1( sion_filedesc );
      DPRINTFP((2, "_lsion_apply_hints", -1, "apply GPFS hints for metablock1 (%d,%lld,%lld)\n",fd, startpos, length));
      rc=_lsion_apply_hint_gpfs_access_range(fd, startpos, length, iswrite);
    }
    ...
    if(access_type==SION_HINTS_ACCESS_TYPE_CHUNK) {
      ...
      DPRINTFP((2, "_lsion_apply_hints", -1, "apply  GPFS hints (access) for chunk (%d,%lld,%lld)\n",fd, startpos, length));
      rc=_lsion_apply_hint_gpfs_access_range(fd, startpos, length, iswrite);
    }
    ...
    if(access_type==SION_HINTS_FREE_TYPE_CHUNK) {
      long long startpos = sion_filedesc->startpos + sion_filedesc->currentblocknr * sion_filedesc->globalskip;
      long long length   = sion_filedesc->chunksize;
      DPRINTFP((2, "_lsion_apply_hints", -1, "apply GPFS hints (free)    for metablock1 (%d,%lld,%lld)\n",fd, startpos, length));
      rc=_lsion_apply_hint_gpfs_free_range(fd, startpos, length, iswrite);
    }
