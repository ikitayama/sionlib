...
#define DFUNCTION "_lsion_flags_init_entry"
void _lsion_flags_init_entry(_sion_flags_entry* entry)
{
...
#define DFUNCTION "_lsion_flags_create_entry"
/*! \brief Create a flags entry
 *
 * Use _lsion_flags_destroy_entry to free the memory when not needed any longer.
 *
 * @return     flag entry
 */
_sion_flags_entry* _lsion_flags_create_entry(void)
{
  ...
  _lsion_flags_init_entry(new_entry);
...
#define DFUNCTION "_lsion_flags_destroy_entry"
void _lsion_flags_destroy_entry(_sion_flags_entry** entry)
{
  DPRINTFP((2, DFUNCTION, -1, "enter\n"));
...
#define DFUNCTION "_lsion_flags_iter"
_sion_flags_entry* _lsion_flags_iter(_sion_flags_store* store)
{
  DPRINTFP((2, DFUNCTION, -1, "enter\n"));
  DPRINTFP((2, DFUNCTION, -1, "leave\n"));
...
#define DFUNCTION "_lsion_flags_init_store"
void _lsion_flags_init_store(_sion_flags_store* store)
{
  DPRINTFP((2, DFUNCTION, -1, "enter\n"));

  store->root    = _lsion_flags_create_entry();
...
#define DFUNCTION "_lsion_flags_create_store"
/*! \brief Create a flags entry
 *
 * Use _lsion_flags_destroy_store to free the memory when not needed any longer.
 *
 * @return     flag store with flag entries (key value pairs)
 */
_sion_flags_store* _lsion_flags_create_store(void)
{
  ...
  _lsion_flags_init_store(new_store);
...
#define DFUNCTION "_lsion_flags_destroy_store"
void _lsion_flags_destroy_store(_sion_flags_store** store)
{
  ...
  entry = _lsion_flags_iter(*store);

  while (entry != NULL) {
    next = entry->next;
    _lsion_flags_destroy_entry(&entry);
    entry = next;
  }
...
#define DFUNCTION "_lsion_flags_add_entry"
/*! \brief Create a flags entry
 *
 * Memory is deallocated in _lsion_flags_destroy_entry.
 *
 ...
 */
_sion_flags_entry* _lsion_flags_add_entry(_sion_flags_entry* entry,
                                         const char*        key,
                                         const char*        val)
{
  _sion_flags_entry* new_entry = NULL;
  ...
  new_entry  = _lsion_flags_create_entry();
...
#define DFUNCTION "_lsion_flags_add"
void _lsion_flags_add(_sion_flags_store* store,
                     const char*        key,
                     const char*        val)
{
  DPRINTFP((2, DFUNCTION, -1, "enter\n"));

  store->current = _lsion_flags_add_entry(store->current, key, val);

  DPRINTFP((2, DFUNCTION, -1, "leave\n"));
}
#undef DFUNCTION

#define DFUNCTION "_lsion_flags_get"
_sion_flags_entry* _lsion_flags_get(_sion_flags_store* store, const char* key)
{
  _sion_flags_entry* iter = NULL;

  DPRINTFP((2, DFUNCTION, -1, "enter\n"));

  for (iter = _lsion_flags_iter(store); iter->next != NULL; iter = iter->next) {
...
#define DFUNCTION "_lsion_flags_update_mask"
sion_int64 _lsion_flags_update_mask(_sion_flags_store* store)
{
  _sion_flags_entry* iter = NULL;
  ...
  for (iter = _lsion_flags_iter(store); iter->next != NULL; iter = iter->next) {
    if ((!strcmp(iter->key, "w")) || (!strcmp(iter->key, "wb")) ||
      ...
      store->mask ^= store->mask & _SION_FMODE_SIONFWD;
    }
    ...
    else if (!strcmp(iter->key, "sionfwd")) {
      store->mask |= _SION_FMODE_SIONFWD;
      store->mask ^= store->mask & _SION_FMODE_ANSI;
      store->mask ^= store->mask & _SION_FMODE_POSIX;
    }
  }
...
#define DFUNCTION "_lsion_parse_flags"
 ...
 * @return     flag store with key value pairs
 */
_sion_flags_store* _lsion_parse_flags(const char* flags)
{
  char*              tmps      = NULL;
  char*              key       = NULL;
  char*              val       = NULL;
  ...
  store = _lsion_flags_create_store();
  ...
  while (pch != NULL) {
    ...
    _lsion_flags_add(store, key, val);
  ...
  _lsion_flags_update_mask(store);
