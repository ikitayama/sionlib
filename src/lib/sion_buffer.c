...
#define DFUNCTION "_lsion_buffer_check_env"
...
int  _lsion_buffer_check_env(_sion_filedesc *sion_filedesc) {
  const char *t;
  int rc = SION_SUCCESS;
  t = _lsion_getenv("SION_BUFFERSIZE");
  if(t) {
  ...
  _lsion_buffer_init(sion_filedesc);
  return (rc);
}
#undef DFUNCTION

#define DFUNCTION "_lsion_buffer_init"
...
int  _lsion_buffer_init(_sion_filedesc *sion_filedesc) {
  ...
  if(sion_filedesc->buffer_size>0) {
    ...
    if (sion_filedesc->buffer == NULL) {
      return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_buffer_init: cannot allocate internal buffer of size %lu , aborting ...\n", (unsigned long) sion_filedesc->buffer_size));
    }
...
#define DFUNCTION "_lsion_buffer_push"
...
#define DFUNCTION "_lsion_buffer_get_data_ptr"
...
#define DFUNCTION "_lsion_buffer_flush"
/*!\brief Flush buffer.
 *
 * @param  *sion_filedesc sion file description struct (_sion_filedesc)
 *
