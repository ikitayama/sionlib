...
#define DFUNCTION "_lsion_init_filedesc"
 ...
 */
int _lsion_init_filedesc(_sion_filedesc *sion_filedesc)
{

  int       rc = SION_SUCCESS;

  if (sion_filedesc == NULL) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN, DFUNCTION ": cannot initalized, data structure is not allocated, aborting ...\n"));
  }
...
#define DFUNCTION "_lsion_realloc_filedesc_blocklist"
 ...
 * @retval      SION_SUCCESS if OK
 */
int _lsion_realloc_filedesc_blocklist(_sion_filedesc *sion_filedesc, sion_int32 maxchunks)
{
  int       rc = SION_SUCCESS;
  int       i;
  if (sion_filedesc->maxchunks <= 0) {
    ...
    if (sion_filedesc->blocksizes == NULL) {
      return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN, DFUNCTION ": cannot allocate temporary memory of size %lu (sion_blocksizes), aborting ...\n",
...
#define DFUNCTION "_lsion_alloc_filedesc_all_chunksizes"
int _lsion_alloc_filedesc_all_chunksizes(_sion_filedesc *sion_filedesc)
{
  int       i, rc = SION_SUCCESS;
  sion_int64 *p;
  
  if (sion_filedesc->ntasks<0) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN, DFUNCTION ": cannot allocate internal arrays ntasks<0, aborting ...\n"));
  }
  ...
  if (sion_filedesc->all_chunksizes == NULL) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN, DFUNCTION ": cannot allocate temporary memory of size %lu (sion_chunksizes), aborting ...\n", 
...
#define DFUNCTION "_lsion_alloc_filedesc_all_startpointers"
int _lsion_alloc_filedesc_all_startpointers(_sion_filedesc *sion_filedesc)
{
  int       i, rc = SION_SUCCESS;
  sion_int64 *p;
  ...
  if (sion_filedesc->all_startpointers == NULL) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN, DFUNCTION ": cannot allocate temporary memory of size %lu (sion_all_startpointers), aborting ...\n",
			    (unsigned long) sion_filedesc->ntasks * sizeof(sion_int64)));
  }
  for (i = 0; i < sion_filedesc->ntasks; i++) p[i] = -1;
...
#define DFUNCTION "_lsion_alloc_filedesc_all_globalranks"
int _lsion_alloc_filedesc_all_globalranks(_sion_filedesc *sion_filedesc)
{
  int       i, rc = SION_SUCCESS;
  sion_int64 *p;
  ...
  if (sion_filedesc->all_globalranks == NULL) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN, DFUNCTION ": cannot allocate temporary memory of size %lu (sion_all_globalranks), aborting ...\n", 
			    (unsigned long) sion_filedesc->ntasks * sizeof(sion_int64)));
  }
  for (i = 0; i < sion_filedesc->ntasks; i++) p[i] = -1;
...
#define DFUNCTION "_lsion_alloc_filedesc_all_localranks"
int _lsion_alloc_filedesc_all_localranks(_sion_filedesc *sion_filedesc)
{
  int       i, rc = SION_SUCCESS;
  sion_int64 *p;
  ...
  if (sion_filedesc->all_localranks == NULL) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN, DFUNCTION ": cannot allocate temporary memory of size %lu (sion_all_localranks), aborting ...\n", 
			    (unsigned long) sion_filedesc->ntasks * sizeof(sion_int64)));
  }
  for (i = 0; i < sion_filedesc->ntasks; i++) p[i] = -1;
...
#define DFUNCTION "_lsion_alloc_filedesc_all_keyvalptr"
int _lsion_alloc_filedesc_all_keyvalptr(_sion_filedesc *sion_filedesc)
{
  int       i, rc = SION_SUCCESS;
  void     **p;
  ...
  if (sion_filedesc->all_keyvalptr == NULL) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN, DFUNCTION ": cannot allocate temporary memory of size %lu (sion_all_keyvalptr), aborting ...\n", 
			    (unsigned long) sion_filedesc->ntasks * sizeof(sion_int64)));
  }
...
#define DFUNCTION "_lsion_alloc_filedesc_arrays"
 ...
 * @retval      SION_SUCCESS if OK
 */
int _lsion_alloc_filedesc_arrays(_sion_filedesc *sion_filedesc)
{
  DPRINTFP((2, DFUNCTION, -1, "enter alloc arrays size=%d (%lu bytes)\n", sion_filedesc->ntasks, (unsigned long) sion_filedesc->ntasks * sizeof(sion_int64)));
  
  _lsion_alloc_filedesc_all_chunksizes(sion_filedesc);
  _lsion_alloc_filedesc_all_startpointers(sion_filedesc);
  _lsion_alloc_filedesc_all_globalranks(sion_filedesc);
...
#define DFUNCTION "_lsion_free_filedesc_all_chunksizes"
int _lsion_free_filedesc_all_chunksizes(_sion_filedesc *sion_filedesc)
{
...
#define DFUNCTION "_lsion_free_filedesc_all_globalranks"
int _lsion_free_filedesc_all_globalranks(_sion_filedesc *sion_filedesc)
{
  DPRINTFP((2, DFUNCTION, -1, "enter free arrays\n"));
  if (sion_filedesc->all_globalranks != NULL) {
...
#define DFUNCTION "_lsion_free_filedesc_all_startpointers"
int _lsion_free_filedesc_all_startpointers(_sion_filedesc *sion_filedesc)
{
  DPRINTFP((2, DFUNCTION, -1, "enter free arrays\n"));
  if (sion_filedesc->all_startpointers != NULL) {
...
#define DFUNCTION "_lsion_free_filedesc_all_localranks"
int _lsion_free_filedesc_all_localranks(_sion_filedesc *sion_filedesc)
{
  DPRINTFP((2, DFUNCTION, -1, "enter free arrays\n"));
  if (sion_filedesc->all_localranks != NULL) {
...
#define DFUNCTION "_lsion_free_filedesc_all_keyvalptr"
int _lsion_free_filedesc_all_keyvalptr(_sion_filedesc *sion_filedesc)
{
  int tasknr = 0;
  DPRINTFP((2, DFUNCTION, -1, "enter free arrays\n"));
  if (sion_filedesc->all_keyvalptr != NULL) {
	...
	DPRINTFP((2, DFUNCTION, -1, "free now KEYVALPTR all_keyvalptr[%d] = %x\n",tasknr,sion_filedesc->all_keyvalptr[tasknr]));
        _lsion_keyvalue_keymngr_destroy((_sion_keyvalue_keymngr **)&(sion_filedesc->all_keyvalptr[tasknr]));
      }
    }
...
#define DFUNCTION "_lsion_free_filedesc_arrays"
 ...
 * @retval      SION_SUCCESS if OK
 */
int _lsion_free_filedesc_arrays(_sion_filedesc *sion_filedesc)
{

  _lsion_free_filedesc_all_chunksizes(sion_filedesc);
  _lsion_free_filedesc_all_globalranks(sion_filedesc);
  _lsion_free_filedesc_all_startpointers(sion_filedesc);
  return (1);
}
#undef DFUNCTION


#define DFUNCTION "_lsion_alloc_filedesc_coll_arrays"
/*!\brief Allocate memory for the internal sion arrays
 *
 * @param  *sion_filedesc                       sion file description struct (_sion_filedesc)
 *
 * @retval      SION_SUCCESS if OK
 */
int _lsion_alloc_filedesc_coll_arrays(_sion_filedesc *sion_filedesc)
{
  int       i;
  sion_int32 *p;
  ...
  if (sion_filedesc->all_coll_collsize == NULL) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN, DFUNCTION ": cannot allocate temporary memory of size %lu (sion_all_collsize), aborting ...\n", 
  ...
  if (sion_filedesc->all_coll_collector == NULL) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN, DFUNCTION ": cannot allocate temporary memory of size %lu (sion_all_collector), aborting ...\n", 
			    (unsigned long) sion_filedesc->ntasks * sizeof(sion_int32)));
  }
  for (i = 0; i < sion_filedesc->ntasks; i++) p[i] = -1;
  ...
  if (sion_filedesc->all_coll_capability == NULL) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN, DFUNCTION ": cannot allocate temporary memory of size %lu (sion_all_capability), aborting ...\n", 
			    (unsigned long) sion_filedesc->ntasks * sizeof(sion_int32)));
  }
  for (i = 0; i < sion_filedesc->ntasks; i++) p[i] = -1;
...
#define DFUNCTION "_lsion_free_filedesc_coll_arrays"
/*!\brief free memory for the internal sion arrays
 *
 * @param  *sion_filedesc                       sion file description struct (_sion_filedesc)
 *
 * @retval      SION_SUCCESS if OK
 */
int _lsion_free_filedesc_coll_arrays(_sion_filedesc *sion_filedesc)
{
...
#define DFUNCTION "_lsion_alloc_filedesc_block_arrays"
 ...
 * @retval      SION_SUCCESS if OK
 */
int _lsion_alloc_filedesc_block_arrays(_sion_filedesc *sion_filedesc)
{
  int       i;
  sion_int64 *p;
  ...
  if (sion_filedesc->all_blockcount == NULL) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN, DFUNCTION ": cannot allocate temporary memory of size %lu (sion_blockcount), aborting ...\n", 
			    (unsigned long) sion_filedesc->ntasks * sizeof(sion_int64)));
  }
  for (i = 0; i < sion_filedesc->ntasks; i++) p[i] = -1;
  ...
  if (sion_filedesc->all_currentpos == NULL) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN, DFUNCTION ": cannot allocate temporary memory of size %lu (sion_currentpos), aborting ...\n", 
			    (unsigned long) sion_filedesc->ntasks * sizeof(sion_int64)));
  }
  for (i = 0; i < sion_filedesc->ntasks; i++) p[i] = -1;
  ...
  if (sion_filedesc->all_currentblocknr == NULL) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN, DFUNCTION ": cannot allocate temporary memory of size %lu (sion_currentblocknr), aborting ...\n",
			    (unsigned long) sion_filedesc->ntasks * sizeof(sion_int64)));
  }
  for (i = 0; i < sion_filedesc->ntasks; i++) p[i] = -1;
...
#define DFUNCTION "_lsion_alloc_filedesc_block_arrays_only"
/*!\brief Allocate memory for the internal sion structure, fields for all chunksizes of all tasks
 *
 * @param  *sion_filedesc                       sion file description struct (_sion_filedesc)
 *
 * @retval      SION_SUCCESS if OK
 */
int _lsion_alloc_filedesc_block_arrays_only(_sion_filedesc *sion_filedesc)
{
  int       i;
  sion_int64 *p;
...
#define DFUNCTION "_lsion_free_filedesc"
int _lsion_free_filedesc(_sion_filedesc *sion_filedesc)
{
  ...
  if (sion_filedesc->all_keyvalptr)
    _lsion_free_filedesc_all_keyvalptr(sion_filedesc);
  if (sion_filedesc->keyvalptr)
    _lsion_keyvalue_keymngr_destroy((_sion_keyvalue_keymngr **)&(sion_filedesc->keyvalptr));
...
#define DFUNCTION "_lsion_print_filedesc"
/*!\brief Print the initialized sion file description
 *
 * @retval      SION_SUCCESS if OK
 */
int _lsion_print_filedesc(_sion_filedesc *sion_filedesc, int level, char *desc, int flag)
{
  ...
  if (sion_filedesc == NULL) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN, DFUNCTION ": cannot print, data structure is not allocated, aborting ...\n"));
  }
  ...
  if(sion_filedesc->fileptr) {
    DPRINTFP((level, desc, sion_filedesc->rank, "D: %-20s=%d %s\n", "fileptr->flags", sion_filedesc->fileptr->flags,_lsion_get_fileptr_desc(sion_filedesc->fileptr)));
  }
  ...
  if( (sion_filedesc->state==SION_FILESTATE_SEROPENMASTER) || (sion_filedesc->state==SION_FILESTATE_PAROPENMAPPEDMASTER) ) {
	...
	_lsion_print_filedesc(sion_filedesc->multifiles[i], level, desc, flag&~(_SION_DEBUG_PRINT_RECURSIVE));
      }
    }
  }
...
#define DFUNCTION "_lsion_get_size_of_filedesc"
 ...
 */
int _lsion_get_size_of_filedesc(_sion_filedesc *sion_filedesc, int *numbytes, int *numfds)
{
  int rc=SION_SUCCESS;
  ...
  if( (sion_filedesc->state==SION_FILESTATE_PAROPEN) || (sion_filedesc->state==SION_FILESTATE_SEROPENRANK) ) {
    if( (sion_filedesc->keyvalptr!=NULL)  ) {
      help_bytes=_lsion_keyvalue_keymngr_key_get_sizeof(sion_filedesc->keyvalptr);
  ...
  if( (sion_filedesc->state==SION_FILESTATE_SEROPEN) || (sion_filedesc->state==SION_FILESTATE_SEROPENMASTER) 
	...
	if(sion_filedesc->all_keyvalptr[i]!=NULL) {
	  help_bytes=_lsion_keyvalue_keymngr_key_get_sizeof(sion_filedesc->all_keyvalptr[i]);
  ...
  if( (sion_filedesc->state==SION_FILESTATE_SEROPENMASTER) || (sion_filedesc->state==SION_FILESTATE_PAROPENMAPPEDMASTER) ) {
    ...
    for (i = 0; i < sion_filedesc->nfiles; i++) {
      ...
      rc=_lsion_get_size_of_filedesc(sion_filedesc->multifiles[i], &help_bytes, &help_fds);
...
#define DFUNCTION "_lsion_dup_filedesc"
 ...
 */
_sion_filedesc* _lsion_dup_filedesc(_sion_filedesc *sion_filedesc)
{
  _sion_filedesc *new_fd=NULL;
  
  if (sion_filedesc == NULL) {
    _lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"cannot allocate filedescriptor structure  (sion_filedesc), aborting ...\n");
    return(NULL);
  }
  new_fd=_lsion_alloc_filedesc();
  ...
  new_fd->collmsa = sion_filedesc->collmsa;
