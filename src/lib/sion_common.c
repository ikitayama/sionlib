...
sion_io_stat_t* lsion_get_io_info_buddy(int sid, int roles, int flag);
...
int lsion_get_locations(int sid,
  ...
  if ((sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR)
    ...
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"invalid sion_filedesc, aborting %d ...\n", sid));
  }
  DPRINTFP((1, "lsion_get_locations", -1, "enter\n"));
  ...
  if((sion_filedesc->state == SION_FILESTATE_SEROPENMASTER) &&
    ...
    _lsion_alloc_filedesc_block_arrays_only(sion_filedesc);
  ...
  *start_of_varheader = sion_filedesc->start_of_varheader;
  ...
  DPRINTFP((1, "lsion_get_locations", -1, "leave\n"));
...
int _lsion_close_sid(int sid)
{
  ...
  if ((sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR)
      || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_close: invalid sion_filedesc, aborting %d ...\n", sid));
  }
  rc=_lsion_close(sion_filedesc);
 ...
 * in parallel mode (sion_paropen..., lsion_open_rank).
 *
 * @param[in]   sid            sion file handle
 ...
 * @return      SION_SUCCESS if ok
 */
int lsion_get_current_location(int sid, int *currentchunknr, sion_int64 *currentpos, int *maxchunks, sion_int64 **chunksizes)
{
  int       rc = SION_SUCCESS;
  _sion_filedesc *sion_filedesc;
  ...
  DPRINTFP((1, "lsion_get_current_location", -1, "enter\n"));
  ...
  *chunksizes     = sion_filedesc->blocksizes;
  ...
  DPRINTFP((1, "lsion_get_current_location", -1, "leave\n"));
  return (rc);
}
...
int lsion_get_mapping(int          sid,
  ...
  DPRINTFP((1, "lsion_get_mapping", -1, "enter\n"));
  ...
  *numfiles=sion_filedesc->nfiles;
  ...
  DPRINTFP((1, "lsion_get_mapping", -1, "leave (mapping_size=%d)\n",*mapping_size));
...
int lsion_get_file_endianness(int sid)
{
  ...
  DPRINTFP((1, "lsion_get_file_endianness", -1, "enter (sid=%d)\n",sid));

  if ((sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR)
      || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"invalid sion_filedesc, aborting %d ...\n", sid));
  }

  DPRINTFP((1, "lsion_get_file_endianness", -1, "leave (sid=%d)\n",sid));
...
int lsion_endianness_swap_needed(int sid)
{
  ...
  DPRINTFP((1, "lsion_endianness_swap_needed", -1, "enter (sid=%d)\n",sid));

  swap_needed = (lsion_get_file_endianness(sid) != lsion_get_endianness());

  DPRINTFP((1, "lsion_endianness_swap_needed", -1, "leave (swap_needed=%d)\n",
            swap_needed));
...
sion_io_stat_t* lsion_get_io_info(int sid) {
  ...
  return(lsion_get_io_info_by_spec(sid,SION_ROLE_COLLECTOR|SION_ROLE_WRITER|SION_ROLE_NOWRITER,SION_GET_IO_INFO_FLAG_NONE));
...
sion_io_stat_t* lsion_get_io_info_by_spec(int sid, int roles, int flag) {
  ...
  if ((sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR)
      || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    _lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"invalid sion_filedesc, aborting %d ...\n", sid);
    return(NULL);
  }

  /* branch to buddy CP */
  if(sion_filedesc->usebuddy) {
    p=lsion_get_io_info_buddy(sid,roles,flag);
  }

  return (p);
}

int lsion_free_io_info(sion_io_stat_t *p) {
  return (_lsion_free_io_info(p));
}
...
size_t lsion_fwrite(const void *data, size_t size, size_t nitems, int sid)
{
  ...
  DPRINTFP((1, "lsion_fwrite", -1, "enter size=%ld nitems=%ld\n",(long) size, (long) nitems));

  if ((sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR)
      || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_ANSI_SIZE_NOT_VALID,_SION_ERROR_RETURN,"invalid sion_filedesc, aborting %d ...\n", sid));
  }

  _lsion_check_on_collective_mode(sion_filedesc);
  ...
  if(sion_filedesc->usebuffer) {
    ...
    if(!lsion_ensure_free_space(sid,bytes_to_write)) {
      return(_lsion_errorprint_on_rank(SION_ANSI_SIZE_NOT_VALID,_SION_ERROR_RETURN,sion_filedesc->rank,
    ...
    if (bbytes + bytes_to_write > bytes_left) {
      ...
      _lsion_buffer_get_data_ptr(sion_filedesc,&bdata,&bbytes);
      if(lsion_ensure_free_space(sid,bbytes)) {
        frc = _lsion_file_write(bdata, bbytes, sion_filedesc->fileptr);
        if(frc != bbytes) {
          return(_lsion_errorprint_on_rank(SION_ANSI_SIZE_NOT_VALID,_SION_ERROR_RETURN,sion_filedesc->rank,
    ...
    /* buffer data */
    sion_int64 bytes_buffered=_lsion_buffer_push(sion_filedesc,data_ptr,bytes_to_write);
    ...
    while(bytes_to_write>0) {
      ...
      bytes_buffered=_lsion_buffer_push(sion_filedesc,data_ptr,bytes_to_write);
      bytes_to_write-=bytes_buffered;
      data_ptr+=bytes_buffered;
    }
    ...
    if(lsion_ensure_free_space(sid,bytes_to_write)) {
      ...
      if ((err == cudaSuccess) && (!attrs.isManaged && (attrs.memoryType == cudaMemoryTypeDevice)) ) {
        ...
        while (frc < bytes_to_write) {
          sion_int64 to_write = (bytes_to_write - frc) > sion_filedesc->fsblksize ? sion_filedesc->fsblksize : (bytes_to_write - frc);
          cudaMemcpy(buffer, data_, to_write, cudaMemcpyDeviceToHost);
          sion_int64 frc_ = _lsion_file_write(buffer, to_write, sion_filedesc->fileptr);
        ...
        frc = _lsion_file_write(data, bytes_to_write, sion_filedesc->fileptr);
      }
#else
      frc = _lsion_file_write(data, bytes_to_write, sion_filedesc->fileptr);
#endif
      ...
      return(_lsion_errorprint_on_rank(SION_ANSI_SIZE_NOT_VALID,_SION_ERROR_RETURN,sion_filedesc->rank,"could not ensure free space for this block, returning %d ...", sid));
    }
    /* increase current position */
    sion_filedesc->currentpos+=bytes_to_write;
  }

  DPRINTFP((1, "lsion_fwrite", -1, "leave rc=%ld\n",(long) rc));
  return(rc);
}
...
size_t lsion_fread( void *data, size_t size, size_t nitems, int sid) {
  ...
  if (!data) {
    ...
    return(_lsion_errorprint(SION_ANSI_SIZE_NOT_VALID,_SION_ERROR_RETURN,"invalid pointer, aborting %d ...\n", data));
  }
  ...
  DPRINTFP((1, "lsion_fread", -1, "enter\n"));
  DPRINTFP((4, "lsion_fread", -1, " parameter size=%ld nitems=%ld\n",(long) size,(long) nitems));
  ...
  /* Check size. bytes_left == 0 is handled by lsion_feof */
  if((sion_filedesc->chunksize < bytes) || ((bytes_left < bytes) &&
                                            (bytes_left != 0))) {
    /* FIXME: replace SION_ANSI_SIZE_NOT_VALID? */
    return(_lsion_errorprint(SION_ANSI_SIZE_NOT_VALID,_SION_ERROR_RETURN,"not enough bytes left in chunk, aborting (%ld < %ld) ...\n", bytes_left, bytes));
  }
  ...
  if(!lsion_feof(sid)) {
    ...
    if ((err == cudaSuccess) && (!attrs.isManaged && (attrs.memoryType == cudaMemoryTypeDevice)) ) {
      ...
      char* data_ = data;
      while (bread < bytes) {
        sion_int64 to_read = (bytes - bread) > sion_filedesc->fsblksize ? sion_filedesc->fsblksize : (bytes - bread);
        sion_int64 bread_ = _lsion_file_read(buffer, to_read, sion_filedesc->fileptr);
      ...
      bread=_lsion_file_read(data,bytes,sion_filedesc->fileptr);
      DPRINTFP((256, "lsion_fread", -1, "  _lsion_file_read returns %ld\n",(long) bread));
    }
#else
    bread=_lsion_file_read(data,bytes,sion_filedesc->fileptr);
    DPRINTFP((256, "lsion_fread", -1, "  _lsion_file_read returns %ld\n",(long) bread));
  ...
  DPRINTFP((4, "lsion_fread", -1, " return value  %ld\n",(long) bread));
  DPRINTFP((1, "lsion_fread", -1, "leave\n"));
  return(bread);
}
 ...
 */
int lsion_seek(int sid, int rank, int currentblocknr, sion_int64 posinblk)
{
  FILE *fileptr;
  int rc=0;
  /* TBD: check if not lsion_open and multi file */
  rc=lsion_seek_fp(sid,rank,currentblocknr,posinblk,&fileptr);
  return(rc);

}

/*!
 * @brief Deprecated. Use lsion_seek() instead.
 *
 ...
 * @retval      SION_SUCCESS if file pointer can be moved to new position
 */
int lsion_seek_fp(int sid, int rank, int currentblocknr, sion_int64 posinblk, FILE **fileptr)
{
  int             rc = SION_SUCCESS;
  _sion_filedesc *sion_filedesc;

  DPRINTFP((1, "lsion_seek_fp", -1, " enter seek with sid=%d\n", sid));
  assert((_lsion_vcdtype(sid) == SION_FILEDESCRIPTOR));

  if ((sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR)
      || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"lsion_seek: invalid sion_filedesc, aborting %d ...\n", sid));
  }

  /* check state */
  if ((sion_filedesc->mode != SION_FILEMODE_READ)  && (sion_filedesc->mode != SION_FILEMODE_WRITE)) {
      return(_lsion_errorprint_on_rank(SION_NOT_SUCCESS,_SION_ERROR_RETURN,sion_filedesc->rank,"lsion_seek_fp: unknown file open state !(READ|WRITE), aborting %d ...", sid));
  }
  if (sion_filedesc->usebuffer) {
    _lsion_buffer_flush(sion_filedesc);
  }
  if ( (sion_filedesc->state != SION_FILESTATE_PAROPEN)
      ...
      return(_lsion_errorprint_on_rank(SION_NOT_SUCCESS,_SION_ERROR_RETURN,sion_filedesc->rank,"lsion_seek_fp: unknown file open state !(PAR|SER|SERRANK|MAPPED), aborting %d ...", sid));
  }
  ...
  if (sion_filedesc->mode == SION_FILEMODE_READ) {  /* READ */
    ...
    if (sion_filedesc->state == SION_FILESTATE_SEROPEN) {
      rc = _lsion_seek_on_all_ranks_read(sion_filedesc, rank, currentblocknr, posinblk);
    }
    else if (sion_filedesc->state == SION_FILESTATE_SEROPENMASTER) {
      rc = _lsion_seek_on_all_ranks_read_master(sion_filedesc, rank, currentblocknr, posinblk);
    }
    else if ((sion_filedesc->state == SION_FILESTATE_SEROPENRANK) ||
             (sion_filedesc->state == SION_FILESTATE_PAROPEN)) {
      rc = _lsion_seek_on_current_rank_read(sion_filedesc, rank, currentblocknr, posinblk);
    }
    else if (sion_filedesc->state == SION_FILESTATE_PAROPENMAPPEDMASTER) {
      rc = _lsion_seek_on_all_ranks_read_mapped(sion_filedesc, rank, currentblocknr, posinblk);
    }
  } else {                                         /* WRITE */
    if (sion_filedesc->state == SION_FILESTATE_SEROPEN) {
      rc = _lsion_seek_on_all_ranks_write(sion_filedesc, rank, currentblocknr, posinblk);
    }
    else if (sion_filedesc->state == SION_FILESTATE_SEROPENRANK) {
      return _lsion_errorprint_on_rank(SION_NOT_SUCCESS, _SION_ERROR_RETURN, sion_filedesc->rank, "lsion_seek_fp: seek not supported for this type (write, lsion_open_rank), aborting ...");
    }
    else if (sion_filedesc->state == SION_FILESTATE_PAROPEN) {
      rc = _lsion_seek_on_current_rank_write(sion_filedesc, rank, currentblocknr, posinblk);
    }
    else if (sion_filedesc->state == SION_FILESTATE_PAROPENMAPPEDMASTER) {
      rc = _lsion_seek_on_all_ranks_write_mapped(sion_filedesc, rank, currentblocknr, posinblk);
    }

  }

  /* TBD: implement lsion_open and lsion_seek_fp for multi-files */
  if(fileptr!=NULL) {
  ...
  DPRINTFP((1, "lsion_seek_fp", -1, "leave seek rc=%d\n",rc));
...
int lsion_feof(int sid)
{
  ...
  if ((sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR)
      || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"lsion_feof: invalid sion_filedesc, aborting %d ...\n", sid));
  }

  DPRINTFP((8, "lsion_feof", -1, "enter feof   sid=%d currentpos=%15lld\n", sid, sion_filedesc->currentpos));
  ...
  if (sion_filedesc->currentpos < maxpos) {
    ...
    rc = 0;
    DPRINTFP((16, "lsion_feof", -1, " end of current block %d not reached sid=%d\n",
    ...
    if (sion_filedesc->currentblocknr < sion_filedesc->lastchunknr) {
      ...
      rc = SION_NOT_SUCCESS;
      DPRINTFP((8, "lsion_feof", -1,
                " end of block %d reached, skipping to next %lld -> %lld position=%lld gs=%lld sid=%d\n",
                sion_filedesc->currentblocknr - 1,
                sion_filedesc->currentpos, _lsion_file_get_position(sion_filedesc->fileptr), sion_filedesc->currentpos, sion_filedesc->globalskip, sid));
    }
    else {
      /* end of last block reached */
      DPRINTFP((8, "lsion_feof", -1, " end of last block %d reached sid=%d\n", sion_filedesc->currentblocknr, sid));
      rc = SION_SUCCESS;
    }
  }

  DPRINTFP((8, "lsion_feof", -1, "leave feof   sid=%d currentpos=%15lld rc=%d\n", sid, sion_filedesc->currentpos, rc));
...
{
  return(lsion_bytes_avail_in_chunk(sid));
}
...
sion_int64 lsion_bytes_avail_in_chunk(int sid)
{
  ...
  if ((sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR)
      || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"lsion_bytes_avail_in_chunk: invalid sion_filedesc, aborting %d ...\n", sid));
  }
  ...
  DPRINTFP((8, "lsion_bytes_avail_in_chunk", -1,
...
sion_int64 lsion_get_position(int sid)
{
  ...
  if ((sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR)
      || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"lsion_get_position: invalid sion_filedesc, aborting %d ...\n", sid));
  }


  _lsion_file_flush(sion_filedesc->fileptr);

  return (_lsion_file_get_position(sion_filedesc->fileptr));
...
int lsion_set_fp_closed(int sid)
{
  ...
  if ((sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR)
      || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"lsion_set_fp_closed: invalid sion_filedesc, aborting %d ...\n", sid));
  }
  sion_filedesc->state = SION_FILESTATE_CLOSE;
  return (rc);
}

int lsion_set_second_fp(int sid, FILE *secondfd)
{
  int       rc = SION_SUCCESS;
  _sion_filedesc *sion_filedesc;
  ...
  if ((sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR)
      || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"lsion_set_second_fp: invalid sion_filedesc, aborting %d ...\n", sid));
  }

  if(!(sion_filedesc->fileptr->flags&SION_FILE_FLAG_ANSI)) {
    return(_lsion_errorprint_on_rank(SION_NOT_SUCCESS,_SION_ERROR_RETURN,sion_filedesc->rank,"lsion_set_second_fp: file was not opened in ANSI mode, aborting %d ...", sid));
  }

  _lsion_file_set_second_fileptr(sion_filedesc->fileptr,secondfd);

  _lsion_print_filedesc(sion_filedesc, 512, "sion_set_second_fd", 1);
  /*
  if (sion_filedesc->mode == SION_FILEMODE_READ) {
    lsion_seek(sid, SION_CURRENT_RANK, 0, 0);
  }
  */
  return (rc);
}
...
int lsion_unset_second_fp(int sid)
{
  ...
  if ((sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR)
      || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"lsion_unset_second_fp: invalid sion_filedesc, aborting %d ...\n", sid));
  }

  if(!(sion_filedesc->fileptr->flags&SION_FILE_FLAG_ANSI)) {
    return(_lsion_errorprint_on_rank(SION_NOT_SUCCESS,_SION_ERROR_RETURN,sion_filedesc->rank,"lsion_unset_second_fp: file was not opened in ANSI mode, aborting %d ...", sid));
  }

  _lsion_file_unset_second_fileptr(sion_filedesc->fileptr);
  return (rc);
}
...
int lsion_optimize_fp_buffer(int sid)
{
  ...
  if (sion_filedesc->fpbuffer == NULL) {
    return(_lsion_errorprint_on_rank(SION_NOT_SUCCESS,_SION_ERROR_RETURN,sion_filedesc->rank,"lsion_optimize_fp_buffer: cannot allocate internal buffer of size %lu , aborting ...", (unsigned long) sion_filedesc->fsblksize));
  }
  sion_filedesc->fpbuffer_size = sion_filedesc->fsblksize;

  rc=_lsion_file_set_buffer(sion_filedesc->fileptr, sion_filedesc->fpbuffer, sion_filedesc->fpbuffer_size);
  return (rc);
}
...
int lsion_flush(int sid)
{
  ...
  rc = _lsion_flush_block(sion_filedesc);
  return (rc);
}
...
int lsion_ensure_free_space(int sid, sion_int64 bytes)
{
  ...
  if ((sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR)
      || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
      return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"lsion_ensure_free_space: invalid sion_filedesc, returning %d ...\n", sid));
  }
  ...
  if (sion_filedesc->mode != SION_FILEMODE_WRITE) {
    ...
    return(_lsion_errorprint_on_rank(SION_NOT_SUCCESS,_SION_ERROR_RETURN,sion_filedesc->rank,"lsion_ensure_free_space[%2d]: file is opened invalid sion_mode, returning ...", sion_filedesc->rank));
  }
  if (!sion_filedesc->usecoll) {
    if (sion_filedesc->fileptr == NULL) {
      DPRINTFP((8, "_sion_ensure_free_space", -1, "file not opened\n"));
      return(_lsion_errorprint_on_rank(SION_NOT_SUCCESS,_SION_ERROR_RETURN,sion_filedesc->rank,"lsion_ensure_free_space[%2d]: file is not open, returning ...", sion_filedesc->rank));
    }
  }
  ...
  if (bytes > sion_filedesc->chunksize) {
    ...
    return(_lsion_errorprint_on_rank(SION_NOT_SUCCESS,_SION_ERROR_RETURN,sion_filedesc->rank,"lsion_ensure_free_space[%2d]: could not write %lld bytes, chunksize=%lld, returning ...",sion_filedesc->rank, bytes, sion_filedesc->chunksize));
  }
  ...
  if ((byteswritten + bytes) > sion_filedesc->chunksize) {
    ...
    _lsion_create_new_block(sion_filedesc);
  }
...
int lsion_get_sizeof(int sid, int *numbytes, int *numfds) {
  ...
  *numfds=SION_SIZE_NOT_VALID;
  ...
  if ((sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR)
      || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"lsion_get_sizeof: invalid sion_filedesc, returning %d ...\n", sid));
  }
  DPRINTFP((2, "lsion_get_sizeof", -1, "enter sid=%d\n", sid));

  rc=_lsion_get_size_of_filedesc(sion_filedesc,numbytes,numfds);

  DPRINTFP((2, "lsion_get_sizeof", -1, "leave sid=%d numbytes=%d numfds=%d\n", sid, *numbytes, *numfds));
...
int lsion_dup(int sid, int mode, int rank, uint64_t key)
{
  ...
  if ((sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR)
      || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"lsion_dup: invalid sion_filedesc, returning %d ...\n", sid));
  }
  DPRINTFP((8, DFUNCTION, -1, "enter sid=%d\n", sid));

  if (sion_filedesc->mode != SION_FILEMODE_READ) {
    DPRINTFP((8, DFUNCTION, -1, "invalid opened\n"));
    return(_lsion_errorprint_on_rank(SION_ID_NOT_VALID,_SION_ERROR_RETURN,sion_filedesc->rank, DFUNCTION "[%2d]: file is not opened in read mode, returning ...", sion_filedesc->rank));
  }

  new_sid=_lsion_dup(sid, mode, rank, key);
...
#define DFUNCTION "lsion_lock_register_lock_callbacks"
/*!
...
int lsion_lock_register_lock_callbacks(int lock(void *), int unlock(void *), void *lock_data) 
{
  ...
  rc=_lsion_lock_register_lock_callbacks(lock,unlock,lock_data);
...
#define DFUNCTION "lsion_lock_user_callbacks_defined"
/*!
 ...
 */
int lsion_lock_user_callbacks_defined() 
{
  int rc = 0;
  DPRINTFP((8, DFUNCTION, -1, "enter\n"));

  rc=_lsion_lock_user_callbacks_defined();
