...
#define DFUNCTION "_lsion_keyvalue_table_hash_fct"
unsigned int _lsion_keyvalue_table_hash_fct(sion_table_key_t key, int tab_size) {
...
#define DFUNCTION "_lsion_keyvalue_table_init"
_sion_keyvalue_table* _lsion_keyvalue_table_init(int size) {
  ...
  if (table == NULL) {
    _lsion_errorprint(0,_SION_ERROR_RETURN,"cannot allocate internal keyvalue table of size %lu , aborting ...\n", (unsigned long) sizeof(_sion_keyvalue_table));
    return(NULL);
  }

  entries = malloc(size * sizeof(_sion_keyvalue_table_entry));
  if (entries == NULL) {
    _lsion_errorprint(0,_SION_ERROR_RETURN,"cannot allocate internal keyvalue table entries of size %lu , aborting ...\n", (unsigned long) size);
    free(table);
    return(NULL);
  }
...
#define DFUNCTION "_lsion_keyvalue_table_destroy"
int  _lsion_keyvalue_table_destroy(_sion_keyvalue_table** table) {
...
#define DFUNCTION "_lsion_keyvalue_table_store"
int _lsion_keyvalue_table_store(_sion_keyvalue_table* table, sion_table_key_t key, void *data) {
   size_t rc=SION_SUCCESS;
   unsigned int index;
   ...
   index = _lsion_keyvalue_table_hash_fct(key,table->size);
...
#define DFUNCTION "_lsion_keyvalue_table_lookup"
void * _lsion_keyvalue_table_lookup(_sion_keyvalue_table* table, sion_table_key_t key) {
  unsigned int index;
...
#define DFUNCTION "_lsion_keyvalue_table_iterator_reset"
int _lsion_keyvalue_table_iterator_reset(_sion_keyvalue_table* table) {
...
#define DFUNCTION "_lsion_keyvalue_table_iterator_next_in_store_order"
int _lsion_keyvalue_table_iterator_next_in_store_order(_sion_keyvalue_table* table, sion_table_key_t *key, void **data) {
  _sion_keyvalue_table_entry *entry;
...
#define DFUNCTION "_lsion_keyvalue_table_iterator_next"
int _lsion_keyvalue_table_iterator_next(_sion_keyvalue_table* table, sion_table_key_t *key, void **data) {
...
#define DFUNCTION "_lsion_keyvalue_table_get_size"
int  _lsion_keyvalue_table_get_size(_sion_keyvalue_table* table) {
