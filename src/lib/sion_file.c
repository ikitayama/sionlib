...
_sion_fileptr   *_lsion_file_open(const char *fname, unsigned int flags, unsigned int addflags)  {
  ...
  if(flags & SION_FILE_FLAG_ANSI) {
    _sion_fileptr *sion_fileptr = _lsion_file_alloc_and_init_sion_fileptr();
	...
	sion_fileptr->fileptr=_lsion_file_open_ansi_write_create(fname,addflags);
      } else {
	sion_fileptr->fileptr=_lsion_file_open_ansi_write_existing(fname,addflags);
      }
    } else {
      sion_fileptr->flags |= SION_FILE_FLAG_READ;
      sion_fileptr->fileptr=_lsion_file_open_ansi_read(fname,addflags);
    }

    /* could not open ANSI file */
    if(!sion_fileptr->fileptr) {
      free(sion_fileptr);
      _lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_WARN,"error: could not open file (ANSI), %s %d %d, aborting ...\n", fname, flags, addflags);
      return(NULL);
    }
    ...
    if(flags & SION_FILE_FLAG_WRITE) {
      ...
      if(flags & SION_FILE_FLAG_CREATE) {
        sion_fileptr->flags |= SION_FILE_FLAG_CREATE;
        sion_fileptr->fd=_lsion_file_open_posix_write_create(fname,addflags);
    } else {
      sion_fileptr->fd=_lsion_file_open_posix_write_existing(fname,addflags);
      }
    } else {
      sion_fileptr->flags |= SION_FILE_FLAG_READ;
      sion_fileptr->fd=_lsion_file_open_posix_read(fname,addflags);
    }

    /* could not open POSIX file */
    if(sion_fileptr->fd<0) {
      free(sion_fileptr);
      _lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_WARN,"error: could not open file (POSIX), %s %d %d, aborting ...\n", fname, flags, addflags);
      return(NULL);
    }
    ...
    if(sion_fileptr->fd == -1) {
      free(sion_fileptr);
      _lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_WARN,"error: could not open file (SIONFWD), %s %d %d, aborting ...\n", fname, flags, addflags);
      return(NULL);
    }
    ...
    /* unknown mode? */
    _lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_WARN,"internal error: unknown open type, %s %d %d, aborting ...\n", fname, flags, addflags);
    return(NULL);
  }
}

#define DFUNCTION "_lsion_file_close"
...
int _lsion_file_close( _sion_fileptr *sion_fileptr ) {
  ...
  if(sion_fileptr->flags & SION_FILE_FLAG_ANSI) {
    if(sion_fileptr->flags & SION_FILE_FLAG_SCNDANSI) {
      _lsion_file_close_ansi(sion_fileptr->second_fileptr);
      sion_fileptr->second_fileptr=NULL;
    }
    rc=_lsion_file_close_ansi(sion_fileptr->fileptr);
    sion_fileptr->fileptr=NULL;
  } else if(sion_fileptr->flags & SION_FILE_FLAG_POSIX) {
    rc=_lsion_file_close_posix(sion_fileptr->fd);
#if defined(_SION_SIONFWD)
...
sion_int64 _lsion_file_write(const void *data, sion_int64 bytes, _sion_fileptr *sion_fileptr ) {
  ...
  if(sion_fileptr->flags & SION_FILE_FLAG_ANSI) {
    if(sion_fileptr->flags & SION_FILE_FLAG_SCNDANSI)
      return _lsion_file_write_ansi(data,bytes,sion_fileptr->second_fileptr);
    else
      return _lsion_file_write_ansi(data,bytes,sion_fileptr->fileptr);
  } else if(sion_fileptr->flags & SION_FILE_FLAG_POSIX) {
    return _lsion_file_write_posix(data,bytes,sion_fileptr->fd);
#if defined(_SION_SIONFWD)
  } else if(sion_fileptr->flags & SION_FILE_FLAG_SIONFWD) {
    return _sion_file_write_sionfwd(data,bytes,sion_fileptr->fd,&sion_fileptr->position);
#endif
  } else {
    return _lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"_lsion_file_write: cannot find valid file flag (flags=%d)\n",sion_fileptr->flags);
  }
}
...
sion_int64 _lsion_file_read(void *data, sion_int64 bytes, _sion_fileptr *sion_fileptr ) {
  ...
  if(sion_fileptr->flags & SION_FILE_FLAG_ANSI) {
    if(sion_fileptr->flags & SION_FILE_FLAG_SCNDANSI)
      return _lsion_file_read_ansi(data,bytes,sion_fileptr->second_fileptr);
    else
      return _lsion_file_read_ansi(data,bytes,sion_fileptr->fileptr);
  } else if(sion_fileptr->flags & SION_FILE_FLAG_POSIX) {
    return _lsion_file_read_posix(data,bytes,sion_fileptr->fd);
#if defined(_SION_SIONFWD)
  } else if(sion_fileptr->flags & SION_FILE_FLAG_SIONFWD) {
    return _sion_file_read_sionfwd(data,bytes,sion_fileptr->fd,&sion_fileptr->position);
#endif
  } else {
    return _lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"_lsion_file_read: cannot find valid file flag (flags=%d)\n",sion_fileptr->flags);
  }
}
 ...
 */
long _lsion_file_get_opt_blksize( _sion_fileptr *sion_fileptr ) {
  if (NULL == sion_fileptr) return SION_SIZE_NOT_VALID;

  if(sion_fileptr->flags & SION_FILE_FLAG_ANSI) {
    return _lsion_file_get_opt_blksize_ansi(sion_fileptr->fileptr);
  } else if(sion_fileptr->flags & SION_FILE_FLAG_POSIX) {
    return _lsion_file_get_opt_blksize_posix(sion_fileptr->fd);
#if defined(_SION_SIONFWD)
  } else if(sion_fileptr->flags & SION_FILE_FLAG_SIONFWD) {
    return _sion_file_get_opt_blksize_sionfwd(sion_fileptr->fd);
#endif
  } else {
    return _lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"_lsion_file_get_opt_blksize: cannot find valid file flag (flags=%d)\n",sion_fileptr->flags);
  }
}
...
int _lsion_file_stat_file(const char *fname) {
  return _lsion_file_stat_file2(fname, SION_FILE_FLAG_POSIX);
}
...
int _lsion_file_stat_file2(const char *fname, unsigned int apiflag) {
  // _lsion_file_stat_file above is incomplete, because it does not
...
#error "No platform selected."
    ...
    return _lsion_errorprint(0,_SION_ERROR_RETURN,"_lsion_file_stat_file2: cannot find valid api flag (flags=%d)\n",apiflag);
  }
}
 ...
 */
sion_int64 _lsion_file_set_position( _sion_fileptr *sion_fileptr, sion_int64 startpointer ) {
  if (NULL == sion_fileptr) return SION_SIZE_NOT_VALID;

  if(sion_fileptr->flags & SION_FILE_FLAG_ANSI) {
    if(sion_fileptr->flags & SION_FILE_FLAG_SCNDANSI)  {
     return _lsion_file_set_position_ansi(sion_fileptr->second_fileptr, startpointer);
    } else {
      return _lsion_file_set_position_ansi(sion_fileptr->fileptr, startpointer);
    }
  } else  if(sion_fileptr->flags & SION_FILE_FLAG_POSIX) {
    return _lsion_file_set_position_posix(sion_fileptr->fd, startpointer);
#if defined(_SION_SIONFWD)
  } else if(sion_fileptr->flags & SION_FILE_FLAG_SIONFWD) {
    ...
    return _lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"_lsion_file_set_position: cannot find valid file flag (flags=%d)\n",sion_fileptr->flags);
  }
}
 ...
 */
sion_int64 _lsion_file_get_position( _sion_fileptr *sion_fileptr ) {
  if (NULL == sion_fileptr) return SION_SIZE_NOT_VALID;

  if(sion_fileptr->flags & SION_FILE_FLAG_ANSI) {
    if(sion_fileptr->flags & SION_FILE_FLAG_SCNDANSI)  {
      return _lsion_file_get_position_ansi(sion_fileptr->second_fileptr);
    } else {
      return _lsion_file_get_position_ansi(sion_fileptr->fileptr);
    }
  } else if(sion_fileptr->flags & SION_FILE_FLAG_POSIX) {
    return _lsion_file_get_position_posix(sion_fileptr->fd);
#if defined(_SION_SIONFWD)
  } else if(sion_fileptr->flags & SION_FILE_FLAG_SIONFWD) {
    return sion_fileptr->position;
#endif
  } else {
    return _lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"_lsion_file_get_position: cannot find valid file flag (flags=%d)\n",sion_fileptr->flags);
  }
}
 ...
 */
int _lsion_file_flush( _sion_fileptr *sion_fileptr ) {
  if (NULL == sion_fileptr) return SION_NOT_SUCCESS;

  if(sion_fileptr->flags & SION_FILE_FLAG_ANSI) {
    if(sion_fileptr->flags & SION_FILE_FLAG_SCNDANSI)  {
      return _lsion_file_flush_ansi(sion_fileptr->second_fileptr);
    } else {
      return _lsion_file_flush_ansi(sion_fileptr->fileptr);
    }
  } else if(sion_fileptr->flags & SION_FILE_FLAG_POSIX) {
    return _lsion_file_flush_posix(sion_fileptr->fd);
#if defined(_SION_SIONFWD)
  } else if(sion_fileptr->flags & SION_FILE_FLAG_SIONFWD) {
    return _sion_file_flush_sionfwd(sion_fileptr->fd);
#endif
  } else {
    return _lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_file_flush: cannot find valid file flag (flags=%d)\n",sion_fileptr->flags);
  }
}
 ...
 * @return  `SION_SUCCESS` if ok
 */
int _lsion_file_purge( _sion_fileptr *sion_fileptr ) {
  if( sion_fileptr == NULL ) return SION_NOT_SUCCESS;

  if(sion_fileptr->flags & SION_FILE_FLAG_ANSI) {
    if(sion_fileptr->flags & SION_FILE_FLAG_SCNDANSI)  {
      return _lsion_file_purge_ansi(sion_fileptr->second_fileptr);
    } else {
      return _lsion_file_purge_ansi(sion_fileptr->fileptr);
    }
  } else if(sion_fileptr->flags & SION_FILE_FLAG_POSIX) {
    return _lsion_file_purge_posix(sion_fileptr->fd);
#if defined(_SION_SIONFWD)
  } else if(sion_fileptr->flags & SION_FILE_FLAG_SIONFWD) {
    return _sion_file_purge_sionfwd(sion_fileptr->fd);
#endif
  } else {
    return _lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_file_purge: cannot find valid file flag (flags=%d)\n",sion_fileptr->flags);
  }
}
 ...
 */
int _lsion_file_set_buffer(_sion_fileptr *sion_fileptr, char *buffer, sion_int32 buffer_size) {
  if( sion_fileptr == NULL ) return SION_NOT_SUCCESS;

  if(sion_fileptr->flags & SION_FILE_FLAG_ANSI) {
    if(sion_fileptr->flags & SION_FILE_FLAG_SCNDANSI)  {
      return _lsion_file_set_buffer_ansi(sion_fileptr->second_fileptr, buffer, buffer_size);
    } else {
      return _lsion_file_set_buffer_ansi(sion_fileptr->fileptr, buffer, buffer_size);
    }
  } else if(sion_fileptr->flags & SION_FILE_FLAG_POSIX) {
    return _lsion_file_set_buffer_posix(sion_fileptr->fd, buffer, buffer_size);
#if defined(_SION_SIONFWD)
  } else if(sion_fileptr->flags & SION_FILE_FLAG_SIONFWD) {
    return _sion_file_set_buffer_sionfwd(sion_fileptr->fd, buffer, buffer_size);
#endif
  } else {
    return _lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_file_set_buffer: cannot find valid file flag (flags=%d)\n",sion_fileptr->flags);
  }
}
...
int _lsion_file_set_second_fileptr( _sion_fileptr *sion_fileptr, FILE* fileptr) {
  ...
  if(sion_fileptr->flags & SION_FILE_FLAG_ANSI) {
    if(sion_fileptr->flags & SION_FILE_FLAG_SCNDANSI)  {
      return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"internal error: second fileptr already set, aborting ...\n"));
    } else {
    ...
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"internal error: second fileptr could not be set for non-ANSI file, aborting ...\n"));
  }
}
...
int _lsion_file_unset_second_fileptr( _sion_fileptr *sion_fileptr) {
  ...
  if(sion_fileptr->flags & SION_FILE_FLAG_ANSI) {
    if(sion_fileptr->flags & SION_FILE_FLAG_SCNDANSI)  {
      ...
      return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"internal error: second fileptr was not set, aborting ...\n"));
    }
  } else {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"internal error: second fileptr could not be unset for non-ANSI file, aborting ...\n"));
  }
}
...
int _lsion_file_get_fd( _sion_fileptr *sion_fileptr) {
  ...
  if(sion_fileptr->flags & SION_FILE_FLAG_POSIX) {
    ...
    return _lsion_errorprint(SION_ID_UNDEF,_SION_ERROR_RETURN,"_lsion_file_get_fd: cannot find valid file flag (flags=%d)\n",sion_fileptr->flags);
  }
}
...
sion_int64 _lsion_file_set_position_ansi(FILE *fileptr, sion_int64 startpointer) {
  ...
  if (offset != startpointer) {
    ...
    return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"_sion_set_position_ansi: cannot set position to %ld (%zu),%lld (%zu)  offset conversion error\n",
      offset, sizeof(offset), startpointer, sizeof(startpointer)));
  }
  int fd = fileno(fileptr);
  off_t result;
...
int _lsion_file_purge_ansi(FILE *fileptr) {
  return _lsion_file_flush_ansi(fileptr);
}
...
int _lsion_file_set_buffer_ansi(FILE *fileptr, char *buffer, sion_int32 buffer_size) {
  DPRINTFP((32, "_lsion_file_set_buffer", -1, "set buffer of fileptr\n"));
...
sion_int64 _lsion_file_set_position_posix(int fd, sion_int64 startpointer) {
  ...
  DPRINTFP((32, "_sion_set_position_posix", -1, "enter (to %lld)\n", (long long) startpointer));
    ...
    off_t offset = (off_t) startpointer;
    if (offset != startpointer) {
      return (_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"_sion_set_position_posix: cannot set position to %ld (%zu),%lld (%zu)  offset conversion error\n",
        offset, sizeof(offset), startpointer, sizeof(startpointer)));
    }
    off_t result;
    newpos = result = lseek(fd, offset, SEEK_SET);
...
_sion_fileptr *_lsion_file_alloc_and_init_sion_fileptr(void) {
  ...
  if (sion_fileptr == NULL) {
    _lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"cannot allocate _sion_fileptr structure of size %lu (_sion_fileptr), aborting ...\n",
 ...
 */
char* _lsion_fileptrflags_to_str (unsigned int flag) {
  switch (flag) {
