...
int _lsion_open(const char *fname, const char *file_mode, int *ntasks, int *nfiles, sion_int64 **chunksizes, sion_int32 *fsblksize, int **globalranks, FILE **fileptr)
{

  int     sid=SION_ID_UNDEF;
  _sion_flags_store* flags_store = NULL;

  DPRINTFP((1, "_lsion_open", 0, "enter open of file %s in %s mode\n", fname, file_mode));

  flags_store = _lsion_parse_flags(file_mode);
  if ( ! flags_store ) {
    return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_open: could not parse file mode in %s, aborting ...\n", file_mode));
  }

  if (flags_store->mask&_SION_FMODE_WRITE) {
    /* **************** WRITE mode **************** */

    sid=_lsion_open_write(fname,flags_store->mask,ntasks,nfiles,chunksizes,fsblksize,globalranks,fileptr);

  } else {
    /* **************** READ mode **************** */
    sid=_lsion_open_read(fname,flags_store->mask,_SION_READ_ALL_OF_MULTI_FILES,ntasks,nfiles,chunksizes,fsblksize,globalranks,fileptr);

  }

  _lsion_flags_destroy_store(&flags_store);

  DPRINTFP((1, "_lsion_open", 0, "leave open of file %s in %s mode sid=%d\n", fname, file_mode,sid));
 ...
 */
int _lsion_open_write(const char *fname, sion_int64 file_mode_flags, int *ntasks, int *nfiles, sion_int64 **chunksizes, sion_int32 *fsblksize, int **globalranks, FILE **fileptr)
{
  ...
  /* check parameter */
  if (*ntasks<0) {
    return(_lsion_errorprint(SION_ID_UNDEF,_SION_ERROR_RETURN,"lsion_open: wrong number of tasks specific: ntasks=%d (<0), returning ...\n", *ntasks));
  }

  /* check parameter */
  if ((chunksizes==NULL) || (*chunksizes==NULL)) {
    return(_lsion_errorprint(SION_ID_UNDEF,_SION_ERROR_RETURN,"lsion_open: chunksizes seems not to be a pointer to an array, returning ...\n"));
  }

  /* check parameter */
  if ((globalranks==NULL) || (*globalranks==NULL)) {
    return(_lsion_errorprint(SION_ID_UNDEF,_SION_ERROR_RETURN,"lsion_open: globalranks seems not to be a pointer to an array, returning ...\n"));
  }

  /* check parameter */
  if (*nfiles>1) {
    return(_lsion_errorprint(SION_ID_UNDEF,_SION_ERROR_RETURN,"lsion_open: write with nfiles > 1 currently not supported (nfiles=%d), returning ...\n",(int) *nfiles));
  }

  /* allocate and initialise internal data structure with default values (NULL and -1) */
  sion_filedesc = _lsion_alloc_filedesc();
  if (sion_filedesc == NULL) {
    return(_lsion_errorprint(SION_ID_UNDEF,_SION_ERROR_RETURN,"lsion_open: cannot allocate filedescriptor structure of size %lu (sion_filedesc), aborting ...\n", (unsigned long) sizeof(sion_filedesc)));
  }
  _lsion_init_filedesc(sion_filedesc);
  sion_filedesc->fname     = strdup(fname); /* Set the filename */

  /* New sion file handle */
  sid = _lsion_newvcd(sion_filedesc, SION_FILEDESCRIPTOR);
  sion_filedesc->sid=sid;

  /* Allocate memory for storing MAXCHUNKS chunksize infos in internal structure */
  _lsion_realloc_filedesc_blocklist(sion_filedesc, MAXCHUNKS);

  /* determine endianness */
  endianness = _lsion_get_endianness_with_flags(file_mode_flags);

  DPRINTFP((1, "lsion_open", 0, " it is a write operation #tasks=%d\n", *ntasks));
  ...
  _lsion_alloc_filedesc_arrays(sion_filedesc);

  /* open file */
  sion_fileptr = _lsion_file_open(fname,apiflag|SION_FILE_FLAG_WRITE|SION_FILE_FLAG_CREATE,0);
  if (!sion_fileptr) {
    return(_lsion_errorprint(SION_ID_UNDEF,_SION_ERROR_RETURN,"lsion_open: cannot open %s for reading, aborting ...\n", fname));
  }
  ...
  _lsion_keyval_check_env(sion_filedesc, file_mode_flags);
  if(sion_filedesc->keyvalmode!=SION_KEYVAL_NONE) {
    _lsion_alloc_filedesc_all_keyvalptr(sion_filedesc);
  }

  _lsion_calculate_startpointers(sion_filedesc);

  /* write header */
  _lsion_write_header(sion_filedesc);

  /* needed for writing pointer to var part of metadata at the end of the file */
  sion_filedesc->end_of_header = _lsion_file_get_position(sion_fileptr);
  sion_filedesc->start_of_data = sion_filedesc->all_startpointers[0];

  /* initalize current positions */
  _lsion_alloc_filedesc_block_arrays(sion_filedesc);
  for (i = 0; i < *ntasks; i++) {
  ...
  _lsion_file_set_position(sion_fileptr, sion_filedesc->currentpos);
  ...
  _lsion_print_filedesc(sion_filedesc, 512, "_lsion_open_write", 1);
...
int _lsion_open_read(const char *fname, sion_int64 file_mode_flags, int read_all, int *ntasks, int *nfiles, sion_int64 **chunksizes, sion_int32 *fsblksize, int **globalranks, FILE **fileptr)
{
  ...
  sion_filedesc = _lsion_alloc_filedesc();
  if (sion_filedesc == NULL) {
    return(_lsion_errorprint(SION_ID_UNDEF,SION_ID_UNDEF,"lsion_open: cannot allocate filedescriptor structure of size %lu (sion_filedesc), aborting ...\n", (unsigned long) sizeof(sion_filedesc)));
  }
  _lsion_init_filedesc(sion_filedesc);
  sion_filedesc->fname     = strdup(fname); /* Set the filename */
  ...
  DPRINTFP((1, "lsion_open", 0, " it is a read operation\n"));

  /* open file */
  sion_fileptr = _lsion_file_open(fname,apiflag|SION_FILE_FLAG_READ,0);
  if (!sion_fileptr) {
    return(_lsion_errorprint(SION_ID_UNDEF,_SION_ERROR_RETURN,"lsion_open: cannot open %s for reading, aborting ...\n", fname));
  }
  sion_filedesc->fileptr = sion_fileptr;
  ...
  rc = _lsion_read_header_fix_part(sion_filedesc);
  if (rc!=SION_SUCCESS) {
    return(_lsion_errorprint(SION_ID_UNDEF,_SION_ERROR_RETURN,"lsion_open: cannot read header from file %s, aborting ...\n", fname));
  }
  sion_filedesc->rank  = 0;
  ...
  rc = _lsion_read_header_var_part(sion_filedesc);
  if (rc!=SION_SUCCESS) {
    return(_lsion_errorprint(SION_ID_UNDEF,_SION_ERROR_RETURN,"lsion_open: cannot read header (var part) from file %s, aborting ...\n", fname));
  }
  ...
  _lsion_alloc_filedesc_block_arrays(sion_filedesc);
  rc = _lsion_read_header_var_part_blocksizes(sion_filedesc);
  if (rc!=SION_SUCCESS) {
    return(_lsion_errorprint(SION_ID_UNDEF,_SION_ERROR_RETURN,"lsion_open: cannot read header (var part block sizes) from file %s, aborting ...\n", fname));
  }
  ...
  if((sion_filedesc->nfiles>1) && (sion_filedesc->filenumber==0)) {
    ...
    if(read_all==_SION_READ_ALL_OF_MULTI_FILES) {
      sid=_lsion_open_read_master(fname,file_mode_flags,ntasks,nfiles,chunksizes,fsblksize,globalranks,fileptr,sion_filedesc);
    } else {
      sid=_lsion_open_read_single(fname,file_mode_flags,ntasks,nfiles,chunksizes,fsblksize,globalranks,fileptr,sion_filedesc);
      /* read mapping file, will be moved later to master file */
      _lsion_read_header_var_part_mapping(sion_filedesc);       
      { int rank;
	for (rank = 0; rank < sion_filedesc->mapping_size; rank++) {
	  DPRINTFP((2048, "lsion_open", 0, " mapping[%d] = %d , %d \n", rank,sion_filedesc->mapping[rank*2+0],sion_filedesc->mapping[rank*2+1]));
	}
      }
    }
...
int _lsion_open_read_single(const char *fname, sion_int64 file_mode_flags, int *ntasks, int *nfiles, sion_int64 **chunksizes, sion_int32 *fsblksize, int **globalranks, FILE **fileptr, _sion_filedesc *sion_filedesc)
{
  ...
  _lsion_file_set_position(sion_filedesc->fileptr, sion_filedesc->currentpos);
  ...
  *fsblksize = sion_filedesc->fsblksize;
  ...
  if (chunksizes != NULL) {
    ...
    if ((*chunksizes) == NULL) {
      ...
      if (helpptr == NULL) {
	return(_lsion_errorprint(SION_ID_UNDEF,_SION_ERROR_RETURN,"cannot allocate filedescriptor structure of size %lu (chunksizes), aborting ...\n", (unsigned long) sizeof(sion_int64)));
      }
      *chunksizes = helpptr;
    } else {
      helpptr = *chunksizes;
  ...
  if (globalranks != NULL) {
    ...
    if ((*globalranks) == NULL) {
      ...
      if (helpptr == NULL) {
	return(_lsion_errorprint(SION_ID_UNDEF,_SION_ERROR_RETURN,"lsion_open: cannot allocate memory of size %lu (globalranks), aborting ...\n", (unsigned long) (*ntasks) * sizeof(int)));
      }
      *globalranks = helpptr;
    } else {
      helpptr = *globalranks;
  ...
  _lsion_print_filedesc(sion_filedesc, 512, "_lsion_open_read_single", 1);

  return(sid);
}


int _lsion_open_read_master(const char *fname, sion_int64 file_mode_flags, int *ntasks, int *nfiles, sion_int64 **chunksizes, sion_int32 *fsblksize, int **globalranks, FILE **fileptr, _sion_filedesc *sion_filedesc)
{
  ...
  { int rank;
    for (rank = 0; rank < sion_filedesc->mapping_size; rank++) {
      DPRINTFP((32, "lsion_open", 0, " mapping[%d] = %d , %d \n", rank,sion_filedesc->mapping[rank*2+0],sion_filedesc->mapping[rank*2+1]));
    }
  }
  ...
  if (sion_filedesc_master == NULL) {
    return(_lsion_errorprint(SION_ID_UNDEF,_SION_ERROR_RETURN,"lsion_open: cannot allocate filedescriptor structure of size %lu (sion_filedesc_master), aborting ...\n", 
  ...
  /* New sion file handle */
  sid_master = _lsion_newvcd(sion_filedesc_master, SION_FILEDESCRIPTOR);
  ...
  if (sion_filedesc_master->multifiles == NULL) {
    return(_lsion_errorprint(SION_ID_UNDEF,_SION_ERROR_RETURN,"cannot allocate filedescriptor structure vector of size %lu (sion_filedesc), aborting ...\n", 
  ...
  for(filenr=1;filenr<sion_filedesc->nfiles;filenr++) {
    ...
    FILE *sub_fileptr;
    char *multi_fname;
    multi_fname = _lsion_get_multi_filename(fname, filenr);
    DPRINTFP((32, "lsion_open", 0, "open now sub file = %d %s\n", filenr, multi_fname));
    subsid=_lsion_open_read(multi_fname,
			   file_mode_flags, _SION_READ_ALL_OF_MULTI_FILES, &sub_ntasts, &sub_nfiles, NULL, &sub_chunksize, NULL, &sub_fileptr);
    sion_filedesc_sub=_lsion_get_filedesc(subsid);
    sion_filedesc_master->multifiles[filenr]=sion_filedesc_sub;
    DPRINTFP((32, "lsion_open", 0, "sub file = %d %s opened\n", filenr, multi_fname));
    free(multi_fname);
  }
  ...
  *fsblksize = sion_filedesc->fsblksize;
  ...
  _lsion_print_filedesc(sion_filedesc, 512, "_lsion_open_read_master", 1);

  return(sid);
}

int _lsion_open_rank(const char *fname, const char *file_mode, sion_int64 *chunksize, sion_int32 *fsblksize, int *rank, FILE **fileptr)
{
  ...
  DPRINTFP((1, "_lsion_open_rank", *rank, "enter open of file %s in %s mode\n", fname, file_mode));

  /* allocate and initialise internal data structure with default values (NULL and -1) */
  sion_filedesc = _lsion_alloc_filedesc();
  if (sion_filedesc == NULL) {
    return(_lsion_errorprint_on_rank(SION_ID_UNDEF,_SION_ERROR_RETURN,*rank,"_lsion_open_rank: cannot allocate filedescriptor structure of size %lu (sion_filedesc), aborting ...", (unsigned long) sizeof(sion_filedesc)));
  }
  _lsion_init_filedesc(sion_filedesc);
  sion_filedesc->fname = strdup(fname); /* Set the filename */
  ...
  flags_store = _lsion_parse_flags(file_mode);
  if ( ! flags_store ) {
    return(_lsion_errorprint_on_rank(SION_ID_UNDEF,_SION_ERROR_RETURN,*rank,"lsion_paropen_mpi: could not parse file mode in %s, aborting ...", file_mode));
  }
  ...
  if (flags_store->mask&_SION_FMODE_WRITE) {
    ...
    DPRINTFP((1, "_lsion_open_rank", *rank, " it is a write operation\n"));

    _lsion_flags_destroy_store(&flags_store);
    return(_lsion_errorprint_on_rank(SION_ID_UNDEF,_SION_ERROR_RETURN,*rank,"_lsion_open_rank: %s for writing currently not supported, aborting ...", fname));
...
/*                                                                      */ DPRINTFTS(*rank, "start open read");
  DPRINTFP((1, "_lsion_open_rank", *rank, " it is a read operation rank=%d\n",*rank));

  /* open file */
  sion_fileptr = _lsion_file_open(fname,apiflag|SION_FILE_FLAG_READ,0);
  if (!sion_fileptr) {
    return(_lsion_errorprint_on_rank(SION_ID_UNDEF,_SION_ERROR_RETURN,*rank,"_lsion_open_rank: cannot open %s for reading, aborting ...", fname));
  }
  sion_filedesc->fileptr = sion_fileptr;
  ...
  if (rc==SION_NOT_SUCCESS) {
    return(_lsion_errorprint_on_rank(SION_ID_UNDEF,_SION_ERROR_RETURN,*rank,"_lsion_open_rank: cannot read header from file %s, aborting ...", fname));
  }
  ...
  if (rc==SION_NOT_SUCCESS) {
    return(_lsion_errorprint_on_rank(SION_ID_UNDEF,_SION_ERROR_RETURN,*rank,"_lsion_open_rank: cannot read var part header from file %s, aborting ...", fname));
  }

  DPRINTFP((1, "_lsion_open_rank", *rank, "found number of files %d in %s rank=%d\n", sion_filedesc->nfiles,sion_filedesc->fname,*rank));
  if(sion_filedesc->nfiles>1) {
    /* read mapping for rank */
    rc=_lsion_read_header_var_part_mapping_rank(sion_filedesc);
    /*                                                                      */ DPRINTFTS(*rank, "start after read mapping rank 1");
    if (rc==SION_NOT_SUCCESS) {
      return(_lsion_errorprint_on_rank(SION_ID_UNDEF,_SION_ERROR_RETURN,*rank,"cannot read var part mapping header from file %s, aborting ...", fname));
    }
    mapping_filenr = sion_filedesc->mapping[0];
    mapping_lrank  = sion_filedesc->mapping[1];
    DPRINTFP((1, "_lsion_open_rank", *rank, "data for rank %d is in file %d (lrank=%d)\n", *rank, mapping_filenr,mapping_lrank));
    ...
    if(mapping_filenr>0) {
      ...
      /* close current file */
      _lsion_free_filedesc_arrays(sion_filedesc);
      _lsion_file_close(sion_filedesc->fileptr);
      sion_filedesc->fileptr=NULL;
      /*                                                                      */ DPRINTFTS(*rank, "start after close file 1");

      /*  and open again the correct one */
      sion_filedesc->fname=_lsion_get_multi_filename(fname,mapping_filenr);

      DPRINTFP((1, "_lsion_open_rank", *rank, "open file sion_filedesc->fname=%s  fname=%s rank=%d\n", sion_filedesc->fname,fname,*rank));

      sion_fileptr = _lsion_file_open(sion_filedesc->fname,apiflag|SION_FILE_FLAG_READ,0);
      if (!sion_fileptr) {
        return(_lsion_errorprint_on_rank(SION_ID_UNDEF,_SION_ERROR_RETURN,*rank,"_lsion_open_rank: cannot open %s for reading, aborting ...", fname));
      }
      sion_filedesc->fileptr = sion_fileptr;
      ...
      rc = _lsion_read_header_fix_part(sion_filedesc);
      if (rc==SION_NOT_SUCCESS) {
        return(_lsion_errorprint_on_rank(SION_ID_UNDEF,_SION_ERROR_RETURN,*rank,"lsion_open: cannot read header from file %s, aborting ...", fname));
      }
    /*                                                                      */ DPRINTFTS(*rank, "start after read header fix part i");
      _lsion_alloc_filedesc_arrays(sion_filedesc);
      rc = _lsion_read_header_var_part(sion_filedesc);
      if (rc==SION_NOT_SUCCESS) {
        return(_lsion_errorprint_on_rank(SION_ID_UNDEF,_SION_ERROR_RETURN,*rank,"cannot var mapping data from file %s, aborting ...", fname));
      }
  ...
  DPRINTFP((1, "_lsion_open_rank", *rank, "max blocks=%d\n", sion_filedesc->maxusedchunks));
  _lsion_realloc_filedesc_blocklist(sion_filedesc, sion_filedesc->maxusedchunks);
  ...
  rc = _lsion_read_header_var_part_blocksizes_rank(sion_filedesc);
  if (rc==SION_NOT_SUCCESS) {
    return(_lsion_errorprint_on_rank(SION_ID_UNDEF,_SION_ERROR_RETURN,sion_filedesc->rank,"cannot read header from file %s, aborting ...", fname));
  }
  ...
  *fsblksize=sion_filedesc->fsblksize;
  ...
  _lsion_print_filedesc(sion_filedesc, 512, "_lsion_open_rank", 1);

  DPRINTFP((1, "_lsion_open_rank", 0, "leave open of file %s in %s mode\n", fname, file_mode));
...
int _lsion_close(_sion_filedesc *sion_filedesc)
{
  ...
  DPRINTFP((1, "_lsion_close", -1, "enter close   sid=%d currentpos=%15lld\n", sion_filedesc->sid, sion_filedesc->currentpos));
  _lsion_print_filedesc(sion_filedesc, 512, "_lsion_close", 1);
  ...
  if ((sion_filedesc->state != SION_FILESTATE_SEROPEN)
    ...
    return(_lsion_errorprint_on_rank(SION_NOT_SUCCESS,_SION_ERROR_RETURN,sion_filedesc->rank,"_lsion_close: invalid file open state (!SEROPEN), aborting %d ...", sion_filedesc->sid));
  }

  if (sion_filedesc->mode == SION_FILEMODE_WRITE) {
    /* write rest of metadata in headers */

    DPRINTFP((1, "_lsion_close", -1, " file was opened in write mode sid=%d setpos to %lld\n", sion_filedesc->sid, sion_filedesc->end_of_header));

    /* update meta data of current rank */
    _lsion_flush_block(sion_filedesc);
    DPRINTFP((1, "_lsion_close", -1, " after flush block sid=%d  fileptr is at position %14lld\n", sion_filedesc->sid, _lsion_file_get_position(sion_filedesc->fileptr)));

    if (sion_filedesc->usebuffer) {
      _lsion_buffer_flush(sion_filedesc);
    }
    ...
    _lsion_write_header_var_info(sion_filedesc);
    ...
    _lsion_write_header_var_part_blocksizes(sion_filedesc);

    /* write mapping info to second meta data block */
    mapping_size = 0;
    _lsion_write_header_var_part_mapping(sion_filedesc,mapping_size,NULL);
    ...
    if(sion_filedesc->state == SION_FILESTATE_SEROPENMASTER) {
      for(filenr=1;filenr<sion_filedesc->nfiles;filenr++) {
	_lsion_close(sion_filedesc->multifiles[filenr]);
	_lsion_freevcd(sion_filedesc->multifiles[filenr]->sid);
	_lsion_free_filedesc(sion_filedesc->multifiles[filenr]);
  ...
  DPRINTFP((1, "_lsion_close", -1, "leave close   sid=%d\n", sion_filedesc->sid));
 ...
 */
char * _lsion_get_multi_filename(const char *fname, int filenumber)
{
  char *newfname;

  newfname = malloc(SION_FILENAME_LENGTH);
  if (newfname == NULL) {
    _lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_get_multi_filename: Cannot allocate string newfname\n");
    return(NULL);
  }
  if(filenumber>0) {
    if(strlen(fname)<SION_FILENAME_LENGTH-7) {
      ...
      _lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_get_multi_filename: filename too long\n");
      free(newfname);
      return(NULL);
    }
  } else {
 ...
 */
int _lsion_flush_file(int sid)
{
  int       rc = SION_SUCCESS;
  _sion_filedesc *sion_filedesc;

  if ((_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR) || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_flush_file: invalid sion_filedesc, aborting %d ...\n", sid));
  }
  DPRINTFP((32, "_lsion_flush_file", sion_filedesc->rank, "flush sid=%d\n", sid));
 ...
 * @retval SION_SUCCESS if OK
 */
int _lsion_update_fileposition( _sion_filedesc *sion_filedesc )
{
  int       rc = SION_SUCCESS;

  DPRINTFP((2, "_lsion_update_fileposition", sion_filedesc->rank, "enter\n"));
  ...
  if(sion_filedesc->fileptr_exported) {
    ...
    if( (!sion_filedesc->usecoll) || (sion_filedesc->collector==sion_filedesc->rank) ) { 
      _lsion_file_flush(sion_filedesc->fileptr);
      sion_filedesc->currentpos = _lsion_file_get_position(sion_filedesc->fileptr);
    } 
#endif
  ...
  DPRINTFP((2, "_lsion_update_fileposition", sion_filedesc->rank, "leave pos=%lld usecoll=%d collector=%d rank\n",
 ...
 * @retval SION_SUCCESS if OK
 */
int _lsion_check_on_collective_mode( _sion_filedesc *sion_filedesc) {
  int       rc = SION_SUCCESS;
  ...
  /* if(sion_filedesc->fileptr_exported) { */
	...
	_lsion_file_set_position(sion_filedesc->fileptr,sion_filedesc->currentpos);;
      ...
      return(rc);
      /* return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_WARN,"_lsion_check_on_collective_mode: individual SION read/write function call in collective mode, switching back to no-collective mode")); */
    }
  /* } */
  return(rc);
}
 ...
 * @retval 1 if OK
 */
int _lsion_flush_block( _sion_filedesc *sion_filedesc )
{
  int       rc = SION_SUCCESS;
  sion_int64 byteswritten;

  DPRINTFP((2, "_lsion_flush_block", sion_filedesc->rank, "enter fileposition=%lld\n", sion_filedesc->currentpos));

  _lsion_update_fileposition(sion_filedesc);

  DPRINTFP((2, "_lsion_flush_block", sion_filedesc->rank, "after update fileposition=%lld\n", sion_filedesc->currentpos));
  ...
  if (byteswritten > 0) {
    ...
    DPRINTFP((2, "_lsion_flush_block", sion_filedesc->rank,
    ...
    rc = SION_NOT_SUCCESS;
    DPRINTFP((2, "_lsion_flush_block", sion_filedesc->rank, "not flushed lastchunknr->%d byteswritten=%lld fileposition=%lld startpos=%lld  (%lld)\n",
  ...
  DPRINTFP((2, "_lsion_flush_block", sion_filedesc->rank, "leave\n"));
 ...
 * @retval  SION_SUCCESS if OK
 */
int _lsion_create_new_block( _sion_filedesc *sion_filedesc )
{
  int       rc = SION_SUCCESS;

  DPRINTFP((2, "_lsion_create_new_block", _SION_DEFAULT_RANK, "enter alloc \n"));
  ...
  if(sion_filedesc->currentblocknr<sion_filedesc->lastchunknr) {
    ...
    _lsion_apply_hints(sion_filedesc,SION_HINTS_FREE_TYPE_CHUNK);
    ...
    rc = _lsion_apply_hints(sion_filedesc,SION_HINTS_ACCESS_TYPE_CHUNK);

    /* advance fp to next block */
    if(sion_filedesc->fileptr) {
      _lsion_file_flush(sion_filedesc->fileptr);
      rc = _lsion_file_set_position(sion_filedesc->fileptr, sion_filedesc->currentpos);
    }
    
    DPRINTFP((2, "_lsion_create_new_block", _SION_DEFAULT_RANK, "skip to next already allocated block currentblocknr=%d lastchunknr->%d currentpos=%lld\n", 
    ...
    if ((sion_filedesc->lastchunknr + 1) >= sion_filedesc->maxchunks) {
      _lsion_realloc_filedesc_blocklist(sion_filedesc, sion_filedesc->maxchunks + MAXCHUNKS);
    }
    ...
    DPRINTFP((2, "_lsion_create_new_block", _SION_DEFAULT_RANK, "skip to new allocated block currentblocknr=%d lastchunknr->%d currentpos=%lld\n", 
	      sion_filedesc->currentblocknr, sion_filedesc->lastchunknr, sion_filedesc->currentpos));
 ...
 *  return endianness
 */
sion_int32 _lsion_get_endianness_with_flags(sion_int64 flags) {
  sion_int32 endianness = 0;
  
  DPRINTFP((2, "_sion_get_endianness_flags", -1, "enter with flags 0x%x\n", flags));

  /* determine endianness */
  endianness = lsion_get_endianness();
...
#define DFUNCTION "_lsion_getenv"
/*! encapsulate getenv
 *
 */
char *_lsion_getenv(const char *name) {
 ...
 */
sion_io_stat_t* _lsion_alloc_io_info(int p_nf) {
  ...
  if (p == NULL) {
    _lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_alloc_io_info: cannot allocate memory of size %lu (p_fn), aborting ...\n",
  ...
  if (p->names == NULL) {
    _lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_alloc_io_info: cannot allocate memory of size %lu (names), aborting ...\n",
  ...
  if (p->sizes == NULL) {
    _lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_alloc_io_info: cannot allocate memory of size %lu (sizes), aborting ...\n",
  ...
  if (p->roles == NULL) {
    _lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_alloc_io_info: cannot allocate memory of size %lu (roles), aborting ...\n",
 ...
 *    rc
 */
int _lsion_free_io_info(sion_io_stat_t *p) {
  int rc=SION_SUCCESS;
