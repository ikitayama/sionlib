...
#define SION_CACHE_TODISK           40          /*!< Cache will store data directly to disk (in sion_write or in lsion_close if file is opened for reading and cachedata is dirty) */
...
struct _sion_filedesc_struct {
  ...
  sion_int32 usebuffer;            /*!< is set to 1 if data is/should be buffered internally in lsion_fwrite/lsion_fread */
  char*      buffer;               /*!< buffer for internal buffering data in lsion_fwrite/lsion_fread */
  ...
  sion_int32 collmsa;              /*!< adapt collector groups to peculiarities of the Modular Supercomputer Architecture (MSA) */
...
int _lsion_print_filedesc( _sion_filedesc *sion_filedesc, int level, char *desc, int all );
