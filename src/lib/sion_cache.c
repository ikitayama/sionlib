...
#if defined(_SION_LINUX)
      rc=_lsion_cache_create_linux(sion_filedesc);
#elif defined(_SION_AIX)
#elif defined(_SION_BGP)
#endif
    } else if(sion_filedesc->mode == SION_FILEMODE_READ) {
#if defined(_SION_LINUX)
      rc=_lsion_cache_load_linux(sion_filedesc);
#elif defined(_SION_AIX)
#elif defined(_SION_BGP)
#endif
    } else {
      _lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_cache_init: unknown SION mode\n");
    }
  }
...
int  _lsion_cache_destroy(_sion_filedesc *sion_filedesc) {
  ...
  if(sion_filedesc->usecache) {
#if defined(_SION_LINUX)
    rc=_lsion_cache_destroy_linux(sion_filedesc);
#elif defined(_SION_AIX)
#elif defined(_SION_BGP)
#endif
  }
