 ...
 */
int lsion_open(char *fname, const char *file_mode, int *ntasks, int *nfiles, sion_int64 **chunksizes, sion_int32 *fsblksize, int **globalranks, FILE **fileptr)
{

  int     sid=0;

  DPRINTFP((1, "lsion_open", 0, "enter open of file %s in %s mode\n", fname, file_mode));
  
  sid=_lsion_open(fname,file_mode,ntasks,nfiles,chunksizes,fsblksize,globalranks,fileptr);
 ...
 * @retval              sid             sion file handle or -1 if error occured
 */
int lsion_open_rank(char *fname, const char *file_mode, sion_int64 *chunksize, sion_int32 *fsblksize, int *rank, FILE **fileptr)
{
  int             sid;

  /*                                                                      */ DPRINTFTS(*rank, "before open rank");
  DPRINTFP((1, "lsion_open_rank", *rank, "enter open of file %s in %s mode\n", fname, file_mode));
  sid=_lsion_open_rank(fname, file_mode, chunksize, fsblksize, rank, fileptr);
  DPRINTFP((1, "lsion_open_rank", 0, "leave open of file %s in %s mode sid=%d\n", fname, file_mode,sid));
 ...
 */
int lsion_close(int sid)
{

  int       rc = 0;
  DPRINTFP((1, "lsion_close", -1, "enter close   sid=%d\n", sid));
  rc=_lsion_close_sid(sid);
  DPRINTFP((1, "lsion_close", -1, "leave close   sid=%d\n", sid));
