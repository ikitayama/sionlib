 ...
 */
int _lsion_dup( int sid, int mode, int rank, uint64_t key ) {
  int        rc = SION_NOT_SUCCESS, new_sid = SION_ID_NOT_VALID;
  _sion_filedesc *sion_filedesc;
  _sion_filedesc *new_filedesc;

  /*                                                                      */ DPRINTFTS(-1, "enter lsion_dup");
  DPRINTFP((2, DFUNCTION, -1, "enter dup sid=%d\n",sid));

  if ((sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR)
      || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN, DFUNCTION ": invalid sion_filedesc, returning %d ...\n", sid));
  }

  if (sion_filedesc->mode != SION_FILEMODE_READ) {
    DPRINTFP((8, DFUNCTION, -1, "invalid opened\n"));
    return(_lsion_errorprint_on_rank(SION_ID_NOT_VALID,_SION_ERROR_RETURN,sion_filedesc->rank,
				    DFUNCTION "[%2d]: file is not opened in read mode, returning ...", sion_filedesc->rank));
  }

  _lsion_print_filedesc(sion_filedesc, 512, "_sion_dup_orig ", _SION_DEBUG_PRINT_ALL|_SION_DEBUG_PRINT_RECURSIVE);


  new_filedesc = _lsion_dup_filedesc(sion_filedesc);
  if (!new_filedesc) {
    return(_lsion_errorprint_on_rank(SION_ID_NOT_VALID,_SION_ERROR_RETURN,sion_filedesc->rank, DFUNCTION"[%2d]: could not duplicate internal data structure, returning ...", sion_filedesc->rank));
  }
  ...
  new_sid = _lsion_newvcd(new_filedesc, SION_FILEDESCRIPTOR);
  ...
  if (sion_filedesc->state == SION_FILESTATE_PAROPEN) {
    ...
    rc=_lsion_dup_paropen(sion_filedesc, new_filedesc);
  }
  ...
  if (sion_filedesc->state == SION_FILESTATE_PAROPENMAPPEDMASTER) {
    ...
    rc=_lsion_dup_paropenmappedmaster(sion_filedesc, new_filedesc);
  }
  ...
  if ( (sion_filedesc->state == SION_FILESTATE_SEROPEN) || 
    ...
    /* _lsion_print_filedesc(new_filedesc, 512, "_sion_dup_clone_before_seek", _SION_DEBUG_PRINT_ALL|_SION_DEBUG_PRINT_RECURSIVE); */
    lsion_seek(new_filedesc->sid,rank,SION_CURRENT_CHUNK,SION_CURRENT_POS);
  }

  _lsion_print_filedesc(new_filedesc, 512, "_sion_dup_clone", _SION_DEBUG_PRINT_ALL|_SION_DEBUG_PRINT_RECURSIVE);

  DPRINTFP((2, DFUNCTION, -1, "leave dup sid=%d new_sid=%d\n",sid,new_sid));
  /*                                                                      */ DPRINTFTS(-1, "leave lsion_dup");
...
int _lsion_dedup( int sid ) {
  ...
  if (sion_filedesc->dup_mode == SION_DESCSTATE_ORIG) {
    return(_lsion_errorprint_on_rank(SION_NOT_SUCCESS,_SION_ERROR_RETURN,sion_filedesc->rank, DFUNCTION "[%2d]: descriptor is not a duplicate, returning ...", sion_filedesc->rank));
  }
  ...
  if ( (sion_filedesc->state == SION_FILESTATE_SEROPENMASTER) 
	...
	sion_filedesc_sub->keyvalptr = NULL;
      ...
      if (sion_filedesc_sub->usebuffer)  _lsion_buffer_flush(sion_filedesc_sub);
...
#define DFUNCTION "_lsion_dup_paropen"
/*!
 * @brief Duplicate sion_filedesc if the file is opened in paropen_mapped_master.
 *
 * @return     SION_SUCCESS if okay
 */
int _lsion_dup_paropen( _sion_filedesc *sion_filedesc, _sion_filedesc *new_filedesc ) {
  int rc = SION_SUCCESS;

  DPRINTFP((2, DFUNCTION, -1, "enter \n"));
  
  _lsion_dup_blocksizes(sion_filedesc, new_filedesc);
  
  /* open again the file */
  new_filedesc->fileptr=_lsion_file_open(new_filedesc->fname,sion_filedesc->fileptr->flags,0);
  ...
  _lsion_file_set_position(new_filedesc->fileptr, new_filedesc->currentpos);

 
  /* duplicate keyval structures */
  if(new_filedesc->keyvalmode!=SION_KEYVAL_NONE) {
    _lsion_dup_keyvalptr(sion_filedesc, new_filedesc);
  }
...
#define DFUNCTION "_lsion_dup_paropenmappedmaster"
/*!
 * @brief Duplicate sion_filedesc if the file is opened in paropen_mapped_master.
 *
 * @return     SION_SUCCESS if okay
 */
int _lsion_dup_paropenmappedmaster( _sion_filedesc *sion_filedesc, _sion_filedesc *new_filedesc ) {
  int rc = SION_SUCCESS;
  _sion_filedesc *sion_filedesc_sub;
  int filenr, lfile, lrank;
  ...
  for(filenr=0;filenr<sion_filedesc->nfiles;filenr++) {
    new_filedesc->multifiles[filenr] = _lsion_dup_filedesc(sion_filedesc->multifiles[filenr]);
    ...
    if(sion_filedesc->multifiles[filenr]->fileptr!=NULL) { /* only if needed */
      ...
      new_filedesc->multifiles[filenr]->fileptr=_lsion_file_open(sion_filedesc->multifiles[filenr]->fname,
								sion_filedesc->multifiles[filenr]->fileptr->flags,0);
      if (!new_filedesc->multifiles[filenr]->fileptr) {
	return(_lsion_errorprint_on_rank(SION_ID_NOT_VALID,_SION_ERROR_RETURN,sion_filedesc->rank, DFUNCTION 
					"[%2d]: could not duplicate fileptr, returning ...", sion_filedesc->rank));
      }

      /* set filepointer to position in original descriptor */
      _lsion_file_set_position(new_filedesc->multifiles[filenr]->fileptr, new_filedesc->multifiles[filenr]->currentpos);

      /* copy further data structures */
      _lsion_dup_blocksizes(sion_filedesc->multifiles[filenr], new_filedesc->multifiles[filenr]);
      
      /* copy further all_* structures */
      _lsion_dup_all_ds(sion_filedesc->multifiles[filenr], new_filedesc->multifiles[filenr]);
      
      /* duplicate keyval structures */
      if(new_filedesc->multifiles[filenr]->keyvalmode!=SION_KEYVAL_NONE) {
	_lsion_dup_all_keyvalptr(sion_filedesc->multifiles[filenr], new_filedesc->multifiles[filenr]);
      }
...
#define DFUNCTION "_lsion_dup_blocksizes"
/*!
 * @brief Duplicate the blocksizes array of sion_filedesc.
 *
 * @return     SION_SUCCESS if okay
 */
int _lsion_dup_blocksizes( _sion_filedesc *sion_filedesc, _sion_filedesc *new_filedesc ) {
  int rc = SION_SUCCESS;
  int i;
  ...
  /* realloc array */
  _lsion_realloc_filedesc_blocklist(new_filedesc, new_filedesc->maxusedchunks);
...
#define DFUNCTION "_lsion_dup_all_ds"
/*!
 * @brief Duplicate the all* array of sion_filedesc.
 *
 * @return     SION_SUCCESS if okay
 */
int _lsion_dup_all_ds( _sion_filedesc *sion_filedesc, _sion_filedesc *new_filedesc ) {
  int rc = SION_SUCCESS;
  int i,ltask;
  ...
  /* alloc arrays */
  _lsion_alloc_filedesc_all_chunksizes(new_filedesc);
  _lsion_alloc_filedesc_all_globalranks(new_filedesc);
  _lsion_alloc_filedesc_all_localranks(new_filedesc);
  _lsion_alloc_filedesc_all_startpointers(new_filedesc);

  _lsion_alloc_filedesc_block_arrays(new_filedesc); /* all_blockcount, all_currentpos, all_currentblocknr, all_blocksizes */
...
#define DFUNCTION "_lsion_dup_keyvalptr"
/*!
 * @brief Duplicate the keyvalue data structure of sion_filedesc.
 *
 * @return     SION_SUCCESS if okay
 */
int _lsion_dup_keyvalptr( _sion_filedesc *sion_filedesc, _sion_filedesc *new_filedesc ) {
  int rc = 0;

  DPRINTFP((2, DFUNCTION, -1, "enter\n"));

  rc=_lsion_keyval_dup_dataptr(sion_filedesc,new_filedesc);
...
#define DFUNCTION "_lsion_dup_all_keyvalptr"
/*!
 * @brief Duplicate the keyvalue data structures of sion_filedesc.
 *
 * @return     SION_SUCCESS if okay
 */
int _lsion_dup_all_keyvalptr( _sion_filedesc *sion_filedesc, _sion_filedesc *new_filedesc ) {
  int rc = SION_SUCCESS;
  int ltask;
  void *savep;
  ...
  /* alloc arrays */
  _lsion_alloc_filedesc_all_keyvalptr(new_filedesc);
  ...
  for(ltask=0;ltask<new_filedesc->ntasks;ltask++) {
    if(sion_filedesc->all_keyvalptr[ltask]!=NULL) {
      ...
      _lsion_keyval_dup_dataptr(sion_filedesc,new_filedesc);
