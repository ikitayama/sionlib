...
/* Utility functions */
_sion_keyvalue_keymngr * _lsion_get_or_init_key_info(_sion_filedesc *sion_filedesc);
int _lsion_move_to_pos(_sion_filedesc *sion_filedesc, size_t pos);
int _lsion_scan_forward_to_key(_sion_filedesc *sion_filedesc, uint64_t key, int search_mode);
int _lsion_move_to_next_chunk(_sion_filedesc *sion_filedesc);
size_t _lsion_write_data_to_chunks_inline(_sion_filedesc *sion_filedesc, const void *data, sion_int64 bytes_to_write);
sion_int64 _lsion_compute_next_position_inline(_sion_filedesc *sion_filedesc, sion_int64 bytes_to_read);
size_t _lsion_skip_data_from_chunks_inline(_sion_filedesc *sion_filedesc, sion_int64 bytes_to_read);
size_t _lsion_read_data_from_chunks_inline(_sion_filedesc *sion_filedesc, void *data, sion_int64 bytes_to_read);
...
#define DFUNCTION "_lsion_store_and_write_key_and_len_inline"
size_t _lsion_store_and_write_key_and_len_inline(_sion_filedesc *sion_filedesc, uint64_t key, size_t len) {
  ...
  frc=_lsion_write_data_to_chunks_inline(sion_filedesc, (const void *) buffer, (sion_int64) bytes_to_write);
...
#define DFUNCTION "_lsion_write_value_inline"
size_t _lsion_write_value_inline(_sion_filedesc *sion_filedesc, const void *data, uint64_t key, size_t len) {
  ...
  bytes_to_write=len;
  frc=_lsion_write_data_to_chunks_inline(sion_filedesc, data, (sion_int64) bytes_to_write);
  if(frc != bytes_to_write) {
    return(_lsion_errorprint_on_rank(-1,_SION_ERROR_RETURN,sion_filedesc->rank,
				"could not write data (%d bytes) to file (frc=%d sid=%d) ...",  (int) bytes_to_write, (int) frc, sion_filedesc->sid));
  }
...
#define DFUNCTION "_lsion_find_and_read_key_and_len_inline"
int _lsion_find_and_read_key_and_len_inline(_sion_filedesc *sion_filedesc, uint64_t key, size_t len, size_t *datalen) {
  ...
  keymngr=_lsion_get_or_init_key_info(sion_filedesc);
  ...
  *datalen=0;  
  ...
  rc=_lsion_keyvalue_keymngr_lookup(keymngr, mkey,  &key_current_pos, &key_bytes_left);
  if(rc!=SION_SUCCESS) {
    DPRINTFP((2, DFUNCTION, -1, "key %ld is not found in keymngr, scan forward\n", (long)mkey));

    /* scan forward until key is found */
    rc=_lsion_scan_forward_to_key(sion_filedesc, key, SEARCH_TO_KEY);
  ...
  if(position_found) {
    if(key_current_pos!=sion_filedesc->currentpos) {
      rc=_lsion_move_to_pos(sion_filedesc, key_current_pos);
    }
    /* ready to read data  */
  }
...
#define DFUNCTION "_lsion_read_value_inline"
size_t _lsion_read_value_inline(_sion_filedesc *sion_filedesc, void *data, uint64_t key, size_t len) {
  ...
  bread=_lsion_read_data_from_chunks_inline(sion_filedesc, (void *) out, (sion_int64) bytes_to_read);
  if (bread != bytes_to_read) {
    return(_lsion_errorprint_on_rank(SION_NOT_SUCCESS,_SION_ERROR_RETURN,sion_filedesc->rank,
  ...
  _lsion_keyvalue_keymngr_update_read_pos(keymngr, mkey, (size_t) bread, (sion_int64) sion_filedesc->currentpos);
...
#define DFUNCTION "_lsion_key_full_scan_inline"
int _lsion_key_full_scan_inline(_sion_filedesc *sion_filedesc) {
  ...
  if(!_lsion_keyvalue_keymngr_is_scan_done(keymngr)) {
    rc=_lsion_scan_forward_to_key(sion_filedesc, key, SEARCH_TO_END);
  } else {
    /* scan already done */
    rc=SION_SUCCESS;
  }
...
#define DFUNCTION "_lsion_iterator_reset_inline"
int _lsion_iterator_reset_inline(_sion_filedesc *sion_filedesc) {
  ...
  rc=_lsion_keyvalue_keymngr_iterator_reset(keymngr);
...
#define DFUNCTION "_lsion_iterator_next_inline"
int _lsion_iterator_next_inline(_sion_filedesc *sion_filedesc, uint64_t *keyptr, size_t *sizeptr) {
  ...
  if(_lsion_keyvalue_keymngr_iterator_next(keymngr, &key, &current_pos, &offset, &len)==SION_SUCCESS) {
    /* next block found */
    rc=SION_SUCCESS;
  } else {
    /* scan forward if not at end of file to get more meta data */
    if (_lsion_scan_forward_to_key(sion_filedesc, key, SEARCH_TO_NEXT)==SION_SUCCESS) {
...
#define DFUNCTION "_lsion_seek_key_inline"
int _lsion_seek_key_inline(_sion_filedesc *sion_filedesc, uint64_t key, int blocknum, sion_int64 posinblock) {
  ...
  rc=_lsion_keyvalue_keymngr_lookup_and_set_pos(keymngr, key, blocknum, posinblock, &offset, &len);
  DPRINTFP((2, DFUNCTION, -1, "after first lookup rc=%d offset=%d len=%d\n",rc, (int)offset, (int) len ));

  while( (rc!=SION_SUCCESS)  && !_lsion_keyvalue_keymngr_is_scan_done(keymngr) ) {
  ...
  if( rc==SION_SUCCESS ) {
    ...
    rc=_lsion_move_to_pos(sion_filedesc, offset);
  }
...
#define DFUNCTION "_lsion_key_list_iterator_reset_inline"
int _lsion_key_list_iterator_reset_inline(_sion_filedesc *sion_filedesc) {
  ...
  rc=_lsion_keyvalue_keymngr_key_list_iterator_reset(keymngr);
...
int _lsion_key_list_iterator_next_inline(_sion_filedesc *sion_filedesc, uint64_t *keyptr) {
  ...
  if(_lsion_keyvalue_keymngr_key_list_iterator_next(keymngr, &key)==SION_SUCCESS) {
    *keyptr  = (uint64_t) key;
    rc=SION_SUCCESS;
  } else {
    *keyptr  = 0;
...
#define DFUNCTION "_lsion_key_get_stat_inline"
int _lsion_key_get_stat_inline(_sion_filedesc *sion_filedesc, uint64_t searchkey, sion_key_stat_t *keystat) {
  int rc=SION_NOT_SUCCESS;
  _sion_keyvalue_keymngr* keymngr;
  sion_table_key_t key=0;
  ...
  if(_lsion_keyvalue_keymngr_key_get_stat(keymngr, key, keystat)==SION_SUCCESS) {
    rc=SION_SUCCESS;
  } else {
    rc=SION_NOT_SUCCESS;
  }
...
#define DFUNCTION "_lsion_scan_forward_to_key"
int _lsion_scan_forward_to_key(_sion_filedesc *sion_filedesc, uint64_t key, int search_mode) {
  int        rc = SION_SUCCESS;
  _sion_keyvalue_keymngr *keymngr;
  sion_table_key_t mkey;
  ...
  if(_lsion_keyvalue_keymngr_get_next_scan_pos(keymngr, &scanpos) != SION_SUCCESS) {
  ...
  if(scanpos>=lastposinfile) {
    ...
    _lsion_keyvalue_keymngr_set_scan_done(keymngr);
    rc=SION_NOT_SUCCESS;
  }

  /* move to scanpos if necessary */
  if( (sion_filedesc->currentpos != scanpos) && (!_lsion_keyvalue_keymngr_is_scan_done(keymngr)) ) {
    DPRINTFP((2, DFUNCTION, -1, "move from %ld to new scanpos (%ld) \n",(long) sion_filedesc->currentpos,(long) scanpos));
    rc=_lsion_move_to_pos(sion_filedesc, scanpos);
  }

  /* search forward */
  while ( (!_lsion_keyvalue_keymngr_is_scan_done(keymngr)) && (!key_found) ) {
    ...
    if ( bytes_left > 0 ) {
      ...
      bread=_lsion_read_data_from_chunks_inline(sion_filedesc, (void *) key_and_len, (sion_int64) sizeof(key_and_len));
      if (bread != sizeof(key_and_len)) {
	return(_lsion_errorprint_on_rank(SION_NOT_SUCCESS,_SION_ERROR_RETURN,sion_filedesc->rank,
					"could not read data (%d bytes) from file ...",  (int) sizeof(key_and_len)));
      }
      lsion_swap(key_and_len, key_and_len, sizeof(sion_int64), 2, sion_filedesc->swapbytes);
	...
	if(! _lsion_move_to_next_chunk(sion_filedesc)) {
	  return(_lsion_errorprint_on_rank(SION_NOT_SUCCESS,_SION_ERROR_RETURN,sion_filedesc->rank,
	...
	DPRINTFP((2, DFUNCTION, -1, "search loop: position is moved to %ld bytes_left=%ld\n",(long) sion_filedesc->currentpos, (long) bytes_left));
      ...
      mkey=key_and_len[0];
      len =key_and_len[1];
      rc=_lsion_keyvalue_keymngr_add_block(keymngr, mkey, (size_t) sion_filedesc->currentpos, len);
	...
	scanpos=_lsion_compute_next_position_inline(sion_filedesc, len); /* next search behind data of that block */
      } else {
	...
	bskip=_lsion_skip_data_from_chunks_inline(sion_filedesc, (sion_int64) len);
	...
	if(bytes_left==0) {
	  ...
	  if(! _lsion_move_to_next_chunk(sion_filedesc)) {
	    _lsion_keyvalue_keymngr_set_scan_done(keymngr); 		/* end of loop */
  ...
  /* update scanpos */
  if(_lsion_keyvalue_keymngr_set_next_scan_pos(keymngr, scanpos) != SION_SUCCESS) {
    return(_lsion_errorprint_on_rank(SION_NOT_SUCCESS,_SION_ERROR_RETURN,sion_filedesc->rank,
...
#define DFUNCTION "_lsion_get_or_init_key_info"
_sion_keyvalue_keymngr * _lsion_get_or_init_key_info(_sion_filedesc *sion_filedesc) {
  ...
  if (sion_filedesc->keyvalptr == NULL) {
    ...
    keymngr =_lsion_keyvalue_keymngr_init(TABLE_SIZE);
    if (keymngr == NULL) {
      _lsion_errorprint_on_rank(-1,_SION_ERROR_RETURN,sion_filedesc->rank,"could not allocate keymngr ...");
    }
...
#define DFUNCTION "_lsion_move_to_next_chunk"
int _lsion_move_to_next_chunk(_sion_filedesc *sion_filedesc) {
  int rc = SION_SUCCESS;
...
#define DFUNCTION "_lsion_move_to_pos"
int _lsion_move_to_pos(_sion_filedesc *sion_filedesc, size_t pos) {
  int rc = SION_SUCCESS;
...
#define DFUNCTION "_lsion_write_data_to_chunks_inline"
size_t _lsion_write_data_to_chunks_inline(_sion_filedesc *sion_filedesc, const void *data, sion_int64 bytes_to_write) {
  ...
  while(bytes_to_write>0) {
    ...
    if ((err == cudaSuccess) && (!attrs.isManaged && (attrs.memoryType == cudaMemoryTypeDevice)) ) {
      ...
      frc = _lsion_file_write((char *)data + offset, btowr, sion_filedesc->fileptr);
    }
#else
    frc = _lsion_file_write(((char *) data+offset), btowr, sion_filedesc->fileptr);
#endif
...
#define DFUNCTION "_lsion_compute_next_position_inline"
sion_int64 _lsion_compute_next_position_inline(_sion_filedesc *sion_filedesc, sion_int64 bytes_to_read) {
  ...
  if ((bytesread + bytes_to_read) <= sion_filedesc->blocksizes[sion_filedesc->currentblocknr]) {
	...
	return(_lsion_errorprint_on_rank(-1,_SION_ERROR_RETURN,sion_filedesc->rank,"internal error next block not available, but should  ..."));
      }
...
/* skip data block from current and following chunks, data could be distributed over several chunks */
#define DFUNCTION "_lsion_skip_data_from_chunks_inline"
size_t _lsion_skip_data_from_chunks_inline(_sion_filedesc *sion_filedesc, sion_int64 bytes_to_read) {
...
#define DFUNCTION "_lsion_read_data_from_chunks_inline"
size_t _lsion_read_data_from_chunks_inline(_sion_filedesc *sion_filedesc, void *data, sion_int64 bytes_to_read) {
  sion_int64 btord, bytesread, offset;  
  size_t rc=0, frc;
  ...
  while(bytes_to_read>0) {
    ...
    if ((err == cudaSuccess) && (!attrs.isManaged && (attrs.memoryType == cudaMemoryTypeDevice)) ) {
      ...
      while (frc < btord) {
        ...
        sion_int64 frc_ = _lsion_file_read(buffer, to_read, sion_filedesc->fileptr);
      ...
      frc = _lsion_file_read(((char *)data + offset), btord, sion_filedesc->fileptr);
    }
#else
    frc = _lsion_file_read(((char *)data+offset), btord, sion_filedesc->fileptr);
#endif
...
#define DFUNCTION "_lsion_keyval_dup_dataptr_inline"
int _lsion_keyval_dup_dataptr_inline(_sion_filedesc *sion_filedesc, _sion_filedesc *new_filedesc) {
  ...
  if(sion_filedesc->keyvalptr!=NULL) {
    ...
    keymngr =_lsion_keyvalue_keymngr_dup(keymngr_orig,new_filedesc->dup_mode, new_filedesc->dup_sel_key);
    if (keymngr == NULL) {
      return(_lsion_errorprint_on_rank(-1,_SION_ERROR_RETURN,sion_filedesc->rank,"dup: could not duplicate keymngr ..."));
    }
