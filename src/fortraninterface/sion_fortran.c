...
#define DFUNCTION "lfsion_open_c"
/*!
		  ...
		  int file_mode_len)
{
  ...
  if (!strcmp(fmode_tmp, "bw") || !strcmp(fmode_tmp, "wb")) {
    /* **************** WRITE mode **************** */
    (*sid) = lsion_open(fname_tmp, fmode_tmp, ntasks, nfiles, &chunksizes, fsblksize, &globalranks, NULL);
  } else {
    (*sid) = lsion_open(fname_tmp, fmode_tmp, ntasks, nfiles, NULL, fsblksize, NULL, NULL);
  }
 ...
 * from each task instead of read once and distribute (lsion_open).
 * lsion_open_rank reads only the tasks specific meta data  from the meta data
                       ...
                       int file_mode_len)
{
  ...
  (*sid) = lsion_open_rank(fname_tmp, fmode_tmp, chunksize, fsblksize, rank, NULL);
 ...
 *
 * This procedure closes a sion file which was opened in serial mode with lsion_open or lsion_open_rank.
 ...
 * for this task, lsion_feof will advance the filepointer to the start position of the
...
#define DFUNCTION "lfsion_seek_c"
/*!
                  ...
                  int *ierr)
{
  ...
  (*ierr) = lsion_seek(*sid, *rank, *currentblocknr, *posinblk);
}
#undef DFUNCTION
                               ...
                               int *ierr)
{
  ...
  (*ierr) = lsion_ensure_free_space(*sid, *bytes);
}
...
sion_int64 lfsion_bytes_avail_in_block_c(int *sid)
{
  ...
  return lsion_bytes_avail_in_block(*sid);
}
                           ...
                           int *ierr)
{
  ...
  (*ierr) = lsion_get_locations(*sid, ntasks, maxblocks, globalskip, start_of_varheader, sion_localsizes,
                                   ...
                                   int *ierr)
{
  ...
  (*ierr) = lsion_get_current_locations(*sid, ntasks, sion_currentpos, sion_currentblocknr);
}
			    ...
			    int *ierr)
{
  ...
  if ((_lsion_vcdtype(*sid) != SION_FILEDESCRIPTOR)
...
#define DFUNCTION "lfsion_get_mapping_c"
                        ...
                        int *filenumber)
{
  ...
  *filenumber = _lsion_file_get_fd(sion_filedesc->fileptr);
  return;
}
...
void lfsion_get_file_endianness_c(int *sid, 
                                 int *endianness)
{
  *endianness = lsion_get_file_endianness(*sid);
  return;
}
 ...
 */
void lfsion_get_endianness_c(int *endianness)
{
...
void lfsion_endianness_swap_needed_c(int *sid,
                                    int *needed)
{
  *needed = lsion_endianness_swap_needed(*sid);
  return;
}
