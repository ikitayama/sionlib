...
/*!
 * @brief Fortran wrapper function that calls lsion_paropen_mpi for 1 file
 *
...
#pragma omp master
  {
	  if (*globalrank == 0){
		...
		fprintf(stderr,"lfsion_paropen_ompi_c: filename_len: %d\n", (int) fname_len);
		fprintf(stderr,"lfsion_paropen_ompi_c: filename: %s\n",fname_tmp);

		fprintf(stderr,"lfsion_paropen_ompi_c: filemode_len: %d\n", (int) file_mode_len);
		fprintf(stderr,"lfsion_paropen_ompi_c: filemode: %s\n",fmode_tmp);

		fprintf(stderr,"lfsion_paropen_ompi_c: chunksize: %lld\n",*chunksize);
		fprintf(stderr,"lfsion_paropen_ompi_c: fsblksize: %d\n",*fsblksize);
		fprintf(stderr,"lfsion_paropen_ompi_c: numFiles:  %d\n",*numFiles);
	  }
  }
#pragma omp barrier
#endif

  (*sid) = lsion_paropen_ompi(fname_tmp, fmode_tmp, numFiles, cgComm, &clComm, chunksize, fsblksize, globalrank, NULL, NULL);
                                ...
                                int        newfname_len)
{
  ...
  (*sid) = lsion_paropen_ompi(fname_tmp, fmode_tmp, numFiles, cgComm, &clComm, chunksize, fsblksize, globalrank, NULL, &newfname_tmp);
 ...
 */
void lfsion_parclose_ompi_c(int *sid, 
                           int *ierr)
{
  (*ierr) = lsion_parclose_ompi(*sid);
}
