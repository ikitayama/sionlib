...
/*!
 * @brief Fortran wrapper function that calls lsion_paropen_omp
 *
		         ...
		         int        newfname_len) 
{
  ...
  if (*globalrank == 0){
    fprintf(stderr,"lfsion_paropen_omp: filename_len: %d\n", (int) fname_len);
    fprintf(stderr,"lfsion_paropen_omp: filename: %s\n",fname_tmp);

    fprintf(stderr,"lfsion_paropen_omp: filemode_len: %d\n", (int) file_mode_len);
    fprintf(stderr,"lfsion_paropen_omp: filemode: %s\n",fmode_tmp);
    
    fprintf(stderr,"lfsion_paropen_omp: chunksize: %lld\n",*chunksize);
    fprintf(stderr,"lfsion_paropen_omp: fsblksize: %d\n",*fsblksize);
  }
#endif

  (*sid) = lsion_paropen_omp(fname_tmp, fmode_tmp, chunksize, fsblksize, globalrank, NULL, NULL);
 ...
 */
void lfsion_parclose_omp_c(int *sid, 
                          int *ierr)
{
  (*ierr) = lsion_parclose_omp(*sid);
}
