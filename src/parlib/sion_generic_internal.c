 ...
 *          -1 if error occured
 */
int _lsion_paropen_generic_one_file(
				   int    sid,
				   char  *fname,
				   ...
				   _sion_generic_buddy   *buddy_data )
{
  ...
  DPRINTFP((2, "_lsion_paropen_generic_one_file", rank, "enter parallel open of file %s in mode %d #tasks=%d\n", fname, (int) flags_store->mask, ntasks));
  DPRINTFP((32, "_lsion_paropen_generic_one_file", rank, "sizeof: int=%d long=%d longlong=%d sion_int32=%d sion_int64=%d\n", sizeof(int), sizeof(long),
  ...
  DPRINTFP((2, "_lsion_paropen_generic_one_file", rank, "do_open_file=%d\n", do_open_file));

  sion_filedesc = _lsion_alloc_filedesc();
  if (sion_filedesc == NULL) {
    _lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_ABORT,"_sion_paropen_omp: cannot allocate filedescriptor structure of size %lu (sion_filedesc), aborting ...\n",
		     (unsigned long) sizeof(sion_filedesc));
  }
  _lsion_init_filedesc(sion_filedesc);
  sion_filedesc->fname       = strdup(fname); /* Set the filename */

  _lsion_reassignvcd(sid,sion_filedesc, SION_FILEDESCRIPTOR);
  ...
  if (flags_store->mask&_SION_FMODE_WRITE) {
    ...
    DPRINTFP((32, "_lsion_paropen_generic_one_file", rank, " starting open for write #tasks=%d\n", ntasks));

    /* check parameter */
    if (ntasks<0) {
      return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"sion_paropen: wrong number of tasks specific: ntasks=%d (<0), returning ...\n", (int) ntasks));
    }

    /* check parameter */
    if ((chunksize != NULL) && (*chunksize<0)) {
      return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"sion_paropen: ((chunksize != NULL) && (*chunksize<0)), returning ...\n"));
    }

    /* check parameter */
    if ((flag & _SION_INTERNAL_FLAG_NORMAL ) && (globalrank != NULL) && (*globalrank<0)) {
      return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"sion_paropen: ((globalrank != NULL) && (*globalrank<0)), returning ...\n"));
    }
	    
    sion_filedesc->state      = SION_FILESTATE_PAROPEN;
    sion_filedesc->mode       = SION_FILEMODE_WRITE;
    sion_filedesc->endianness = _lsion_get_endianness_with_flags(flags_store->mask); /* Endianness */
    ...
    sion_filedesc->collmsa       = !!_lsion_flags_get(flags_store, "collmsa");
    sion_filedesc->usebuddy      = (flags_store->mask&_SION_FMODE_BUDDY)>0;
    if(sion_filedesc->usebuddy) {
      sion_filedesc->buddylevel    = atoi(_lsion_flags_get(flags_store,"buddy")->val);
	...
	_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_WARN,"sion_paropen_generic: requested coalescing I/O but API does not support this mode, falling back to individual mode ...\n");
    ...
    if (rank == 0) {
      _lsion_hints_check_env(sion_filedesc);
    }
    ...
    if (rank == 0) {
      ...
      if(sion_filedesc->usecoll)  _lsion_alloc_filedesc_coll_arrays(sion_filedesc);
    }
    ...
    DPRINTFP((32, "_lsion_paropen_generic_one_file", rank, "lchunksize=%lld lglobalrank=%lld\n", lchunksize,lglobalrank));
    ...
    if (rank == 0) {
      ...
      DPRINTFP((32, "_lsion_paropen_generic_one_file", rank, "chunksizes[%d - 1]=%ld\n", ntasks,(long) sion_filedesc->all_chunksizes[ntasks - 1]));
      if (!sion_filedesc->usecoll) _lsion_calculate_startpointers(sion_filedesc);
      else {
	if (sion_filedesc->collmergemode) _lsion_calculate_startpointers_collective_merge(sion_filedesc);
        else if (sion_filedesc->collmsa) _lsion_calculate_startpointers_collective_msa(sion_filedesc);
	else                               _lsion_calculate_startpointers_collective(sion_filedesc);
      }
    ...
    if(do_open_file) {
      ...
      sion_fileptr = _lsion_file_open(fname,apiflag|SION_FILE_FLAG_WRITE,0);
      if (!sion_fileptr) {
	return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"sion_paropen_generic: cannot open %s for writing, aborting ...\n", fname));
      }
    ...
    if (rank == 0) {
      ...
      _lsion_apply_hints(sion_filedesc,SION_HINTS_ACCESS_TYPE_METADATABLOCK1);
	...
	+ sion_filedesc->all_chunksizes[ntasks - 1];
      ...
      _lsion_file_set_position(sion_filedesc->fileptr, lstartpointer);
    ...
    if(sion_filedesc->usecoll) {
      ...
      _lsion_free_filedesc_coll_arrays(sion_filedesc);
    }

    /* distribute globalskip */
    sion_gendata->apidesc->bcastr_cb(&sion_filedesc->globalskip, comm_group, _SION_INT64, 1, 0);

    DPRINTFP((32, "_lsion_paropen_generic_one_file", rank, " start position is %10lld %10.4f MB chunksize=%10lld %10.4f MB\n", 
    ...
    if(do_open_file) {
      _lsion_file_flush(sion_filedesc->fileptr);
      _lsion_file_set_position(sion_filedesc->fileptr, sion_filedesc->startpos);
    }
    ...
    */
    ...
    _lsion_apply_hints(sion_filedesc,SION_HINTS_ACCESS_TYPE_CHUNK);

    /*                                                                      */ DPRINTFTS(rank, "after setp");
    DPRINTFP((32, "_lsion_paropen_generic_one_file", rank, " ending open for write #tasks=%d filepos=%lld\n", ntasks, _lsion_file_get_position(sion_filedesc->fileptr)));

  }
  else if (flags_store->mask&_SION_FMODE_READ) {
    /* **************** READ mode **************** */
    if (rank == 0)
      DPRINTFP((32, "_lsion_paropen_generic_one_file", rank, " starting open for read #tasks=%d\n", ntasks));
    ...
    if(do_open_file) {
      ...
      if (!sion_fileptr) {
	DPRINTFP((32, "_lsion_paropen_generic_one_file", rank, " cannot open %s for reading, aborting ...\n", fname));
	return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_generic_one_file: cannot open %s for reading, aborting ...\n", fname));
      }
    } else {
      sion_fileptr=NULL;
    }
	...
	sion_filedesc->collsize=sion_filedesc->ntasks;
      ...
      DPRINTFP((32, "_lsion_paropen_generic_one_file", rank, " create buddy mapping ntasks=%d filentasks=%d\n",ntasks,sion_filedesc->ntasks));

      if (rank == 0) {
	sion_tmpintfield_buddy32 = (sion_int32 *) malloc(ntasks * sizeof(sion_int32));
	if (sion_tmpintfield_buddy32 == NULL) {
	  return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_generic_one_file: cannot allocate temporary memory of size %lu (sion_tmpintfield_buddy), aborting ...\n",
	...
	if (sion_tmpintfield_map == NULL) {
	  ...
	  return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_generic_one_file: cannot allocate temporary memory of size %lu (sion_tmpintfield_map), aborting ...\n",
				  (unsigned long) ntasks * sizeof(sion_int32)));
	}
	for (j = 0; j < ntasks; j++)  sion_tmpintfield_map[j]=-1;
      ...
      if (rank == 0) {
	for (j = 0; j < ntasks; j++)
	  DPRINTFP((64, "_lsion_paropen_generic_one_file", rank, " buddy map[%d]=%d\n", j, (int) sion_tmpintfield_map[j]));
      }
      
    }

    if (rank == 0) {
      rc = _lsion_read_header_fix_part(sion_filedesc); /* overwrites sion_filedesc->ntasks */
      if (rc!=SION_SUCCESS) {
        free(sion_tmpintfield_buddy32);
        free(sion_tmpintfield_buddy64);
	return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_generic_one_file: cannot read header from file %s, aborting ...\n", fname));
      }
      DPRINTFP((32, "_lsion_paropen_generic_one_file", rank,
    ...
    DPRINTFP((32, "_lsion_paropen_generic_one_file", rank," usecoll=%d\n", sion_filedesc->usecoll));
    ...
    if (sion_filedesc->maxusedchunks > MAXCHUNKS)  _lsion_realloc_filedesc_blocklist(sion_filedesc, sion_filedesc->maxusedchunks);
    ...
    if (rank == 0) {
      ...
      if (sion_tmpintfield == NULL) {
        free(sion_tmpintfield_buddy32);
	return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_generic_one_file: cannot allocate temporary memory of size %lu (sion_tmpintfield), aborting ...\n",
				(unsigned long) ntasks * sizeof(sion_int64)));
      }
      _lsion_read_header_var_part_blockcount_to_field(sion_filedesc, sion_filedesc->ntasks, sion_tmpintfield);

      for (j = 0; j < sion_filedesc->ntasks; j++)
	DPRINTFP((2048, "_lsion_paropen_generic_one_file", rank, " read, blockcount on task %02d is %10ld\n", j, (long) sion_tmpintfield[j]));
    }
    ...
    DPRINTFP((32, "_lsion_paropen_generic_one_file", rank, "  lastchunknr on task %02d is %10ld\n", rank, (long) sion_filedesc->lastchunknr));

    for (i = 0; i < sion_filedesc->maxusedchunks; i++) {
      if (rank == 0) _lsion_read_header_var_part_nextblocksizes_to_field(sion_filedesc, sion_filedesc->ntasks, sion_tmpintfield);
      /*                                                                      */ DPRINTFTS(rank, "before scatter");
      if (! (flag&_SION_INTERNAL_FLAG_BUDDY_READ) ) {
	sion_gendata->apidesc->scatterr_cb(sion_tmpintfield, &helpint64, comm_group, _SION_INT64, 1, 0);
      } else {
...
#define BGFLUSH
  ...
  _lsion_print_filedesc(sion_filedesc, 512, "_lsion_paropen_generic_one_file", 1);

  DPRINTFP((32, "_lsion_paropen_generic_one_file", rank, " start position on task %02d is at end of sion_paropen_generic %10lld\n", rank,
	    _lsion_file_get_position(sion_filedesc->fileptr)));

  DPRINTFP((2, "_lsion_paropen_generic_one_file", rank, "leave parallel open of file %s in mode 0x%lx #tasks=%d\n", fname, (long) flags_store->mask, ntasks));
 ...
 */
int _lsion_parclose_generic(int sid,
			   int rank,
			   int ntasks,
			   int mapping_size,
			   sion_int32 *mapping,
			   ...
			   _sion_generic_buddy   *buddy_data)
{
  ...
  if ((_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR) || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_parclose_generic: invalid sion_filedesc, aborting %d ...\n", sid));
  }

  if (sion_filedesc->state != SION_FILESTATE_PAROPEN) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_parclose_generic: sion file with sid=%d was not opened by a sion_paropen\n", sid));
  }

  DPRINTFP((2, "_lsion_parclose_generic", rank, "enter parallel close  sid=%d\n", sid));
  ...
  if (sion_filedesc->mode == SION_FILEMODE_READ) {
    if (sion_filedesc->state != SION_FILESTATE_CLOSE) {
      ...
      _lsion_print_filedesc(sion_filedesc, 512, "_lsion_parclose_generic", 1);
      DPRINTFP((32, "_lsion_parclose_generic", rank, " parallel close (read mode)  sid=%d, call fclose on file\n", sid));
    ...
    /* _lsion_buffer_flush(sion_filedesc); */ /* clear internal buffer */
    ...
    if (rank != 0) {
      if (sion_filedesc->state != SION_FILESTATE_CLOSE) {
        DPRINTFP((32, "_lsion_parclose_generic", rank, " parallel close (write mode)  sid=%d, call fclose on file\n", sid));
	if(do_close_file) {
	  _lsion_file_close(sion_filedesc->fileptr);
	}
        sion_filedesc->fileptr = NULL;
    ...
    DPRINTFP((32, "_lsion_parclose_generic", rank, " parallel close sid=%d: lastchunknr=%d globalskip=%lld\n", sid, sion_filedesc->lastchunknr,
              sion_filedesc->globalskip));
    for (blknum = 0; blknum <= sion_filedesc->lastchunknr; blknum++) {
      DPRINTFP((1024, "_lsion_parclose_generic", rank, " parallel close sid=%d: local block %02d -> %10lld bytes\n", sid, blknum,
    ...
    if (rank == 0) {
      sion_tmpintfield = (sion_int64 *) malloc(sion_filedesc->ntasks * sizeof(sion_int64));
      if (sion_tmpintfield == NULL) {
	return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_parclose_generic: cannot allocate temporary memory of size %lu (sion_tmpintfield), aborting ...\n",
    ...
    if (rank == 0) {
      ...
      _lsion_write_header_var_part_blockcount_from_field(sion_filedesc,sion_filedesc->ntasks,sion_tmpintfield);
    ...
    for (blknum = 0; blknum < sion_filedesc->maxusedchunks; blknum++) {
      ...
      if (rank == 0) {
        for (lrank = 0; lrank < ntasks; lrank++)
          DPRINTFP((2048, "_lsion_parclose_generic", rank, " parallel close sid=%d: write total chunksize for block %d: %2lld rank=%d\n", sid, blknum,
                    sion_tmpintfield[lrank], lrank));

	_lsion_write_header_var_part_nextblocksizes_from_field(sion_filedesc,sion_filedesc->ntasks,sion_tmpintfield);
    ...
    if (mapping != NULL) {
      _lsion_write_header_var_part_mapping(sion_filedesc, mapping_size, mapping);
    }
  ...
  _lsion_free_filedesc(sion_filedesc);
  sion_filedesc = NULL;


  DPRINTFP((2, "_lsion_parclose_generic", rank, "leave parallel close  sid=%d\n", sid));
			    ...
			    _sion_generic_gendata *sion_gendata)
{
  ...
  if ((_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR) || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_parreinit_generic: invalid sion_filedesc, aborting %d ...\n", sid));
  }
	  
  if (sion_filedesc->state != SION_FILESTATE_PAROPEN) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_parreinit_generic: sion file with sid=%d was not opened by a sion_paropen\n", sid));
  }
	  
  DPRINTFP((2, "_lsion_parreinit_generic", sion_filedesc->rank, "enter parallel reinit  sid=%d\n", sid));
  
  comm_group=sion_gendata->comm_data_local;

  if (sion_filedesc->mode == SION_FILEMODE_READ) {
        return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_parreinit_generic: sion file with sid=%d only allowed for files openend for write\n", sid));
  }
  ...
  DPRINTFP((32, "_lsion_parreinit_generic", sion_filedesc->rank, " start position is %10lld %10.4f MB\n", 
	    sion_filedesc->startpos, sion_filedesc->startpos / 1024.0 / 1024.0));
  ...
  DPRINTFP((32, "_lsion_parreinit_generic", sion_filedesc->rank, " ending open for write #tasks=%d filepos=%lld\n", 
	    sion_filedesc->ntasks, _lsion_file_get_position(sion_filedesc->fileptr)));

  DPRINTFP((2, "_lsion_parreinit_generic", sion_filedesc->rank, "leave parallel reinit of file %s in  #tasks=%d\n", 
 ...
 * @return 1 if successful
 */
#define DFUNCTION "_lsion_generic_collect_mapping"
int _lsion_generic_collect_mapping( _sion_filedesc *sion_filedesc,
				   int            *mapping_size,
				   sion_int32    **mapping ) {
    int rc=SION_SUCCESS;
    int t;
	...
	*mapping = (sion_int32 *) malloc(*mapping_size * 2 * sizeof(sion_int32));
	if (*mapping == NULL) {
	  return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"lsion_generic_parclose: Cannot allocate memory for mapping"));
	}
      } 
	...
	if (receivemap == NULL) {
	  return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"lsion_generic_parclose: Cannot allocate memory for receivemap"));
	}
      }
 ...
 *
 */
int _lsion_generic_renumber_collmsa(_sion_generic_gendata *comm, _sion_flags_store *flags)
{
  ...
  if (0 != strcmp(comm->apidesc->name, "SIONlib_MPI_API")) {
    return _lsion_errorprint(SION_NOT_SUCCESS, _SION_ERROR_RETURN, "MSA Collectives: not supported for generic API \"%s\"\n", comm->apidesc->name);
  }

  _sion_filedesc fd;
  _lsion_init_filedesc(&fd);
  fd.ntasks = gsize;
  _sion_flags_entry *flags_entry = _lsion_flags_get(flags, "collsize");
  if (flags_entry) {
    fd.collsize = atoi(flags_entry->val);
  }
  _lsion_coll_check_env(&fd);
  sion_int32 collsize = fd.collsize;
  if (collsize < 2) {
    return _lsion_errorprint(SION_NOT_SUCCESS, _SION_ERROR_RETURN, "MSA Collectives: size of collective groups should be 2 or more, but is %d\n", collsize);
  }
  sion_int32 n_groups = gsize / collsize + ((gsize % collsize) ? 1 : 0);

  if (!fd.usecoll) {
    return _lsion_errorprint(SION_NOT_SUCCESS, _SION_ERROR_RETURN, "MSA Collectives: usecoll == false\n");
  }

  int is_candidate = _lsion_generic_is_candidate(comm);
  int n_candidates = 0;
  ...
  // bring your own Allreduce and Exscan
  {
    ...
    if (0 == comm->grank) {
      ...
      if (!candidates) {
        _lsion_errorprint(SION_NOT_SUCCESS, _SION_ERROR_ABORT, "MSA Collectives: malloc returned NULL\n");
      }
    }
  ...
  if ((n_candidates < n_groups) || (n_candidates < comm->numfiles)) {
    return _lsion_errorprint(SION_NOT_SUCCESS, _SION_ERROR_RETURN, "MSA Collectives: insufficient number of candidates %d, number of groups %d, number of files %d\n", n_groups, comm->numfiles);
...
int _lsion_generic_is_candidate(_sion_generic_gendata *comm) {
  ...
  regex_t compiled;
  int compile_error;
  if ((regex = _lsion_getenv("SION_MSA_COLLECTOR_HOSTNAME_EREGEX"))) {
    compile_error = regcomp(&compiled, regex, REG_EXTENDED|REG_ICASE|REG_NOSUB);
  } else if ((regex = _lsion_getenv("SION_MSA_COLLECTOR_HOSTNAME_REGEX"))) {
  ...
  if (compile_error) {
    ...
    regfree(&compiled);
    _lsion_errorprint(SION_NOT_SUCCESS, _SION_ERROR_ABORT, "MSA Collectives: error compiling regex \"%s\": %s%s\n", regex, error_msg, (error_size > 1024) ? "..." : "");
  }
  ...
  if (hostname_error) {
    regfree(&compiled);
    _lsion_errorprint(SION_NOT_SUCCESS, _SION_ERROR_ABORT, "MSA Collectives: error getting host name\n");
  }
  ...
  if (exec_status == 0) {
    ...
    regfree(&compiled);
    _lsion_errorprint(SION_NOT_SUCCESS, _SION_ERROR_ABORT, "MSA Collectives: error matching regex \"%s\": %s%s\n", regex, error_msg, (error_size > 1024) ? "..." : "");
