...
int _lsion_gen_info_from_gcomm_ompi(int numFiles, MPI_Comm gComm, int *filenumber, int *lrank, int *lsize)
{
  int       gtasks, gRank;
  int       rc = SION_SUCCESS;
  int       task_per_file;
  ...
  DPRINTFP((1, "_lsion_get_info_from_splitted_comm_mpi", gRank, "enter: gcomm: %d of %d, numfiles=%d\n", 
	    gRank,gtasks,numFiles));

  if (gtasks < numFiles) {
    return(_lsion_errorprint_ompi(SION_NOT_SUCCESS,_SION_ERROR_RETURN,
  ...
  DPRINTFP((1, "_lsion_get_info_from_splitted_comm_mpi", gRank, "Global communicator divided in %d local communicators (%d: %d of %d)\n", 
	    numFiles,*filenumber,*lrank,*lsize));

  return (rc);
}

int _lsion_get_info_from_splitted_comm_ompi(MPI_Comm gComm, MPI_Comm lComm, int *numComm, int *CommNumber, int *lrank, int *lsize) {
  ...
  if (gRank == 0) {
    ...
    if ((allRanks == NULL) || (allSizes == NULL)) {
      free(allRanks);
      free(allSizes);
      return(_lsion_errorprint_ompi(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_get_info_from_splitted_comm_mpi: Cannot allocate temp arrays of size %lu\n", (unsigned long) gSize * sizeof(int)));
    }
  }
...
int _lsion_errorprint_ompi(int rc, int level, const char *format, ...)
{
  int       rank=-1;
  va_list ap;
...
int _lsion_map_rank_ompi_to_mpi(int ompi_rank, int num_threads) {
  int mpi_rank = 0;
...
int _lsion_map_rank_ompi_to_thread_num(int ompi_rank, int num_threads) {
  int thread_num = 0;
...
int _lsion_map_rank_mpi_to_ompi(int mpi_rank, int num_threads, int thread_num) {
  int ompi_rank = 0;
...
int _lsion_get_size_ompi(int mpi_size, int num_threads) {
  int ompi_size = 0;
