...
int _lsion_register_callbacks_mpi(void) {
  int aid=0;
  aid=lsion_generic_create_api("SIONlib_MPI_API");
  
 
  lsion_generic_register_create_local_commgroup_cb(aid,&_lsion_mpi_create_lcg_cb);
  lsion_generic_register_free_local_commgroup_cb(aid,&_lsion_mpi_free_lcg_cb);

  lsion_generic_register_barrier_cb(aid,&_lsion_mpi_barrier_cb);
  lsion_generic_register_bcastr_cb(aid,&_lsion_mpi_bcastr_cb);
  lsion_generic_register_gatherr_cb(aid,&_lsion_mpi_gatherr_cb);
  lsion_generic_register_scatterr_cb(aid,&_lsion_mpi_scatterr_cb);
  lsion_generic_register_gathervr_cb(aid,&_lsion_mpi_gathervr_cb);
  lsion_generic_register_scattervr_cb(aid,&_lsion_mpi_scattervr_cb);
  lsion_generic_register_gather_and_execute_cb(aid,&_lsion_mpi_gather_process_cb);
  lsion_generic_register_execute_and_scatter_cb(aid,&_lsion_mpi_process_scatter_cb);
  lsion_generic_register_get_capability_cb(aid,&_lsion_mpi_get_capability_cb);

  return(aid);
} 

int _lsion_mpi_create_lcg_cb(void **local_commgroup, void *global_commgroup, 
			    int grank, int gsize, 
			    int lrank, int lsize,
...
int _lsion_mpi_free_lcg_cb(void *local_commgroup) {
  int       rc = 0;
...
int _lsion_mpi_barrier_cb(void *commdata)
{
  int       rc;
...
int _lsion_mpi_bcastr_cb(void *data, void *commdata, int dtype, int nelem, int root)
{
  int       rc;
  _mpi_api_commdata* sapi= (_mpi_api_commdata *) commdata;
  MPI_Comm commp = sapi->comm;
...
int _lsion_mpi_gatherr_cb(void *indata, void *outdata, void *commdata, int dtype, int nelem, int root)
{
  int       rc;
  int       size, rank;
  _mpi_api_commdata* sapi= (_mpi_api_commdata *) commdata;
...
int _lsion_mpi_scatterr_cb(void *indata, void *outdata, void *commdata, int dtype, int nelem, int root)
{
  int       rc;
  _mpi_api_commdata* sapi= (_mpi_api_commdata *) commdata;
  MPI_Comm commp = sapi->comm;
...
int _lsion_mpi_gathervr_cb(void *indata, void *outdata, void *commdata, int dtype, int *counts, int nelem, int root)
{
  int       rc;
...
int _lsion_mpi_scattervr_cb(void *indata, void *outdata, void *commdata, int dtype, int *counts, int nelem, int root)
{
  int       rc;
  int       size, rank, t, offset;
  int       *displs=NULL;
