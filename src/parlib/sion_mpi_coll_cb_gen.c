...
int _lsion_mpi_gather_process_cb(const void *indata, sion_int64 *spec, int spec_len, sion_int64 fsblksize,
  ...
  if(rank == collector) {
    ...
    if (buffer == NULL) {
      return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_mpi_gather_process_cb: cannot allocate temporary memory of size %lu (buffer), aborting ...\n",
	...
	if(rc != SION_STD_SUCCESS) {
	  return(_lsion_errorprint(SION_STD_NOT_SUCCESS,_SION_ERROR_RETURN,"_mpi_gather_process_cb: problems writing data ...\n"));
	}
...
int _lsion_mpi_process_scatter_cb(void *outdata, sion_int64 *spec, int spec_len, sion_int64 fsblksize,
  ...
  if(rank == collector) {
    ...
    buffer = (char *) malloc(fsblksize * sizeof(char));
    if (buffer == NULL) {
      return(_lsion_errorprint(SION_STD_NOT_SUCCESS,_SION_ERROR_RETURN,"_mpi_gather_process_cb: cannot allocate temporary memory of size %lu (buffer), aborting ...\n",
			      (unsigned long) fsblksize * sizeof(char)));
    }
    ...
    if ( (rank>=range_start) && (rank<=range_end) ) {
      ...
      if(spec[0]!=-1) { 		/* no error in lsion_feof */
