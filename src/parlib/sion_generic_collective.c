...
/* collective I/O */
#define DFUNCTION "lsion_coll_fwrite"
size_t lsion_coll_fwrite(const void *data, size_t size, size_t nitems, int sid) {
  ...
  if ( (sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR) || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"lsion_coll_fwrite: invalid sion_filedesc %d", sid));
  }
  ...
  if(!sion_filedesc->usecoll) {
    return(lsion_fwrite(data,size,nitems,sid));
  } 

  /* branch to merge mode if enabled */
  if(sion_filedesc->collmergemode) {
    return(_lsion_coll_fwrite_merge(data,size,nitems,sid));
  } 
  ...
  if (sion_filedesc->collsize<=0) {
    return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"lsion_coll_fwrite: collsize=%d <= 0, returning ...\n", (int) sion_filedesc->collsize));
  }
  ...
  if(lsion_ensure_free_space(sid,size*nitems) != SION_SUCCESS) {
  ...
  if(sion_filedesc->rank == sion_filedesc->collector) {
    rc_own=_lsion_generic_collective_process_write(data,spec,sid);
  }
  ...
  if(!sion_apidesc->gather_execute_cb ) {
    return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"lsion_coll_fwrite: API %s not correctly initalized, collective I/O calls missing, aborting",sion_apidesc->name));
  }
  rc_cb=sion_apidesc->gather_execute_cb(data,spec,2, sion_filedesc->fsblksize,
					   ...
					   _lsion_generic_collective_process_write);
 
  /* set own position to end of own block written in this call */ 
  if(sion_filedesc->rank == sion_filedesc->collector) {
    _lsion_file_flush(sion_filedesc->fileptr);
    _lsion_file_set_position(sion_filedesc->fileptr,ownnewposition);sion_filedesc->currentpos=ownnewposition;
  }
  ...
  if(sion_filedesc->usebuddy ) {
    rc_buddy=_lsion_coll_fwrite_buddy(data, size, nitems, sid, sion_gendata);
  }
...
#define DFUNCTION "lsion_coll_fread"
size_t lsion_coll_fread( void *data, size_t size, size_t nitems, int sid) {
  ...
  if ( (sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR) || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"lsion_coll_fread: invalid sion_filedesc %d", sid));
  }
  DPRINTFP((4, DFUNCTION, _SION_DEFAULT_RANK, "enter usecoll=%d collector=%d collsize=%d (%d tasks, %d files)\n", 
	    sion_filedesc->usecoll, sion_filedesc->collector, sion_filedesc->collsize, sion_filedesc->ntasks,sion_filedesc->nfiles));
  ...
  /* no collective mode */
  if(!sion_filedesc->usecoll) {
    return(lsion_fread(data,size,nitems,sid));
  } 

  /* switch to buddy checkpointing, if enabled */
  if(sion_filedesc->usebuddy ) {
    return( _lsion_coll_fread_buddy(data, size, nitems, sid));
  }
  ...
  sion_filedesc->collcmdused=1;
    ...
    /* check collsize */
  if (sion_filedesc->collsize<=0) {
    return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"lsion_coll_fread: collsize=%d <= 0, returning ...\n", 
			    (int) sion_filedesc->collsize));
  }
  ...
  /* read own part */
  if(sion_filedesc->rank == sion_filedesc->collector) {
    rc_own=_lsion_generic_collective_process_read(data,spec,sid);
  }
  ownnewposition=sion_filedesc->currentpos;
  ...
  if(!sion_apidesc->execute_scatter_cb ) {
    return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,
			    "lsion_coll_fread: API %s not correctly initalized, collective I/O calls missing, aborting",sion_apidesc->name));
  }
...
#define DFUNCTION "_lsion_generic_collective_process_write"
int _lsion_generic_collective_process_write( const void *data, sion_int64 *spec, int sid ) {
  ...
  if ( (sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR) || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"_lsion_generic_collective_process_write: invalid sion_filedesc %d", sid));
  }
  ...
  if(sion_filedesc->currentpos!=destpos)   { 
    _lsion_file_flush(sion_filedesc->fileptr);
    _lsion_file_set_position(sion_filedesc->fileptr,destpos);sion_filedesc->currentpos=destpos;
  } 
  ...
  if ((err == cudaSuccess) && (!attrs.isManaged && (attrs.memoryType == cudaMemoryTypeDevice)) ) {
    ...
    while (bwrote < bytestowrite) {
      ...
      sion_int64 bwrote_ = _lsion_file_write(buffer, to_write, sion_filedesc->fileptr);
    ...
    bwrote = _lsion_file_write(data, bytestowrite, sion_filedesc->fileptr);
  }
#else
  bwrote = _lsion_file_write(data, bytestowrite, sion_filedesc->fileptr);
#endif
  if(bwrote != bytestowrite) {
    return(_lsion_errorprint(SION_STD_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_generic_collective_process_write: problems writing data ...\n"));
  }
  ...
  DPRINTFP((4, DFUNCTION, _SION_DEFAULT_RANK, "collector wrote data block (bwrote=%d) of size %ld new pos %lld, %lld\n", 
	    bwrote, (long) bytestowrite, (long long) sion_filedesc->currentpos, (long long) _lsion_file_get_position(sion_filedesc->fileptr)));
...
#define DFUNCTION "_lsion_generic_collective_process_read"
int _lsion_generic_collective_process_read( void *data, sion_int64 *spec, int sid ) {
  ...
  if ( (sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR) || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"_lsion_generic_collective_process_read: invalid sion_filedesc %d", sid));
  }
  DPRINTFP((4, DFUNCTION, _SION_DEFAULT_RANK, "enter spec[0]=%d spec[1]=%d sid=%d\n", (int) spec[0],(int) spec[1],sid));
  ...
  if(sion_filedesc->currentpos!=destpos)   { 
    if(sion_filedesc->fileptr!=NULL) {
      _lsion_file_flush(sion_filedesc->fileptr);
      _lsion_file_set_position(sion_filedesc->fileptr,destpos);
    }
  ...
  if ((err == cudaSuccess) && (!attrs.isManaged && (attrs.memoryType == cudaMemoryTypeDevice)) ) {
    ...
    bread = _lsion_file_read(data, bytestoread, sion_filedesc->fileptr);
  }
#else
  bread = _lsion_file_read(data, bytestoread, sion_filedesc->fileptr);
#endif
  if(bread != bytestoread) {
    return(_lsion_errorprint(SION_STD_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_generic_collective_process_read: problems reading data ...\n"));
  }
  ...
  DPRINTFP((4, DFUNCTION, _SION_DEFAULT_RANK, "collector read data block (bread=%d) of size %ld new pos %lld, %lld\n", 
	    bread, (long) bytestoread, (long long) sion_filedesc->currentpos, (long long) _lsion_file_get_position(sion_filedesc->fileptr)));
 ...
 */
#define DFUNCTION "_lsion_coll_check_env"
int  _lsion_coll_check_env(_sion_filedesc *sion_filedesc) {
  ...
  cd = _lsion_getenv("SION_COLLDEBUG");
  if(cd) {
    sion_filedesc->colldebug=atoi(cd);
  }

  cs = _lsion_getenv("SION_COLLSIZE");
  cn = _lsion_getenv("SION_COLLNUM");
  if(cs) {
