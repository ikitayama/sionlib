 ...
 *          -1 if error occured
 */
#define DFUNCTION "_lsion_paropen_generic_buddy"
				...
				_sion_generic_gendata *sion_gendata )
{
  ...
  buddylevel = atoi(_lsion_flags_get(flags_store,"buddy")->val);
  ...
  if (buddies == NULL) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"cannot allocate buddies structure of size %lu (_sion_generic_buddy), aborting ...\n", 
  ...
  if (buddy_fn == NULL) {
    free(buddies);
    return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_generic_buddy: cannot allocate temporary memory of size %lu (buddy_fn), aborting ...\n", 
  ...
  if (flags_store->mask&_SION_FMODE_WRITE) {
    ...
    nfname=(sion_apidesc->get_multi_filename_cb?sion_apidesc->get_multi_filename_cb:_lsion_get_multi_filename)
    ...
    rc=_lsion_paropen_generic_one_file(sid, nfname, flags_store, prefix, 
    ...
    if ((rc<0) || (sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR) || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
      free(buddies);
      free(buddy_fn);
      return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_generic_buddy: invalid sion_filedesc %d", sid));
    }
    ...
    for(b=0;b<sion_filedesc->buddylevel;b++)  {
      ...
      rc=_lsion_buddy_map(sion_gendata,capability,b+1,&buddyptr->buddy_send,&buddyptr->buddy_coll);
	...
	DPRINTFP((1, DFUNCTION, sion_gendata->grank, "BUDDYO pass=%d [grank=%2d] op=%d\n",pass,*globalrank,
		  _lsion_buddy_bwsched(buddyptr->buddy_send.groupnum, sion_gendata->numfiles, pass))); 


	switch (_lsion_buddy_bwsched(buddyptr->buddy_send.groupnum, sion_gendata->numfiles, pass)) {
	case _SION_BW_SCHED_ACTIONA: 
	  ...
	  buddyptr->buddy_coll.sid = _lsion_newvcd(NULL,SION_FILEDESCRIPTOR);
	  rc=_lsion_paropen_generic_one_file(buddyptr->buddy_coll.sid, nfname, flags_store, prefix, 
	...
	case _SION_BW_SCHED_ACTIONB: 
	  ...
	  buddyptr->buddy_send.sid = _lsion_newvcd(NULL,SION_FILEDESCRIPTOR);
	  rc=_lsion_paropen_generic_one_file(buddyptr->buddy_send.sid, nfname, flags_store, prefix, 
    ...
    if(sion_gendata->grank==0) {
      ...
      if (sion_tmpintfield1 == NULL) {
        free(buddies);
        free(buddy_fn);
	return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_generic_buddy: cannot allocate temporary memory of size %lu (sion_tmpintfield), aborting ...\n",
    ...
    for(b=-1; ( (b<buddylevel) && (!readyflag));b++)  {
      ...
      nfname=(sion_apidesc->get_multi_filename_cb?sion_apidesc->get_multi_filename_cb:_lsion_get_multi_filename)(buddy_fn,0);
	...
	if(_lsion_file_stat_file2(nfname, apiflag)) {
	  filefound=1;
	...
	root=-1;
        if (sion_tmpintfield1 == NULL) {
          free(buddies);
          free(buddy_fn);
          return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_ABORT, DFUNCTION ": gatherr_cb returned sion_tmpintfield1 == NULL"));
        }
	...
	DPRINTFP((1, DFUNCTION, sion_gendata->grank, "after scan master: root for this file is %d #tasks=%d\n",root,sion_gendata->gsize));
      ...
      if(root!=-1) {
	_lsion_generic_buddy_get_and_distribute_info_from_file(sion_gendata,nfname,root, &filenumber, &numfiles, &lrank, &lsize);
	...
	/* check physical file */
	nfname=(sion_apidesc->get_multi_filename_cb?sion_apidesc->get_multi_filename_cb:_lsion_get_multi_filename)(buddy_fn,file);
	filefound=-1;
	if(sion_gendata->lrank==0) {
	  if(_lsion_file_stat_file2(nfname, apiflag)) {
	    filefound=lgroup; 	
	...
	if(root>=0) {
	  ...
	  if(!masterfile_found) {
	    ...
	    DPRINTFP((1, DFUNCTION, sion_gendata->grank, "BUDDY[%d] befre read globalranks file=%d of %d, lrank=%d of %d\n",
		      b, filenumber, numfiles, lrank, lsize));
	    _lsion_generic_buddy_get_and_distribute_info_from_one_file(sion_gendata, nfname, root, &filenumber, &numfiles, &lrank, &lsize);
	  ...
	  if (numreaderinthisfile>0) {
		...
		if (stepvector == NULL) {
		  return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_generic_buddy: cannot allocate temporary memory of size %lu (stepvector), aborting ...\n",
		...
		/*DPRINTFP((1, DFUNCTION, sion_gendata->grank, "BUDDY STEP assign1: task %d stepcount_reader_rank=%d stepcount_noreader_rank=%d %s\n",
		  task, stepcount_reader_rank, stepcount_noreader_rank,_lsion_buddy_role_to_str(sion_tmpintfield1[task])));*/
	    ...
	    if(myrole!=SION_ROLE_NONE) {
	      ...
	      if (buddy_info == NULL) {
                free(buddies);
		return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_generic_buddy: cannot allocate temporary memory of size %lu (buddy_info), aborting ...\n",
		...
		buddy_info->filelrank=-1;
	      ...
	      DPRINTFP((1, DFUNCTION, sion_gendata->grank, "create group #%d (step=%d, rank=%d, size=%d, collsize=%d, from=%2d, to=%2d) (bnum=%2d file# %2d) with role %s \n",
			...
			buddy_info->from_index, buddy_info->to_index, buddy_info->bnum, buddy_info->filenum, _lsion_buddy_role_to_str(buddy_info->myrole)));
    ...
    if(!readyflag) {
      ...
      free(buddies);
      free(buddy_fn);
      return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_generic_buddy: cannot open file, not all files available"));
    }
	...
	nfname=(sion_apidesc->get_multi_filename_cb?sion_apidesc->get_multi_filename_cb:_lsion_get_multi_filename)(buddy_fn,buddy_info->filenum);
	...
	if( (buddy_info->myrole == SION_ROLE_READER) 
	  ...
	  rc=_lsion_paropen_generic_one_file(buddy_info->sid, nfname, flags_store, prefix, 
	  ...
	  buddy_info->sid = _lsion_newvcd(NULL,SION_FILEDESCRIPTOR); /* create a new sid */
	  rc=_lsion_paropen_generic_one_file(buddy_info->sid, nfname, flags_store, prefix, 
					    &sion_gendata->numfiles, &buddy_info->filenum, 
    ...
    free(buddies);
    free(buddy_fn);
    return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_generic_buddy: unknown file mode"));
  }
 ...
 */
#define DFUNCTION "_lsion_parclose_generic_buddy"
int _lsion_parclose_generic_buddy(int sid,
  ...
  if ( (sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR) || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_parclose_generic_buddy: invalid sion_filedesc %d", sid));
  }
  /* sion_apidesc=sion_gendata->apidesc; */
  ...
  if (sion_filedesc->mode == SION_FILEMODE_WRITE) {
    ...
    for(b=0;b<sion_filedesc->buddylevel;b++)  {
      ...
      /* collect mapping */
      _lsion_generic_collect_mapping_buddy(buddyptr, sion_gendata, &b_mapping_size, &b_mapping);
	...
	case _SION_BW_SCHED_ACTIONA: 
	  ...
	  _lsion_parclose_generic(buddyptr->buddy_coll.sid, buddyptr->buddy_coll.rank, buddyptr->buddy_coll.size, 
	...
	case _SION_BW_SCHED_ACTIONB: 
	  ...
	  _lsion_parclose_generic(buddyptr->buddy_send.sid, buddyptr->buddy_send.rank, buddyptr->buddy_send.size, 
	...
	DPRINTFP((1, DFUNCTION, sion_gendata->grank, "call parallel close of %d files, sid=%d\n", 
		  sion_gendata->numfiles, sid));
	buddies->currentgroup=group;
	rc = _lsion_parclose_generic( buddy_info->sid, buddy_info->rank,buddy_info->size, -1, NULL, _SION_INTERNAL_FLAG_BUDDY_READ,
    ...
    return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_parclose_generic_buddy: unknown file mode"));
  }
 ...
 * @return  SION_SUCCESS if successful
 */
#define DFUNCTION "_lsion_coll_fwrite_buddy"
int _lsion_coll_fwrite_buddy(const void *data, 
  ...
  /* obtain pointers to internal data structures */
  if ( (sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR) || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_coll_fwrite_buddy: invalid sion_filedesc %d", sid));
  }
  sion_apidesc=sion_gendata->apidesc;
  buddies=sion_filedesc->buddies;
  ...
  for(b=0;b<sion_filedesc->buddylevel;b++)  {
    ...
    if ( (buddyptr->buddy_coll.sid<0) || (_lsion_vcdtype(buddyptr->buddy_coll.sid) != SION_FILEDESCRIPTOR) || !(sion_filedesc_coll = _lsion_vcdtovcon(buddyptr->buddy_coll.sid))) {
      return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_coll_fwrite_buddy: invalid sion_filedesc %d", buddyptr->buddy_coll.sid));
    }
    if ( (buddyptr->buddy_send.sid<0) || (_lsion_vcdtype(buddyptr->buddy_send.sid) != SION_FILEDESCRIPTOR) || !(sion_filedesc_send = _lsion_vcdtovcon(buddyptr->buddy_send.sid))) {
      return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_coll_fwrite_buddy: invalid sion_filedesc %d", buddyptr->buddy_send.sid));
    }
	...
	rc = sion_apidesc->gather_execute_cb(data,spec,2, sion_filedesc_coll->fsblksize,
					     ...
					     buddyptr->buddy_coll.sid, _lsion_generic_collective_process_write);
	if(sion_filedesc_coll->rank == collector) {
	  _lsion_file_flush(sion_filedesc_coll->fileptr);
	  _lsion_file_set_position(sion_filedesc_coll->fileptr,ownnewposition);sion_filedesc_coll->currentpos=ownnewposition;
	  DPRINTFP((1, DFUNCTION, sion_gendata->grank, "BUDDYw call parallel write COLL currentpos=%d\n",(int) sion_filedesc_coll->currentpos)); 
	}
	break;
      case _SION_BW_SCHED_ACTIONB: 
	...
	if(lsion_ensure_free_space(buddyptr->buddy_send.sid,size*nitems) != SION_SUCCESS) {
	  _lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"could not ensure free space for this block, returning %d ...\n", sid);
	...
	rc = sion_apidesc->gather_execute_cb(data,spec,2, sion_filedesc_send->fsblksize,
					     ...
					     buddyptr->buddy_send.sid, _lsion_generic_collective_process_write);
 ...
 * @return  SION_SUCCESS if successful
 */
#define DFUNCTION "_lsion_coll_fread_buddy"
int _lsion_coll_fread_buddy( void *data, size_t size, size_t nitems, int sid) {
  ...
  /* obtain pointers to internal data structures */
  if ( (sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR) || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"_lsion_coll_fread_buddy: invalid sion_filedesc %d", sid));
  }
  ...
  if (sion_filedesc->collsize<=0) {
    return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"_lsion_coll_fread_buddy: collsize=%d <= 0, returning ...\n", 
  ...
  for(step=0;step<buddies->numsteps;step++) {
    ...
    if(group!=-1) {
      ...
      if ( (buddy_info->sid<0) || (_lsion_vcdtype(buddy_info->sid) != SION_FILEDESCRIPTOR) || !(b_sion_filedesc = _lsion_vcdtovcon(buddy_info->sid))) {
	return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"_lsion_coll_fread_buddy: invalid sion_filedesc %d", buddy_info->sid));
      }
	...
	if(size*nitems>0) {
	  if(lsion_feof(buddy_info->sid)) {
	    _lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,"early eof found for this block, returning %d ...\n", buddy_info->sid);
	...
	rc_own=_lsion_generic_collective_process_read(data,spec,buddy_info->sid);
	...
	return(_lsion_errorprint(SION_SIZE_NOT_VALID,_SION_ERROR_RETURN,
				"_lsion_coll_fread_buddy: API %s not correctly initalized, collective I/O calls missing, aborting",sion_apidesc->name));
      }
      ...
      rc_cb=sion_apidesc->execute_scatter_cb(data,spec,2, b_sion_filedesc->fsblksize,
					     ...
					     _lsion_generic_collective_process_read);
      ...
      if( (buddy_info->myrole == SION_ROLE_COLLECTOR) 
	  || (buddy_info->myrole == SION_ROLE_COLLECTOR_READER)) {
	_lsion_file_flush(b_sion_filedesc->fileptr);
	_lsion_file_set_position(b_sion_filedesc->fileptr,ownnewposition);b_sion_filedesc->currentpos=ownnewposition;
      }
	...
	_lsion_file_set_position(b_sion_filedesc->fileptr,b_sion_filedesc->currentpos);
	}
      }
...
#define DFUNCTION "_lsion_buddy_map"
int _lsion_buddy_map(
  ...
  if(sion_gendata->grank==0) {
    ...
    if (tasktogroup == NULL) {
      return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_buddy_map: cannot allocate temporary memory of size %lu (tasktogroup), aborting ...\n",
			      (unsigned long) tmpsize * sizeof(sion_int32)));
    }
    ...
    if (tmpintfield1 == NULL) {
      free(tasktogroup);
      return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_buddy_map: cannot allocate temporary memory of size %lu (tmpintfield1), aborting ...\n",
			      (unsigned long) tmpsize * sizeof(sion_int32)));
    }
    ...
    if (size_per_group == NULL) {
      free(tasktogroup);
      free(tmpintfield1);
      return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_buddy_map: cannot allocate temporary memory of size %lu (size_per_group), aborting ...\n",
  ...
  if (group_map_orig_to_new == NULL) {
    ...
    free(tmpintfield1);
    return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_buddy_map: cannot allocate temporary memory of size %lu (group_map_orig_to_new), aborting ...\n",
			    (unsigned long) sion_gendata->numfiles * sizeof(int)));
  }
  ...
  if (group_map_new_to_orig == NULL) {
    ...
    free(tasktogroup);
    free(tmpintfield1);
    return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_buddy_map: cannot allocate temporary memory of size %lu (group_map_new_to_orig), aborting ...\n",
			    (unsigned long) sion_gendata->numfiles * sizeof(int)));
  }
  ...
  for(pass=1;pass<=_SION_BW_SCHED_NUM_PASSES;pass++) {
    DPRINTFP((1, DFUNCTION, sion_gendata->grank, "pass=%d [grank=%2d] op=%d\n",pass,sion_gendata->grank,
	      _lsion_buddy_bwsched(mynewgroupnr, sion_gendata->numfiles, pass))); 
    
    switch (_lsion_buddy_bwsched(mynewgroupnr, sion_gendata->numfiles, pass)) {
    case _SION_BW_SCHED_ACTIONA: 
...
#undef DFUNCTION
  ...
  /* pass=1..3 */
#define DFUNCTION "_lsion_buddy_bwsched"
  ...
  *mapping = NULL;  *mapping_size = 0;
  ...
  if((buddyptr->buddy_coll.groupnum==0) && (buddyptr->buddy_coll.rank==0)) {
    ...
    *mapping = (sion_int32 *) malloc(*mapping_size * 2 * sizeof(sion_int32));
    if (*mapping == NULL) {
      return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_generic_collect_mapping_buddy: Cannot allocate memory for mapping"));
    }
  } 
  ...
  if(sion_gendata->grank==0) {
    ...
    if (receivemap == NULL) {
      return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_generic_collect_mapping_buddy: Cannot allocate memory for receivemap"));
    }
  }
...
#define DFUNCTION "_lsion_generic_buddy_get_and_distribute_info_from_file"
int _lsion_generic_buddy_get_and_distribute_info_from_file(  _sion_generic_gendata *sion_gendata, char *fname, int root,
  ...
  if(sion_gendata->grank == root) {
    ...
    if(sid>=0) {
      ...
      rc=lsion_get_mapping(sid,&mapping_size,&mapping,numfiles);
  ...
  if((sion_gendata->grank == root) && (*numfiles>1)) {
    ...
    if(mapping_size!=sion_gendata->gsize) {
      return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,
			      "_lsion_generic_buddy_get_and_distribute_info_from_file: Incorrect sum of ntasks of files %d <> %d\n", 
			      mapping_size, sion_gendata->gsize));
    }
  }
  ...
  if(*numfiles<0) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,
			    "_lsion_generic_buddy_get_and_distribute_info_from_file: could not get numfiles from sion file\n"));
  }
...
#define DFUNCTION "_lsion_generic_buddy_get_and_distribute_info_from_one_file"
int _lsion_generic_buddy_get_and_distribute_info_from_one_file(  _sion_generic_gendata *sion_gendata, char *fname, int root,
  ...
  if(sion_gendata->grank == root) {
    ...
    if (sion_filedesc == NULL) {
      return(_lsion_errorprint(SION_ID_UNDEF,SION_ID_UNDEF,
			      "lsion_open: cannot allocate filedescriptor structure of size %lu (sion_filedesc), aborting ...\n", 
			      (unsigned long) sizeof(sion_filedesc)));
    }
    _lsion_init_filedesc(sion_filedesc);
    
    /* open file */
    sion_fileptr = _lsion_file_open(fname,SION_FILE_FLAG_POSIX|SION_FILE_FLAG_READ,0);
    if (!sion_fileptr) {
    ...
    mapping = (sion_int32 *) malloc(mapping_size * 2 * sizeof(sion_int32));
    if (mapping == NULL) {
	return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_generic_buddy_get_and_distribute_info_from_one_file: cannot allocate temporary memory of size %lu (mapping), aborting ...\n",
  ...
  if(file_numfiles!=*numfiles) {
    ...
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,
			    "_lsion_generic_buddy_get_and_distribute_info_from_one_file: Incorrect number of files %d <> %d\n", 
