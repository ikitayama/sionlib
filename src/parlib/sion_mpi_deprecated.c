...
size_t lsion_coll_fwrite_mpi(const void *data, size_t size, size_t nitems, int sid) {
  return(lsion_coll_fwrite(data,size,nitems,sid));
}


/* wrapper for old SIONlib MPI function */
size_t lsion_coll_fread_mpi(void *data, size_t size, size_t nitems, int sid) {
  return(lsion_coll_fread(data,size,nitems,sid));
}
...
{
  *numFiles=0;
  return(lsion_paropen_mpi(fname,file_mode,numFiles,gComm,&lComm,chunksize,fsblksize,globalrank,fileptr,&newfname));
}
...
int lsion_startof_transaction_mpi(   int      sid  ) {
  ...
  if ( (sid<0) || (_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR) || !(sion_filedesc = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint_mpi(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"lsion_parclose_mpi: invalid sion_filedesc %d", sid));
  }
  ...
  t = _lsion_getenv("SION_SERBLOCKSIZE");
  ...
  DPRINTFP((1, "lsion_startof_transaction_mpi", grank, "enter start of transaction of sid %d blocksize=%d lrank=%d\n", sid,blocksize,lrank));

  lcommgroup->ts=_lsion_get_time();

  if(lrank>blocksize) {
    if(sion_filedesc->filenumber==0)    fprintf(stderr,"lsion_startof_transaction_mpi ts=%8.4fs on grank=%5d lrank=%3d waiting for message from %5d\n",_lsion_get_time()-lcommgroup->ts,
						grank,lrank,lrank-blocksize);
    MPI_Recv(&step, 1, MPI_INT, lrank-blocksize, 1430, lComm, &status);
  }

  lcommgroup->step=step;
  if(sion_filedesc->filenumber==0)  fprintf(stderr,"lsion_startof_transaction_mpi ts=%8.4fs on grank=%5d lrank=%3d starting transaction (blocksize=%d, step=%d)\n",_lsion_get_time()-lcommgroup->ts,
					    grank,lrank,blocksize,step);
  DPRINTFP((1, "lsion_startof_transaction_mpi", grank, "leave start of transaction of sid %d\n", sid));
...
int lsion_endof_transaction_mpi  (   int      sid  ) {
  ...
  DPRINTFP((1, "lsion_endof_transaction_mpi", grank, "enter end of transaction of sid %d blocksize=%d lrank=%d\n", sid,blocksize,lrank));


  if(lrank+blocksize<lsize) {
    if(sion_filedesc->filenumber==0) fprintf(stderr,"lsion_endof_transaction_mpi   ts=%8.4fs on grank=%5d lrank=%3d sending step %2d  to %5d\n",_lsion_get_time()-lcommgroup->ts,
					     grank,lrank,step,lrank+blocksize);
    MPI_Send(&step, 1, MPI_INT, lrank+blocksize, 1430, lComm);
  }

  if(sion_filedesc->filenumber==0) fprintf(stderr,"lsion_endof_transaction_mpi   ts=%8.4fs on grank=%5d lrank=%3d end of transaction step=%2d\n",_lsion_get_time()-lcommgroup->ts,
					   grank,lrank,step);

  DPRINTFP((1, "lsion_endof_transaction_mpi", grank, "leave end of transaction of sid %d\n", sid));
