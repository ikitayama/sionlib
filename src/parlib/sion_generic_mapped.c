...
int _lsion_paropen_mapped_generic(
				 int sid,
				 char  *fname,
  ...
  DPRINTFP((2, "_lsion_paropen_mapped_generic", rank, "enter parallel open  sid=%d\n", sid));
  
  sion_filedesc_master = _lsion_alloc_filedesc();
  if (sion_filedesc_master == NULL) {
    return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_mapped_generic: cannot allocate filedescriptor structure of size %lu (sion_filedesc_master), aborting ...\n", 
                            (unsigned long) sizeof(sion_filedesc_master)));
  }
  
  _lsion_init_filedesc(sion_filedesc_master);
  ...
  if (file_mode_flags&_SION_FMODE_WRITE) {
    ...
    DPRINTFP((32, "_lsion_paropen_mapped_generic", rank, " starting open for write #tasks=%d\n", ntasks));


    for(ltask=0;ltask<*nlocaltasks;ltask++) {
      DPRINTFP((128, "_lsion_paropen_mapped_generic", rank, "input: %2d: f=%d gtask=%2d ltask=%d  --> %d\n", ltask, 
    ...
    _lsion_reassignvcd(sid,sion_filedesc_master, SION_FILEDESCRIPTOR);
    ...
    if (sion_filedesc_master->multifiles == NULL) {
      return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"cannot allocate filedescriptor structure vector of size %lu (chunksizes), aborting ...\n", 
    ...
    */   
    ...
    if (lfilecounter == NULL) {
      return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_mapped_generic: cannot allocate temporary memory of size %lu (lfilecounter), aborting ...\n",
                              (unsigned long) *numFiles * sizeof(int)));
    }
    lfilemanager = (int *) malloc(*numFiles * sizeof(int));
    if (lfilemanager == NULL) {
      free(lfilecounter);
      return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_mapped_generic: cannot allocate temporary memory of size %lu (lfilemanager), aborting ...\n",
                              (unsigned long) *numFiles * sizeof(int)));
    }
    ...
    if (sion_count == NULL) {
      free(lfilemanager);
      free(lfilecounter);
      return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_mapped_generic: cannot allocate temporary memory of size %lu (sion_count), aborting ...\n",
    ...
    for(ltask=0;ltask<*nlocaltasks;ltask++) {
      ...
      if((filenr<0) || (filenr>=*numFiles)){
        ...
        free(lfilemanager);
        free(lfilecounter);
        return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_mapped_generic: on task %d: wrong file number index (%d) in mapping for task %d, aborting ...\n",
                                rank,filenr,ltask));
      }
      lfilecounter[filenr]++;
    ...
    _lsion_keyval_check_env(sion_filedesc_master, file_mode_flags);

    /* LOOP over files */
    for(filenr=0;filenr<*numFiles;filenr++) {
      
      DPRINTFP((4, "_lsion_paropen_mapped_generic", rank, " starting init of SION #%d of %d (%d local tasks) \n", filenr,*numFiles,lfilecounter[filenr]));
      ...
      if(root<0){
        return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_mapped_generic: on task %d: local rank 0 of file number index (%d) will not used by any tasks, aborting ...\n",
                                rank,filenr));
      }

      DPRINTFP((32, "_lsion_paropen_mapped_generic", rank, "  file #%d will be managed by task %d \n", filenr,root));

      /* allocate and initialise internal data structure with default values (NULL and -1) */
      sion_filedesc_sub = _lsion_alloc_filedesc();
      if (sion_filedesc_sub == NULL) {
        return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_mapped_generic: cannot allocate filedescriptor structure of size %lu (sion_filedesc), aborting ...\n", 
                                (unsigned long) sizeof(sion_filedesc_sub)));
      }
      sion_filedesc_master->multifiles[filenr]=sion_filedesc_sub;

      _lsion_init_filedesc(sion_filedesc_sub);
      
      sion_filedesc_sub->fname     = (sion_gendata->apidesc->get_multi_filename_cb?sion_gendata->apidesc->get_multi_filename_cb:_lsion_get_multi_filename)(fname, filenr); /* Set the filename */
      if(rank == root ){
      ...
      sion_filedesc_sub->endianness = _lsion_get_endianness_with_flags(file_mode_flags); /* Endianness */
      ...
      _lsion_keyval_check_env(sion_filedesc_sub, file_mode_flags);
      
      /* allocate memory for storing MAXCHUNKS chunksize infos in internal structure */
      _lsion_realloc_filedesc_blocklist(sion_filedesc_sub, MAXCHUNKS);
      ...
      */
      ...
      DPRINTFP((32, "_lsion_paropen_mapped_generic", rank, "mapping size increased to %d\n",sion_filedesc_master->mapping_size));

      /* root stores in sion_count the number of local task of this file for each task */

      DPRINTFP((32, "_lsion_paropen_mapped_generic", rank, "  file #%d (%s) will store chunks from %d tasks (%d local) \n", 
      ...
      if(rank == root) {
        DPRINTFP((32, "_lsion_paropen_mapped_generic", rank, " create file #%d (%s) \n", filenr,sion_filedesc_sub->fname));
        sion_fileptr = _lsion_file_open(sion_filedesc_sub->fname,apiflag|SION_FILE_FLAG_WRITE|SION_FILE_FLAG_CREATE,0);
        if (!sion_fileptr) {
          return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"sion_paropen_generic: cannot open %s for writing, aborting ...\n", fname));
        }
        if(sion_filedesc_sub->fsblksize<=-1) {
          /* check with fstat fsblksize */
          new_fsblocksize=(sion_int64) _lsion_file_get_opt_blksize(sion_fileptr);
          if((new_fsblocksize<0) || (new_fsblocksize>SION_MAX_FSBLOCKSIZE)) new_fsblocksize=SION_DEFAULT_FSBLOCKSIZE;
          DPRINTFP((32, "_lsion_paropen_mapped_generic", rank, "  found new_fsblksize of %d\n", (int) new_fsblocksize));
        }
	if (sion_filedesc_sub->usebuffer) {
	  _lsion_buffer_flush(sion_filedesc_sub);
	}
        _lsion_file_close(sion_fileptr);
      }
      ...
      if(sion_filedesc_sub->fsblksize==-1) {
        ...
        DPRINTFP((32, "_lsion_paropen_mapped_generic", rank, "  setting fsblksize to %d\n", (int) new_fsblocksize));
      }
      ...
      if (sion_tmpintfield2 == NULL) {
        return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_mapped_generic: cannot allocate temporary memory of size %lu (sion_tmpintfield2), aborting ...\n",
      ...
      if (rank == root) {
        ...
        if (sion_tmpintfield3 == NULL) {
          ...
          return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_mapped_generic: cannot allocate temporary memory of size %lu (sion_tmpintfield3), aborting ...\n",
                                  (unsigned long) tmpsize * sizeof(sion_int64)));
        }
      ...
      /*      for(ltask=0;ltask<3*sion_filedesc_sub->nlocaltasks;ltask++) {
              DPRINTFP((64, "_lsion_paropen_mapped_generic", rank, "  debug in %d --> %d\n", ltask, (int) sion_tmpintfield2[ltask] ));
              }*/
      ...
      /*      if (rank == root) {
              ...
              DPRINTFP((64, "_lsion_paropen_mapped_generic", rank, "  debug out %d --> %d\n", ltask, (int) sion_tmpintfield3[ltask] ));
              }
      ...
      if (rank == root) {
        ...
        _lsion_alloc_filedesc_arrays(sion_filedesc_sub);
        ...
        for(ltask=0;ltask<sion_filedesc_sub->ntotaltasksinfile;ltask++) {
          ...
          DPRINTFP((64, "_lsion_paropen_mapped_generic", rank, " sort in ltask=%d --> lrank=%d\n", ltask,lrank));
        ...
        /* calculate startpointers */
        _lsion_calculate_startpointers(sion_filedesc_sub);
	_lsion_print_filedesc(sion_filedesc_sub, 512, "start of paropen_mapped", 1);
      }

      /* open file on all ranks */
      /*                                                                      */ DPRINTFTS(rank, "before open");
      sion_fileptr = _lsion_file_open(sion_filedesc_sub->fname,apiflag|SION_FILE_FLAG_WRITE,0);
      if (!sion_fileptr) {
        return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"sion_paropen_generic: cannot open %s for reading, aborting ...\n", fname));
      }
      sion_gendata->apidesc->barrier_cb(sion_gendata->comm_data_global);
      ...
      DPRINTFP((64, "_lsion_paropen_mapped_generic", rank, " store for file %s fileprt=%x\n",sion_filedesc_sub->fname, sion_fileptr ));
      ...
      if (rank == root) {
        ...
        _lsion_write_header(sion_filedesc_sub);
        /*                                                                      */ DPRINTFTS(rank, "after writeh");
        
        /* needed for writing pointer to var part of metadata at the end of the file */
        sion_filedesc_sub->end_of_header = _lsion_file_get_position(sion_filedesc_sub->fileptr);
        ...
        _lsion_file_flush(sion_filedesc_sub->fileptr);
        _lsion_file_set_position(sion_filedesc_sub->fileptr, lstartpointer);
        /*                                                                      */ DPRINTFTS(rank, "after setp(0)");
        
        /* free on root some of the large fields */
        _lsion_free_filedesc_all_globalranks(sion_filedesc_sub);
        
        /* and realloc with less memory */
        sion_filedesc_sub->ntasks=sion_filedesc_sub->nlocaltasksinfile;
        _lsion_alloc_filedesc_all_globalranks(sion_filedesc_sub);
        _lsion_alloc_filedesc_all_localranks(sion_filedesc_sub);
	...
	_lsion_alloc_filedesc_all_keyvalptr(sion_filedesc_sub);
      }
      ...
      if (rank == root) {
        ...
        for(ltask=0;ltask<sion_filedesc_sub->ntotaltasksinfile;ltask++) {
          ...
          DPRINTFP((64, "_lsion_paropen_mapped_generic", rank, " sort out ltask=%d --> lrank=%d startptr=%ld chunksize=%ld\n",
      ...
      if (rank == root) {
        ...
        _lsion_free_filedesc_all_startpointers(sion_filedesc_sub);
        _lsion_free_filedesc_all_chunksizes(sion_filedesc_sub);
        /* and realloc with less memory */
        _lsion_alloc_filedesc_all_startpointers(sion_filedesc_sub);
        _lsion_alloc_filedesc_all_chunksizes(sion_filedesc_sub);
      }
      ...
      for(ltask=0;ltask<sion_filedesc_sub->nlocaltasksinfile;ltask++) {
        ...
        DPRINTFP((64, "_lsion_paropen_mapped_generic", rank, " get startpointer[%d] --> %ld\n", ltask, sion_filedesc_sub->all_startpointers[ltask]));
      }
      ...
      DPRINTFP((64, "_lsion_paropen_mapped_generic", rank, " allocating block arrays of size %d\n", sion_filedesc_sub->ntasks));
      _lsion_alloc_filedesc_block_arrays(sion_filedesc_sub);
      for (ltask = 0; ltask < sion_filedesc_sub->nlocaltasksinfile; ltask++) {
        sion_filedesc_sub->all_blockcount[ltask] = 1;
        sion_filedesc_sub->all_currentpos[ltask] = sion_filedesc_sub->all_startpointers[ltask];
	DPRINTFP((64, "_lsion_paropen_mapped_generic", rank, " set all_currentpos[%d]=%d\n",ltask,sion_filedesc_sub->all_currentpos[ltask] ));
      ...
      _lsion_file_purge(sion_fileptr);
      _lsion_file_set_position(sion_fileptr, sion_filedesc_sub->currentpos);
      ...
      DPRINTFP((4, "_lsion_paropen_mapped_generic", rank, " ending   init of SION #%d of %d \n", filenr,*numFiles));
	...
	lrank=0;
      ...
      if((sion_filedesc_sub) && (lrank>=0) && (lfile>=0)) {
        _lsion_realloc_filedesc_blocklist(sion_filedesc_master, MAXCHUNKS);
	...
	sion_filedesc_master->ntotaltasksinfile = sion_filedesc_master->mapping_size;
        ...
        /* set position */
        _lsion_file_flush(sion_filedesc_master->fileptr);

        DPRINTFP((32,"sion_paropen_mapped_generic:",rank,"set startpointer to %d\n",(int) sion_filedesc_master->currentpos));
        _lsion_file_set_position(sion_filedesc_master->fileptr, sion_filedesc_master->currentpos);
    ...
    _lsion_print_filedesc(sion_filedesc_master, 512, "_lsion_paropen_mapped_generic", 1);
    ...
    DPRINTFP((1, "_lsion_paropen_mapped_generic", rank, "read mode ntasks=%d\n",ntasks));

    for(ltask=0;ltask<*nlocaltasks;ltask++) {
      DPRINTFP((128, "_lsion_paropen_mapped_generic", rank, "input: %2d: gtask=%2d \n", ltask, (int) (*globalranks)[ltask]));
    }
    ...
    if(rank == 0) {
      ...
      DPRINTFP((1, "_lsion_paropen_mapped_generic", rank, "before open\n"));
      mapsid=_lsion_open_read(fname,_SION_FMODE_READ|_SION_FMODE_ANSI,_SION_READ_MASTER_ONLY_OF_MULTI_FILES,
			     &lntasks,&lnfiles,NULL,&lfsblksize,NULL,&lfileptr);
      if(mapsid>=0) {
	DPRINTFP((1, "_lsion_paropen_mapped_generic", rank, "after open\n"));
	rc=lsion_get_mapping(mapsid,&mapping_size,&mapping,numFiles);
	DPRINTFP((1, "_lsion_paropen_mapped_generic", rank, "sion file %d files #tasks=%d rc=%d\n", *numFiles,mapping_size,rc));
	...
	sion_filedesc_master->keyvalmode=lsion_get_keyval_mode(mapsid);
	...
	if(*numFiles>1) {
	  ...
	  for(task=0;task<mapping_size;task++) {
	    if(mapping[task*2+1]==0) {
	      ...
	      DPRINTFP((32, "_lsion_paropen_mapped_generic", rank, " manager of file %d is grank=%d\n",mapping[task*2+0],task));
	    }
	  }
	...
	*numFiles=-1;
        free(sion_count);
	return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_mapped_generic: cannot find file %s , aborting ...\n",fname));
      }
    }
    ...
    DPRINTFP((32, "_lsion_paropen_mapped_generic", rank, " mapping_size is=%d\n",sion_filedesc_master->mapping_size));

    /* and close file directly */
    if(rank == 0) {
      if (mapsid>=0) _lsion_close_sid(mapsid);       /* frees also mapping vector */
    }
	...
	if(endpos<sion_filedesc_master->mapping_size) endpos=sion_filedesc_master->mapping_size;
      ...
      *nlocaltasks=endpos-startpos;
      ...
      if (globalranks == NULL) {
        free(lfilemanager);
        return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"pre-compute distribution: cannot allocate globalranks, not pointer parameter given, aborting ...\n"));
      }
      if (mapping_filenrs == NULL) {
        free(lfilemanager);
        return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"pre-compute distribution: cannot allocate mapping_filenrs, not pointer parameter given, aborting ...\n"));
      }
      if (mapping_lranks == NULL) {
        free(lfilemanager);
        return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"pre-compute distribution: cannot allocate mapping_lranks, not pointer parameter given, aborting ...\n"));
      }
      if ((*globalranks) != NULL) {
        free(lfilemanager);
        return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"pre-compute distribution: cannot allocate globalranks, memory allready allocated (unknown size), aborting ...\n"));
      }
      if ((*mapping_filenrs) != NULL) {
        free(lfilemanager);
        return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"pre-compute distribution: cannot allocate mapping_filenrs, memory allready allocated (unknown size), aborting ...\n"));
      }
      if ((*mapping_lranks) != NULL) {
        free(lfilemanager);
        return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"pre-compute distribution: cannot allocate mapping_lranks, memory allready allocated (unknown size), aborting ...\n"));
      }
      ...
      *globalranks = (sion_int32 *) malloc(*nlocaltasks * sizeof(sion_int32));
      if (*globalranks == NULL) {
        free(lfilemanager);
	return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"cannot allocate memory of size %lu (globalranks), aborting ...\n", (unsigned long) sizeof(sion_int32)));
      }
    ...
    DPRINTFP((32, "_lsion_paropen_mapped_generic", rank, " keyvalmode is=%d\n",sion_filedesc_master->keyvalmode));
    ...
    for(filenr=0;filenr<sion_filedesc_master->nfiles;filenr++) {
      ...
      DPRINTFP((4, "_lsion_paropen_mapped_generic", rank, " starting init of SION #%d of %d\n", filenr,*numFiles));
	...
	if (rank == 0) {
      ...
      sion_gendata->apidesc->bcastr_cb(&helpint64, sion_gendata->comm_data_global, _SION_INT64, 1, 0);  
      root=helpint64;
      DPRINTFP((32, "_lsion_paropen_mapped_generic", rank, " manager of file %d is rank=%d\n",filenr,root));
      ...
      DPRINTFP((32, "_lsion_paropen_mapped_generic", rank, " starting open for read on file %s\n", sion_filedesc_sub->fname));
      /*                                                                      */ DPRINTFTS(rank, "before open"); 
      sion_filedesc_sub->fileptr=NULL;
     if (rank == root) {
	sion_fileptr = _lsion_file_open(sion_filedesc_sub->fname,apiflag|SION_FILE_FLAG_READ,0);
	if (!sion_fileptr) {
	  return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_mapped_generic: cannot open %s for reading, aborting ...\n", fname));
	}
	sion_filedesc_sub->fileptr = sion_fileptr;
      }
      sion_gendata->apidesc->barrier_cb(sion_gendata->comm_data_global); 
      ...
      if (rank == root) {
	rc = _lsion_read_header_fix_part(sion_filedesc_sub);
	if (rc!=SION_SUCCESS) {
	  return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_mapped_generic: cannot read header from file %s, aborting ...\n", fname));
	}
	DPRINTFP((32, "_lsion_paropen_mapped_generic", rank,
	...
	rc = _lsion_read_header_var_part(sion_filedesc_sub);
	if (rc != SION_SUCCESS) {
	  return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"_lsion_paropen_mapped_generic: cannot read header from file %s, aborting ...\n", fname));
	}
	      
	_lsion_coll_check_env(sion_filedesc_sub);
	if(sion_filedesc_sub->usecoll)  _lsion_alloc_filedesc_coll_arrays(sion_filedesc_sub);
	      
	/* collective */
	if (!sion_filedesc_sub->usecoll) _lsion_calculate_startpointers(sion_filedesc_sub);
	else                             _lsion_calculate_startpointers_collective(sion_filedesc_sub);
	...
	_lsion_keyval_check_env(sion_filedesc_sub, file_mode_flags);
      ...
      if (sion_filedesc_sub->maxusedchunks > sion_filedesc_sub->maxchunks)  _lsion_realloc_filedesc_blocklist(sion_filedesc_sub, sion_filedesc_sub->maxusedchunks);
	...
	DPRINTFP((64, "_lsion_paropen_mapped_generic", rank, "   scan for gtask=%d\n", (int) sion_tmpintfield2[task]));
	for(ltask=0;ltask<*nlocaltasks;ltask++) {
      ...
      DPRINTFP((32, "_lsion_paropen_mapped_generic", rank, " found %d globalranks for file %s\n", sion_filedesc_sub->nlocaltasksinfile, sion_filedesc_sub->fname));
	...
	if(sion_tmpintfield2[task]<0) {
	  ...
	  DPRINTFP((64, "_lsion_paropen_mapped_generic", rank, "   all_globalranks[%d]=%d all_localranks[%d]=%d\n", 
	...
	for(task=0;task<sion_filedesc_sub->ntotaltasksinfile;task++) sion_tmpintfield2[task]=sion_filedesc_sub->all_chunksizes[task];
      ...
      sion_gendata->apidesc->bcastr_cb(sion_tmpintfield2,   sion_gendata->comm_data_global, _SION_INT64, sion_filedesc_sub->ntotaltasksinfile, root);
      if(sion_filedesc_sub->state == SION_FILESTATE_PAROPENMAPPEDMANAGED) {
	_lsion_free_filedesc_all_chunksizes(sion_filedesc_sub);          /* free on root large field */
        _lsion_alloc_filedesc_all_chunksizes(sion_filedesc_sub);         /* and realloc with less memory */
      }
	...
	for(task=0;task<sion_filedesc_sub->ntotaltasksinfile;task++) sion_tmpintfield2[task]=sion_filedesc_sub->all_startpointers[task];
      ...
      sion_gendata->apidesc->bcastr_cb(sion_tmpintfield2,   sion_gendata->comm_data_global, _SION_INT64, sion_filedesc_sub->ntotaltasksinfile, root);
      if(sion_filedesc_sub->state == SION_FILESTATE_PAROPENMAPPEDMANAGED) {
        _lsion_free_filedesc_all_startpointers(sion_filedesc_sub);          /* free on root large field */
        _lsion_alloc_filedesc_all_startpointers(sion_filedesc_sub);         /* and realloc with less memory */
      }
      /* store in local field */
      for(ltask=0;ltask<sion_filedesc_sub->nlocaltasksinfile;ltask++) {
	task=sion_filedesc_sub->all_localranks[ltask];
	sion_filedesc_sub->all_startpointers[ltask]=sion_tmpintfield2[task];
      ...
      if(sion_filedesc_sub->state == SION_FILESTATE_PAROPENMAPPEDMANAGED) {
        sion_filedesc_sub->ntasks=sion_filedesc_sub->ntotaltasksinfile;
	_lsion_read_header_var_part_blockcount_to_field(sion_filedesc_sub, sion_filedesc_sub->ntotaltasksinfile, sion_tmpintfield2);
        sion_filedesc_sub->ntasks=sion_filedesc_sub->nlocaltasksinfile;
	for(task=0;task<sion_filedesc_sub->ntotaltasksinfile;task++) 
	  DPRINTFP((64, "_lsion_paropen_mapped_generic", rank, "   got from file blockcount[%d]=%d\n",task,(int) sion_tmpintfield2[task])); 
      }
      sion_gendata->apidesc->bcastr_cb(sion_tmpintfield2,   sion_gendata->comm_data_global, _SION_INT64, sion_filedesc_sub->ntotaltasksinfile, root);
      /* store in local field */
      for(ltask=0;ltask<sion_filedesc_sub->nlocaltasksinfile;ltask++) {
	...
	DPRINTFP((64, "_lsion_paropen_mapped_generic", rank, "   store blockcount task=%d -> ltask=%d cnt=%d\n",task,ltask,(int) sion_tmpintfield2[task])); 
	...
	if(sion_filedesc_sub->state == SION_FILESTATE_PAROPENMAPPEDMANAGED) {
	  sion_filedesc_sub->ntasks=sion_filedesc_sub->ntotaltasksinfile;
	  _lsion_read_header_var_part_nextblocksizes_to_field(sion_filedesc_sub, sion_filedesc_sub->ntotaltasksinfile, sion_tmpintfield2);
	  sion_filedesc_sub->ntasks=sion_filedesc_sub->nlocaltasksinfile;
	}
	sion_gendata->apidesc->bcastr_cb(sion_tmpintfield2,   sion_gendata->comm_data_global, _SION_INT64, sion_filedesc_sub->ntotaltasksinfile, root);
	/* store in local field */
	...
	sion_gendata->apidesc->bcastr_cb(sion_tmpintfield2,   sion_gendata->comm_data_global, _SION_INT64, sion_filedesc_sub->ntotaltasksinfile, root);
	if(sion_filedesc_sub->state == SION_FILESTATE_PAROPENMAPPEDMANAGED) {
	  _lsion_free_filedesc_coll_arrays(sion_filedesc_sub);          /* free on root large field */
	}
	_lsion_alloc_filedesc_all_chunksizes(sion_filedesc_sub);         /* and alloc with less memory on all tasks */

	/* store in local field */
	DPRINTFP((64, "_lsion_paropen_mapped_generic", rank, "   store collsize+collector for %d elements\n",sion_filedesc_sub->nlocaltasksinfile)); 
	for(ltask=0;ltask<sion_filedesc_sub->nlocaltasksinfile;ltask++) {
	  task=sion_filedesc_sub->all_localranks[ltask];
	...
	_lsion_realloc_filedesc_blocklist(sion_filedesc_master, sion_filedesc_master->maxusedchunks);
	for (blknum = 0; blknum < sion_filedesc_sub->all_blockcount[lrank]; blknum++) {
	  sion_filedesc_master->blocksizes[blknum] = sion_filedesc_sub->all_blocksizes[sion_filedesc_sub->nlocaltasksinfile * blknum + lrank];
	}
	...
	*fileptr=NULL;
	...
	if ((*chunksizes) == NULL) {
	  ...
	  if (helpptr_chunksize == NULL) {
	    return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"cannot allocate filedescriptor structure of size %lu (chunksizes), aborting ...\n", (unsigned long) sizeof(sion_int64)));
	  }
	...
	if ((*mapping_filenrs) == NULL) {
	  ...
	  if (helpptr_mapping_filenrs == NULL) {
	    return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"cannot allocate filedescriptor structure of size %lu (mapping_filenrs), aborting ...\n", (unsigned long) sizeof(sion_int32)));
	  }
	...
	if ((*mapping_lranks) == NULL) {
	  ...
	  if (helpptr_mapping_lranks == NULL) {
	    return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"cannot allocate filedescriptor structure of size %lu (mapping_lranks), aborting ...\n", (unsigned long) sizeof(sion_int32)));
	  }
    ...
    _lsion_print_filedesc(sion_filedesc_master, 512, "_lsion_paropen_mapped_generic", _SION_DEBUG_PRINT_ALL|_SION_DEBUG_PRINT_RECURSIVE);
    ...
    return(_lsion_errorprint(SION_ID_NOT_VALID,_SION_ERROR_RETURN,"lsion_paropen_multi_mpi: unknown file mode"));
  }
...
int _lsion_parclose_mapped_generic(   int    sid,
				     int    rank,
				     int    ntasks,
				     _sion_generic_gendata *sion_gendata ) {
  int rc=SION_SUCCESS;
  ...
  DPRINTFP((2, "_lsion_parclose_mapped_generic", rank, "enter parallel close  sid=%d\n", sid));

  if ((_lsion_vcdtype(sid) != SION_FILEDESCRIPTOR) || !(sion_filedesc_master = _lsion_vcdtovcon(sid))) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_parclose_generic: invalid sion_filedesc_master, aborting %d ...\n", sid));
  }

#ifdef SION_DEBUG
  {
    int numbytes, numfds;
    lsion_get_sizeof(sid, &numbytes, &numfds);
    DPRINTFP((2, "_lsion_parclose_mapped_generic", rank, "internal data size of sid %2d (%d bytes, %d fds) \n", sid,numbytes, numfds));
  }
#endif  
  ...
  if ((sion_filedesc_master->state != SION_FILESTATE_PAROPENMAPPED)
    ...
    return(_lsion_errorprint_on_rank(SION_NOT_SUCCESS,_SION_ERROR_RETURN,sion_filedesc_master->rank,"sion_parclose_mapped: invalid file open state (!PAROPENMAPPED), aborting %d ...", sid));
  }

  /* READ MODE: close files on all tasks  */
  if (sion_filedesc_master->mode == SION_FILEMODE_READ) {

    _lsion_print_filedesc(sion_filedesc_master, 512, "_lsion_parclose_mapped_generic", _SION_DEBUG_PRINT_ALL|_SION_DEBUG_PRINT_RECURSIVE);
	...
	_lsion_free_filedesc(sion_filedesc_sub);
      }
    }
    ...
    _lsion_free_filedesc(sion_filedesc_master);
    ...
    _lsion_flush_block(sion_filedesc_master);
    
    if (sion_filedesc_master->usebuffer) {
      _lsion_buffer_flush(sion_filedesc_master);
    }
    ...
    DPRINTFP((4, "_lsion_parclose_mapped_generic", rank, "on file %d: sub,maxchunk=%d master,maxchunk=%d \n", lfile,sion_filedesc_sub->maxchunks, sion_filedesc_master->maxchunks));
    if(sion_filedesc_sub->maxchunks  < sion_filedesc_master->maxchunks) {
      _lsion_realloc_filedesc_blocklist(sion_filedesc_sub, sion_filedesc_master->maxchunks);
    }

    /* store data of current rank on sub datastructure */
    DPRINTFP((4, "_lsion_parclose_mapped_generic", rank, "store current information lrank=%d lastchunknr=%d\n", lrank,sion_filedesc_sub->lastchunknr));
    ...
    for(filenr=0;filenr<sion_filedesc_master->nfiles;filenr++) {
      ...
      sion_filedesc_sub=sion_filedesc_master->multifiles[filenr];
      DPRINTFP((4, "_lsion_parclose_mapped_generic", rank, " starting close for file %d: %s \n", filenr,sion_filedesc_sub->fname));
	...
	_lsion_buffer_flush(sion_filedesc_sub);
      ...
      if(sion_filedesc_sub->state == SION_FILESTATE_PAROPENMAPPED) {
        DPRINTFP((32, "_lsion_parclose_mapped_generic", rank, ">parallel close (write mode, not managed)  call fclose on file %s (fileptr=%x)\n", sion_filedesc_sub->fname,sion_filedesc_sub->fileptr));
        _lsion_file_close(sion_filedesc_sub->fileptr);
        DPRINTFP((32, "_lsion_parclose_mapped_generic", rank, "<parallel close (write mode, not managed)  call fclose on file %s (fileptr=%x)\n", sion_filedesc_sub->fname,sion_filedesc_sub->fileptr));
        sion_filedesc_sub->fileptr = NULL;
        sion_filedesc_sub->state = SION_FILESTATE_CLOSE;
	...
	return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_parclose_generic: cannot allocate temporary memory of size %lu (sion_tmpintfield_send), aborting ...\n",
				(unsigned long) tmpsize * sizeof(sion_int64)));
      }
      DPRINTFP((4, "_lsion_parclose_mapped_generic", rank, " file %d: allocate send field for all local tasks of file (%d) --> %x\n", filenr, tmpsize, sion_tmpintfield_send));
      ...
      if(sion_filedesc_sub->state == SION_FILESTATE_PAROPENMAPPEDMANAGED) {
        ...
        if (sion_tmpintfield_lrank_recv == NULL) {
          return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_parclose_generic: cannot allocate temporary memory of size %lu (sion_tmpintfield_lrank_recv), aborting ...\n",
                                  (unsigned long) tmpsize * sizeof(sion_int64)));
        }
        ...
        if (sion_tmpintfield_data_recv == NULL) {
          ...
          return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_parclose_generic: cannot allocate temporary memory of size %lu (sion_tmpintfield_data_recv), aborting ...\n",
                                  (unsigned long) tmpsize * sizeof(sion_int64)));
        }
        ...
        if (sion_tmpintfield_data == NULL) {
          ...
          return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_lsion_parclose_generic: cannot allocate temporary memory of size %lu (sion_tmpintfield_data), aborting ...\n",
                                  (unsigned long) tmpsize * sizeof(sion_int64)));
        }
        DPRINTFP((4, "_lsion_parclose_mapped_generic", rank, " file %d: allocate fields for all tasks of file (%d)\n", filenr,tmpsize));
      ...
      DPRINTFP((4, "_lsion_parclose_mapped_generic", rank, " file %d: before call of gathervr_cb %x %x %x %d %d %d\n",
      ...
      for (ltask = 0; ltask < sion_filedesc_sub->nlocaltasksinfile; ltask++) {
	DPRINTFP((4, "_lsion_parclose_mapped_generic", rank, " send all_blockcount[%d]: %d\n", ltask, sion_filedesc_sub->all_blockcount[ltask]));
      ...
      sion_gendata->apidesc->bcastr_cb(&sion_filedesc_sub->maxusedchunks, sion_gendata->comm_data_global, _SION_INT32, 1, root);
      DPRINTFP((4, "_lsion_parclose_mapped_generic", rank, " file %d: maxusedchunks=%d\n", filenr,sion_filedesc_sub->maxusedchunks));
      ...
      if(sion_filedesc_sub->state == SION_FILESTATE_PAROPENMAPPEDMANAGED) {
        ...
        for(ltask=0;ltask<sion_filedesc_sub->ntotaltasksinfile;ltask++) {
          ...
          DPRINTFP((64, "_lsion_parclose_mapped_generic", rank, " sort in ltask=%d --> lrank=%d blkcount=%d\n",
        ...
        _lsion_write_header_var_info(sion_filedesc_sub);
        
        _lsion_write_header_var_part_blockcount_from_field(sion_filedesc_sub,sion_filedesc_sub->ntotaltasksinfile,sion_tmpintfield_data);
      }

      for (blknum = 0; blknum < sion_filedesc_sub->maxusedchunks; blknum++) {
   
        DPRINTFP((32, "_lsion_parclose_mapped_generic", rank, " collect blocksize step %d of %d\n", blknum+1,sion_filedesc_sub->maxusedchunks));
        
        /* collect number of written chunks (blocks) for each file task */
        DPRINTFP((32,"_lsion_parclose_mapped_generic:",rank,"sion_filedesc_sub->nlocaltasksinfile=%d\n",sion_filedesc_sub->nlocaltasksinfile));
        for (ltask = 0; ltask < sion_filedesc_sub->nlocaltasksinfile; ltask++) {
          sion_tmpintfield_send[ltask]=sion_filedesc_sub->all_blocksizes[sion_filedesc_sub->nlocaltasksinfile * blknum + ltask];
          DPRINTFP((32,"_lsion_parclose_mapped_generic:",rank,"sort in blksize[%d][%d]=%d\n",blknum, ltask, sion_tmpintfield_send[ltask]));
        }
        sion_gendata->apidesc->gathervr_cb(sion_tmpintfield_send, sion_tmpintfield_data_recv, sion_gendata->comm_data_global, _SION_INT64, sion_count, sion_filedesc_sub->nlocaltasksinfile, root);
        ...
        if(sion_filedesc_sub->state == SION_FILESTATE_PAROPENMAPPEDMANAGED) {
          ...
          for(ltask=0;ltask<sion_filedesc_sub->ntotaltasksinfile;ltask++) {
            lrank=(int) sion_tmpintfield_lrank_recv[ltask];
            sion_tmpintfield_data[lrank]=sion_tmpintfield_data_recv[ltask];
            DPRINTFP((32,"_lsion_parclose_mapped_generic:",rank,"prepare blksize[%d][%d]=%ld\n",blknum,lrank,sion_tmpintfield_data[lrank]));
          }
          
          _lsion_write_header_var_part_nextblocksizes_from_field(sion_filedesc_sub,sion_filedesc_sub->ntotaltasksinfile,sion_tmpintfield_data);
      ...
      DPRINTFP((4, "_lsion_parclose_mapped_generic", rank, " ending close for file %d: %s \n", filenr,sion_filedesc_sub->fname));

    } /* for */

    /* for mapping table */
    mappingroot=sion_filedesc_master->multifiles[0]->filemanagedbytask;
    DPRINTFP((32,"_lsion_parclose_mapped_generic:",rank,"mappingroot=%d rank=%d\n", mappingroot, rank ));
    if(rank==mappingroot) {
      DPRINTFP((32,"_lsion_parclose_mapped_generic:",rank,"allocate mapping of size %d\n",sion_filedesc_master->mapping_size));
      mapping = (sion_int32 *) malloc(sion_filedesc_master->mapping_size * 2 * sizeof(sion_int32));
      if (mapping == NULL) {
        free(sion_count);
	return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_ABORT,"_sion_parclose_generic_mapped: Cannot allocate memory for mapping"));
      }
    }
    ...
    for(filenr=0;filenr<sion_filedesc_master->nfiles;filenr++) {
      ...
      if (sion_tmpint32field_send == NULL) {
        free(mapping);
	return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_sion_parclose_generic_mapped: cannot allocate temporary memory of size %lu (sion_tmpintfield_send), aborting ...\n",
	...
	if (sion_tmpint32field_data_recv == NULL) {
	  return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"_sion_parclose_generic_mapped: cannot allocate temporary memory of size %lu (sion_tmpintfield_data_recv), aborting ...\n",
				  (unsigned long) tmpsize * sizeof(sion_int32)));
	}
	...
	for(ltask=0;ltask<sion_filedesc_sub->ntotaltasksinfile;ltask++) {
	  ...
	  DPRINTFP((32,"_lsion_parclose_mapped_generic:",rank,"store mapping[%d]=(%d,%d)\n",grank,filenr,lrank));
	...
	/* write mapping */
	if(filenr==0) {
	  DPRINTFP((32,"_lsion_parclose_mapped_generic:",rank,"mapping size is %d\n",sion_filedesc_master->mapping_size));
	  _lsion_write_header_var_part_mapping(sion_filedesc_sub, sion_filedesc_master->mapping_size, mapping);
	}

	DPRINTFP((32, "_lsion_parclose_mapped_generic", rank, " parallel close (write mode, managed)  call fclose on file %s (fileptr=%x)\n", sion_filedesc_sub->fname,sion_filedesc_sub->fileptr));
	_lsion_file_close(sion_filedesc_sub->fileptr);
	sion_filedesc_sub->fileptr = NULL;
      }
  ...
  DPRINTFP((2, "_lsion_parclose_mapped_generic", rank, "leave parallel close  sid=%d\n", sid));
    
  return (rc);
}

/*  END OF _lsion_parclose_generic */
