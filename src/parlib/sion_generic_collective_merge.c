...
/* callback functions */
int _lsion_generic_collective_process_write_merge( const void *data, sion_int64 *spec, int sid );


/* collective I/O */
#define DFUNCTION "_lsion_coll_fwrite_merge"
size_t _lsion_coll_fwrite_merge(const void *data, size_t size, size_t nitems, int sid) {
  ...
  if(sion_filedesc->rank == sion_filedesc->collector) {
    ...
    rc_own=_lsion_generic_collective_process_write_merge(data,spec,sid);
  } else {
  ...
  rc_cb=sion_apidesc->gather_execute_cb(data,spec,2, sion_filedesc->fsblksize,
					...
					_lsion_generic_collective_process_write_merge);
...
#define DFUNCTION "_lsion_generic_collective_process_write_merge"
int _lsion_generic_collective_process_write_merge( const void *data, sion_int64 *spec, int sid ) {
  _sion_filedesc        *sion_filedesc;
  ...
  if(lsion_ensure_free_space(sid,bytestowrite) != SION_SUCCESS) {
