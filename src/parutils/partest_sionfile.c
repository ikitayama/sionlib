...
int test_paropen_multi_mpi(char *filename,
  ...
  if(options->do_write) {
    ...
    if(options->use_posix) {
      strcat(file_mode, ",posix");
    } else {
      strcat(file_mode, ",ansi");
    }
    if (options->collmsa) {
      strcat(file_mode, ",collmsa");
    }
    sid = lsion_paropen_mpi(filename, file_mode, &options->numfiles, communicators->work, &communicators->local, 
			   &options->chunksize, &options->fsblksize, &globalrank, NULL, &newfname);
    using_hints=lsion_using_hints(sid);
    ...
    /* local is computed again by lsion_paropen_mpi if numfiles >=1 */
    /* TIMING                                   */ starttime = MPI_Wtime();
    MPI_Comm_size(communicators->local, &communicators->local_size);
    MPI_Comm_rank(communicators->local, &communicators->local_rank);
    
    if(options->serialize_blocknum==-2) lsion_startof_transaction_mpi(sid);
	...
	/* TIMING                                   */ timings[TIMINGS_WR_WRITE] += MPI_Wtime()-starttime;
      ...
      /* TIMING                                   */ starttime = MPI_Wtime();
      if(options->serialize_blocknum==-2) lsion_endof_transaction_mpi(sid);
      /* TIMING                                   */ timings[TIMINGS_WR_WRITE_SYNC] += MPI_Wtime()-starttime;
  ...
  if(options->do_read) {
    ...
    if (options->collectiveopenforread) {
      ...
      sid = lsion_paropen_mpi(filename,file_mode, &options->numfiles, communicators->workread, 
      ...
      sid = lsion_open_rank(filename, file_mode, &rchunksize, &rfsblksize, &communicators->workread_rank, NULL);
  ...
  *chunkcnt = 0;
  ...
  while (left > 0) {
    ...
    if(bytes_in_chunk>options->chunksize) {
      lsion_ensure_free_space(sid, bwrite);
      bytes_in_chunk=bwrite;
    }
    if(options->collectivewrite) {
      bwrote = lsion_coll_fwrite_mpi(localbuffer, 1, bwrite, sid);
    } else {
      bwrote = lsion_fwrite(localbuffer, 1, bwrite, sid);
    }
  ...
  *chunkcnt = 0;
  ...
  while ((left > 0) && (!myfeof)) {
    ...
    if(options->collectiveread) {
      bread  = lsion_coll_fread_mpi(localbuffer, 1, btoread, sid);
    } else {
      bread = lsion_fread(localbuffer, 1, btoread, sid);
    }
