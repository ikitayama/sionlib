...
int test_paropen_multi_ompi(char *filename,
  ...
  sid = lsion_paropen_ompi(filename, "bw", &options->numfiles, communicators->work, &communicators->local, &options->chunksize, &options->fsblksize, &globalrank, &fp, &newfname);
...
#pragma omp barrier
  ...
  if (options->collectiveopenforread) {
    ...
    sid = lsion_paropen_ompi(filename,
    ...
    sid = lsion_open_rank(filename, "br", &rchunksize, &rfsblksize, &communicators->workread_rank, &fp);
  }
  /*                                    */ opentime = MPI_Wtime() - starttime;
	...
	#pragma omp master
	  {
		  if (((options->debug && communicators->all_rank == 0)) || ((options->Debug && communicators->all_rank == communicators->all_size))) {
			...
			fprintf(stderr, "timings[%03d,t%03d] after read   position in file= %lld restinblock=%lld\n",
				communicators->all_rank,omp_get_thread_num(), lsion_get_position(sid), lsion_bytes_avail_in_block(sid));
		  }
	  }
#pragma omp barrier
	}
