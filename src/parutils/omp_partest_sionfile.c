...
int test_paropen_omp(char *filename,
  ...
  /*                                    */ starttime = MPI_Wtime();
  sid = lsion_paropen_omp(filename, "bw", &options->chunksize, &options->fsblksize, &globalrank, &fp, &newfname);
...
#pragma omp barrier
  ...
  if (options->collectiveopenforread) {
    ...
    sid = lsion_paropen_omp(filename,"br", &options->chunksize, &options->fsblksize, &globalrank, &fp, &newfname);
		...
		#pragma omp master
		  {
			  if (((options->debug && thread_num == 0)) || ((options->Debug && (thread_num == omp_get_num_threads() - 1)))) {
				...
				fprintf(stderr, "timings[t%03d] after read   position in file= %lld restinblock=%lld\n",
					thread_num, lsion_get_position(sid), lsion_bytes_avail_in_block(sid));
			  }
		  }
#pragma omp barrier
	}
