...
test_single_mpi(char *filename,
  ...
  if(options->do_write) {
    ...
    sion_fileptr = _lsion_file_open(fname,api_to_use|SION_FILE_FLAG_WRITE|SION_FILE_FLAG_CREATE,0);
    if (!sion_fileptr) {
    ...
    sion_fileptr = _lsion_file_open(fname,api_to_use|SION_FILE_FLAG_WRITE,0);
    if (!sion_fileptr) {
      fprintf(stderr, "cannot open %s for writing, aborting ...\n", fname);
      MPI_Abort(communicators->all, 1);
    }
	...
	while (left > 0) {
	  ...
	  bwrote = _lsion_file_write(localbuffer, bwrite, sion_fileptr);
  ...
  if(options->do_read) {
    ...
    sion_fileptr = _lsion_file_open(fname,api_to_use|SION_FILE_FLAG_READ,0);
    if (!sion_fileptr) {
      fprintf(stderr, "cannot open %s for writing, aborting ...\n", fname);
      MPI_Abort(communicators->all, 1);
    }
	...
	while (left > 0) {
	  ...
	  bread = _lsion_file_read(localbuffer, btoread, sion_fileptr);
