...
int main(int argc, char **argv)
{
  ...
  sid = lsion_open(infilename, "rb,posix", &nfiles, &ntasks, NULL, &fsblksize, NULL, NULL);
  ...
  for (rank = startrank; rank <= endrank; rank++) {
    ...
    for (blk = startblk; blk <= endblk; blk++) {
      ...
      lsion_seek(sid, rank, blk, 0);
      DPRINTFP((1, "sioncat", 0, "after lsion_seek sid=%d rank=%d blknum=%d fileposition=%lld\n",
                sid, rank, blk, lsion_get_position(sid)));
      ...
      left = sion_blocksizes[ntasks * blk + rank];
      if(use_outfile) {
        _lsion_file_write(localbuffer, left, outfp);
      } else {
