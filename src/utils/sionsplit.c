...
int main(int argc, char **argv)
{
  ...
  sid = lsion_open(infilename, "rb,posix", &ntasks, &nfiles, NULL, &fsblksize, NULL, NULL);
  ...
  printf("sionsplit: current endianness:        %s\n", (lsion_get_endianness())? "big" : "little");
  printf("sionsplit: file endianness:           %s\n", (lsion_get_file_endianness(sid)) ? "big" : "little");
  printf("sionsplit: fsblksize:                 %lu bytes (%6.2f MB)\n", (unsigned long) fsblksize, fsblksize / 1024.0 / 1024.0);

  lsion_get_locations(sid, &ntasks, &maxblocks, &globalskip, &start_of_varheader, &sion_localsizes, &sion_globalranks, &sion_blockcount,
                     &sion_blocksizes);
  ...
  if(sion_filedesc->keyvalmode==SION_KEYVAL_NONE) {
	...
	sprintf(outfilename, fnmask, rank);
      ...
      outfp = _lsion_file_open(outfilename,SION_FILE_FLAG_ANSI|SION_FILE_FLAG_WRITE|SION_FILE_FLAG_CREATE,0);
      if (outfp == NULL) {
	...
	lsion_seek(sid, rank, blknum, 0);
	DPRINTFP((1, "sionsplit", 0, "after lsion_seek sid=%d rank=%d blknum=%d fileposition=%lld\n", sid, rank, blknum, lsion_get_position(sid)));
	...
	while (left > 0) {
	  ...
	  bread = lsion_fread(localbuffer + bsumread, 1, left, sid);
	  left -= bread;
	  bsumread += bread;
	...
	left = sion_blocksizes[ntasks * blknum + rank];
	bwrote = _lsion_file_write(localbuffer, left, outfp);
	printf("sionsplit:                            %lld wrote of left=%lld \n", bwrote, left);
	
      }
      
      _lsion_file_close(outfp);
	...
	if (rank % 16 == 0) {
      ...
      lsion_seek(sid, rank, SION_CURRENT_BLK, SION_CURRENT_POS);
      
      lsion_key_full_scan(sid);
      
      /* reset iterator over keys */
      lsion_key_list_iterator_reset(sid);
      
      /* loop over key-value blocks */
      while(lsion_key_list_iterator_next(sid,&key)==SION_SUCCESS) {
	...
	while( (bread=lsion_fread_key(localbuffer,key,1,fsblksize,sid))>0 ) {
	  if(bread>0) {
	    bwrote = _lsion_file_write(localbuffer, bread, outfp);
	  }
	  if (verbose)
