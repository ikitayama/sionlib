...
int main(int argc, char **argv)
{
  ...
  chunksizes = NULL;
  globalranks = NULL;           /* will be allocated by lsion_open */

  sid = lsion_open(infilename, "rb,posix", &ntasks, &nfiles, &chunksizes, &fsblksize, &globalranks, NULL);
  ...
  lsion_get_locations(sid, &size, &blocks, &globalskip, &start_of_varheader, &sion_chunksizes, &sion_globalranks, &sion_blockcount,
                     &sion_blocksizes);
  ...
  /* preserve endianness */
  if (lsion_get_file_endianness(sid)) {
  ...
  if(sion_filedesc->keyvalmode==SION_KEYVAL_NONE) {
    ...
    outsid = lsion_open(outfilename, file_mode, &ntasks, &onfiles, &chunksizes, &fsblksize, &globalranks, NULL);
	...
	if (rank % 16 == 0) {
      ...
      lsion_seek(outsid, rank, SION_CURRENT_BLK, SION_CURRENT_POS);
	...
	bsumwrote = 0;
	while (left > 0) {
	  bwrote = lsion_fwrite(localbuffer + bsumwrote, 1, left, outsid);
	  if (bwrote == 0)
	    nullcount++;
	  else
	    nullcount = 0;
	...
	if (rank % 16 == 0) {
      ...
      lsion_fread_key_iterator_reset(sid);
      
      /* loop over key-value blocks */
      while(lsion_fread_key_iterator_next(sid,&key,&len)==SION_SUCCESS) {
	...
	/* read data of key */
	bread = lsion_fread_key(localbuffer, key, 1, len, sid);
	/* bread=len; */
	if (bread != len) {
	...
	/* write data of key */
	bwrote = lsion_fwrite_key(localbuffer, key, 1, len, outsid);
	/* bread=len; */
	if (bwrote != len) {
