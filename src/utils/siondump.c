...
int main(int argc, char **argv)
{
  ...
  sid = lsion_open(infilename, "rb,posix", &ntasks, &nfiles, NULL, &fsblksize, NULL, &fp);

  if (sid<0) {
    return(_lsion_errorprint(SION_NOT_SUCCESS,_SION_ERROR_RETURN,"could not open file, aborting %d ...\n", sid));
  }
  ...
  lsion_get_locations(sid, &size, &blocks, &globalskip, &start_of_varheader, &sion_chunksizes, &sion_globalranks,
  ...
  if(printisize) {
    ...
    lsion_get_sizeof(sid,&num_bytes, &num_fds);
  ...
  if (printkeyslist) {
	...
	while(lsion_fread_key_iterator_next(sid,&key,&value_size)==SION_SUCCESS) {
	  if (((i%5)==0) && (i>0)) printf("\n       :");
  ...
  if (sion_filedesc->nfiles > 1) {
    ...
    if(printmap) {
      lsion_get_mapping(sid,&mapping_size,&mapping,&nfiles);
      printf("  mapping: ");
  ...
  lsion_get_current_locations(sid, &size, &sion_currentpos, &sion_currentblocknr);
