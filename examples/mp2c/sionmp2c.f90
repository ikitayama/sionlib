...
subroutine mp2c_sion_ascii(filename, ofilename)
  ...
  call lfsion_open(trim(filename), filemoderb, n_proc, n_files, chunksize, fsblksize, globalranks(1), sid);
  if(sid < 0) then
  ...
  call lfsion_read(fieldheader8(1), sizeof(fieldheader8(1)), 3_8, sid, bread)
  ...
  do rank = 0, n_proc - 1
     call lfsion_seek(sid, rank, SION_CURRENT_BLK, SION_CURRENT_POS, ierr)
     ...
     do n8 = 1, n_part
        call lfsion_read(intbuffer, sizeof(intbuffer), 1_8, sid, bread)
        call lfsion_read(realbuffer(1), sizeof(realbuffer(1)), 6_8, sid, bread)
...
subroutine mp2c_ascii_sion(filename)
  ...
  call lfsion_open(trim(filename), filemodewb, int(n_proc), 1, chunksizes(1), fsblksize, globalranks(1), sid);
  ...
  bwrite = 8
  call lfsion_write(n_part, 1_8, bwrite, sid, bwrote)
  call lfsion_write(nt, 1_8, bwrite, sid, bwrote)
  call lfsion_write(n_proc, 1_8, bwrite, sid, bwrote)
  ...
  do rank = 0, int(n_proc) - 1
     ...
     do i = 1, nextparts
        ...
        bwrite = 8;   call lfsion_write(idx, 1_8, bwrite, sid, bwrote)
        bwrite = 8 * 3; call lfsion_write(r(1), 1_8, bwrite, sid, bwrote)
        bwrite = 8 * 3; call lfsion_write(v(1), 1_8, bwrite, sid, bwrote)
     enddo
  enddo
