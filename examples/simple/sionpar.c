...
int main(int argc, char** argv)
{
  ...
  /* write */
  sid = lsion_paropen_mpi(fname, "bw", &numFiles, gComm, &lComm, &chunksize,
                         &fsblksize, &globalrank, &fileptr, &newfname);
  left   = BUFSIZE;
  bwrote = lsion_fwrite(localbuffer, 1, left, sid);
  ...
  /* read */
  sid = lsion_paropen_mpi(fname, "br", &numFiles, gComm, &lComm, &chunksize,
                         &fsblksize, &globalrank, &fileptr, &newfname);
  while (!lsion_feof(sid)) {
    btoread = lsion_bytes_avail_in_chunk(sid);
