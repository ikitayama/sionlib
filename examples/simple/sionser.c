...
int main(int argc, char** argv)
{
  ...
  sid = lsion_open("serfile.sion", "wb", &ntasks, &nfiles, &chunksizes,
                  &fsblksize, &globalranks, NULL);

  bwrote = lsion_fwrite(buffer, 1, buffer_size, sid);
  ...
  /* write */
  sid = lsion_open("serfile.sion", "rb", &ntasks, &nfiles, &chunksizes,
                  &fsblksize, &globalranks, NULL);

  bavail = lsion_bytes_avail_in_chunk(sid);
  buffer = (char*)malloc(bavail);

  bread = lsion_fread(buffer, 1, bavail, sid);
