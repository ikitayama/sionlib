...
# defaults
MPIRUN="mpirun -np 4 --mca btl sm,self"
MPIRUNEXECPARM=""
...
(
  ...
  grep 'on MPI rank 0:' stdout.log > stdout_0.log
  grep 'on MPI rank 0:' stderr.log > stderr_0.log

  export LSION_DEBUG=siondump_${TESTNAME}

  ${SIONDUMP} -a -m -l mandelbrot.sion 1>> stdout_siondump.log 2>> stderr_siondump.log
