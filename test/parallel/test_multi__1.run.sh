...
(
  ...
  grep 'on rank 0' stdout.log | grep -v 'sid    ' > stdout_0.log
  grep 'on rank 0' stderr.log | grep -v 'sid    ' > stderr_0.log

  export LSION_DEBUG=siondump_${TESTNAME}

  ${SIONDUMP} -a -m -l testB.out 1> stdout_siondump.log 2> stderr_siondump.log

  cat stdout_siondump.log | grep -v 'endianness' | grep -v 'SIONlib version' >> stdout_0.log
