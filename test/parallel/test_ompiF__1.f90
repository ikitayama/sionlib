...
! step 1
  ...
  call lfsion_paropen_ompi(trim(filename), file_mode, nfiles, gComm, lComm, &
  ...
  size = 8
  nelem = 1000
  call lfsion_write(buffer(1), size, nelem, sid, nwrote)
  ...
  size = 8
  nelem = 1000
  call lfsion_read(buffer(1), size, nelem, sid, nread)
  !$omp master
  if (rank .eq. 0) then
     write (6,'(A,I6,A,I6,A,I6)') 'on MPI rank ', rank, ', OMP thread ', tid, &
       ': read= ', nread
