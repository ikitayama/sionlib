...
!$omp parallel private(globalrank,tid,sid,buffer,sum,newfname)
      ...
      chunksize=2000000
      nfiles=1
      call lfsion_paropen_ompi(trim(filename),'bw',nfiles, gComm,lComm,
     $     chunksize,fsblksize,globalrank,newfname,sid)
      ...
      size=8
      nelem=1000
      call lfsion_write(buffer,size,nelem,sid,nwrote)
...
!$omp end master
      ...
      call lfsion_parclose_ompi(sid,ierr)
      ...
      call lfsion_paropen_ompi(trim(filename),'br',nfiles, gComm,lComm,
     $     chunksize,fsblksize,globalrank,newfname,sid)
      ...
      size=8
      nelem=1000
      call lfsion_read(buffer,size,nelem,sid,nread)
!$omp master
      if(rank .eq. 0) then
         write (6,'(A,I6,A,I6,A,I6)') 'on MPI rank ',rank,
