...
! step 1
      ...
      chunksize=2000000
      nfiles=1
      call lfsion_paropen_mpi(trim(filename),'bw',nfiles, gComm,lComm,
     $                      chunksize,fsblksize,globalrank,newfname,sid)
      ...
      size=8
      nelem=1000
      call lfsion_coll_fwrite_mpi(buffer,size,nelem,sid,ierr)
      if(rank .eq. 0) then
      ...
      call lfsion_paropen_mpi(trim(filename),'br',nfiles, gComm,lComm,
     $     chunksize,fsblksize,globalrank,newfname,sid)

      buffer=0
      size=8
      nelem=1000
      call lfsion_coll_fread_mpi(buffer,size,nelem,sid,ierr)
      if(rank .eq. 0) then
