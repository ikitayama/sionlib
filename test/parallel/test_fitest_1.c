...
int print_io_info(int sid, int rank) {
  ...
  for(c=0;c<p->nfiles;c++) {
    printf("on rank %2d: io_info [#%02d: fn=%-30s size=%6d role=%-20s]\n",rank,c,p->names[c],(int) p->sizes[c],_lsion_buddy_role_to_str(p->roles[c]));
  }
  return(1);
}
...
#define CHUNKSIZE 1200
    ...
    buddy_args = sion_file_check_par_args_init_mpi("br,buddy",MPI_COMM_WORLD,numfiles,lcomm2);
...
#define CHUNKSIZE 1200
  ...
  /* ------------------------------------------------------------------------ */
  /* TEST E: check restart, no fallback possible (on node 2)                  */
  /* ------------------------------------------------------------------------ */
  if(1){
#define BUFSIZE 1000  
#define CHUNKSIZE 1200
  ...
  /* ------------------------------------------------------------------------ */
  /* TEST F: check restart, fallback on first buddy (on node 0)               */
  /* ------------------------------------------------------------------------ */
  if(1){
#define BUFSIZE 1000  
#define CHUNKSIZE 1200
  ...
  /* ------------------------------------------------------------------------ */
  /* TEST G: check restart, no fallback possible (on node 0)                  */
  /* ------------------------------------------------------------------------ */
  if(1){
#define BUFSIZE 1000  
#define CHUNKSIZE 1200
  ...
  /* ------------------------------------------------------------------------ */
  /* TEST H: check restart, open buddy file (should not hang)                 */
  /* ------------------------------------------------------------------------ */
  if(0){
#define BUFSIZE 1000  
#define CHUNKSIZE 1200
    ...
    /* SCR checks if file can be opened by SIONlib with a callback function which is provided by SIONlib */
    buddy_args = sion_file_check_par_args_init_mpi("br,buddy",MPI_COMM_WORLD,numfiles,lcomm2);
    rc = sion_file_check_par_cb_mpi("testB.out_BUDDY_00.000002", buddy_args);
    ONRANK printf("on rank %2d: sion_file_check_par_cb_mpi returns %d\n",rank, rc);
    sion_file_check_par_args_free_mpi(buddy_args);
