...
int main(int argc, char **argv)
{
  ...
  /* -------------------------- */
  {
    ...
    if (sion_filedesc == NULL) {
      return(_lsion_errorprint(-1,_SION_ERROR_RETURN,"sion_open: cannot allocate filedescriptor structure of size %lu (sion_filedesc), aborting ...\n", (unsigned long) sizeof(sion_filedesc)));
    }
    _lsion_init_filedesc(sion_filedesc);

    /* test check function of sion_cache */
    rc=_lsion_cache_check_env(sion_filedesc);
    printf("_lsion_cache_check_env: returns rc=%d\n",rc);
    printf("_lsion_cache_check_env: usecache=  %d\n",sion_filedesc->usecache);
    printf("_lsion_cache_check_env: cachesize= %d\n",sion_filedesc->cachesize);

    /* test creation of sion_cache */
    sion_filedesc->mode=SION_FILEMODE_WRITE;
    rc=_lsion_cache_init(sion_filedesc);
    printf("_lsion_cache_check_env: returns rc=%d\n",rc);
    printf("_lsion_cache_check_env: cacheid=   %d\n",sion_filedesc->cacheid);
