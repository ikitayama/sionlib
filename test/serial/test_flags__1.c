...
int main(int argc, char** argv)
{
  ...
  /* ------------------------------- */
  {
    ...
    store = _lsion_parse_flags(flags);

    /* iterate over flags */
    for (entry = _lsion_flags_iter(store); entry->next != NULL;
         entry = entry->next) {
      printf("key = '%s'  |  val = '%s'\n", entry->key, entry->val);
    }
    ...
    for (idx = 0; idx < keys_len; idx++) {
      entry = _lsion_flags_get(store, keys[idx]);
      if (entry) {
        printf("_lsion_flags_get(store, \"%s\") = entry\n", keys[idx]);
        printf("entry->value = \"%s\"\n", entry->val);
      }
      else {
        printf("_lsion_flags_get(store, \"%s\") = NULL\n", keys[idx]);
      }
    }

    /* check mask */
    printf("mask = 0x%llx\n", store->mask);
    _lsion_flags_destroy_store(&store);
  }
  ...
  /* ---------------------- */
  {
    ...
    for (idx = 0; idx < flags_len; idx++) {
      store = _lsion_parse_flags(all_flags[idx]);
